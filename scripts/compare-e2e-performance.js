...
function printComparison(comparison) {
  ...
  Object.entries(comparison.metrics).forEach(([metricName, metric]) => {
    // Use new icons: ⚠️ for regression, ✅ for improvement, 👌 for unchanged
    const statusIcon = metric.status === 'IMPROVED' ? '✅' : 
    ...
    const statusIconInline = metric.status === 'IMPROVED' ? '✅' : 
                            metric.status === 'REGRESSED' ? '⚠️' : 
                            metric.status === 'UNCHANGED' ? '👌' : '❓';
  ...
  if (improvements > regressions && improvements > unchanged) {
    console.log(`✅ PERFORMANCE IMPROVED: ${improvements} improvements vs ${regressions} regressions, ${unchanged} unchanged`);
  } else if (regressions > improvements && regressions > unchanged) {
    console.log(`⚠️ PERFORMANCE REGRESSED: ${regressions} regressions vs ${improvements} improvements, ${unchanged} unchanged`);
  } else {
    console.log(`👌 PERFORMANCE UNCHANGED: ${unchanged} unchanged, ${improvements} improvements, ${regressions} regressions`);
  }
