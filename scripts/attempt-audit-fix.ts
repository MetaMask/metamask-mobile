 ...
 * and folds the result into the advisory context file — see parseYarnWhy()
 * below for why that matters.
 * - The AI's proposal is never trusted at face value. For each proposed fix,
 * this script applies it itself, then re-runs the same isAdvisoryCleared /
 * verifyTreeIsClean checks a plain `yarn up`/resolutions attempt would need
 * to pass. Only advisories this script independently re-verified as cleared
 * move from `manual` to `fixed` — an unverified proposal is reverted and the
...
export function parseAuditNdjson(raw: string): AuditAdvisory[] {
  ...
  for (const line of raw.split('\n')) {
    ...
    // `yarn npm audit --json` emits ID as a number — normalize to a string
    // the same way scripts/collect-audit-advisories.ts does. Without this,
    // every advisory here was silently dropped from `remaining` below, so
    // isAdvisoryCleared() saw an empty list and reported every advisory
    // cleared regardless of whether the applied fix actually worked.
    const id = node.children?.ID;
    if (typeof pkg === 'string' && (typeof id === 'string' || typeof id === 'number')) {
      advisories.push({ pkg, id: String(id) });
    }
  }
  return advisories;
}
...
interface YarnWhyInfo {
  /** Every version of `pkg` actually resolved in the tree right now. */
  versions: string[];
  dependents: { list: Dependent[]; truncated: number };
}

/**
 * Runs `yarn why <pkg> --json` once and extracts both things callers need
 * from it, since they'd otherwise run and hand-parse the identical NDJSON
 * separately:
 *
 * - `versions`: every resolved version of `pkg`, used to report a
 * before/after in the fixed-entry summary.
 * - `dependents`: "who requires this and what range did they ask for" — the
 * AI otherwise has no visibility into the dependency tree at all (see
 * task-prompt.md's file access rules), so for a transitive package it has no
 * way to judge whether a `resolutions` pin would land outside some
 * consumer's declared range. Deduped by consumer package name (the same
 * consumer can appear multiple times as different virtual/peer instances)
 * and capped so a widely-used package like `bn.js` doesn't blow up the
 * advisory context file.
 *
 * Each top-level NDJSON entry is a consumer package; its `children` map has
 * one entry per edge to `pkg` — `descriptor` is the range the consumer asked
 * for, `locator` is what actually got resolved.
 */
export function parseYarnWhy(pkg: string, dependentsLimit = 12): YarnWhyInfo {
  const result = tryShQuiet('yarn', ['why', pkg, '--json']);
  if (!result.ok) return { versions: [], dependents: { list: [], truncated: 0 } };
  ...
  for (const line of result.output.split('\n')) {
    ...
    let entry: { value?: unknown; children?: Record<string, { locator?: string; descriptor?: string }> };
    try {
    ...
    for (const [childKey, child] of Object.entries(children)) {
      const versionMatch = /@npm:([^\s]+)/.exec(child.locator || childKey);
  ...
  const allDependents = [...rangesByConsumer.entries()]
  ...
  return {
    versions: [...versions],
    dependents: {
      list: allDependents.slice(0, dependentsLimit),
      truncated: Math.max(0, allDependents.length - dependentsLimit),
    },
  };
}
...
function runAnalyzer(manual: ManualEntry[]): AiProposedFix[] | null {
  ...
  const contextEntries = manual.map((entry) => {
    const { dependents } = parseYarnWhy(entry.pkg);
    return {
      package: entry.pkg,
      ...
      ...(dependents.truncated > 0 ? { dependents_truncated_count: dependents.truncated } : {}),
    };
  });
...
function applyProposedFix(advisory: ManualEntry, fix: AiProposedFix): FixedEntry | null {
  ...
  const beforeVersions = parseYarnWhy(pkg).versions;
  ...
  if (installResult.ok && verifyTreeIsClean() && isAdvisoryCleared(pkg, id)) {
    const afterVersions = parseYarnWhy(pkg).versions;
...
/**
 * applyProposedFix() verifies each advisory only against the tree as it
 * exists at that advisory's own turn. A later advisory's
 * `yarn install --mode=update-lockfile`/`yarn dedupe` can still rewrite
 * yarn.lock in a way that reintroduces an earlier one — nothing checks that
 * until now. Re-runs isAdvisoryCleared() one more time per already-"fixed"
 * entry, against the batch's final tree state, right before a PR would
 * open. Anything that regressed is demoted back to `manual` instead of
 * being reported (and shipped in a PR) as fixed.
 */
export function reverifyFixedBatch(fixed: FixedEntry[]): { stillFixed: FixedEntry[]; regressed: ManualEntry[] } {
  const stillFixed: FixedEntry[] = [];
  const regressed: ManualEntry[] = [];
  for (const entry of fixed) {
    if (isAdvisoryCleared(entry.pkg, entry.id)) {
      stillFixed.push(entry);
      continue;
    }
    console.log(`[${entry.id}] ${entry.pkg}: no longer cleared after a later advisory's fix changed the lockfile — reverting to manual`);
    const { pkg, id, severity, title, url } = entry;
    regressed.push({
      pkg,
      id,
      severity,
      title,
      url,
      reason: "Verified fixed earlier in this run, but a later advisory's fix rewrote yarn.lock and reintroduced this one — needs manual review.",
    });
  }
  return { stillFixed, regressed };
}
...
export function main(): void {
  ...
  const { stillFixed, regressed } = reverifyFixedBatch(fixed);
  stillManual.push(...regressed);

  if (stillFixed.length === 0) {
  ...
  const prUrl = openPrForFixes(repo, ownerGh, stillFixed);
  if (!prUrl) {
  ...
  result.fixed = [...result.fixed, ...stillFixed];
  result.manual = stillManual;
  writeFileSync(resultPath, `${JSON.stringify(result, null, 2)}\n`);
  console.log(`Verified ${stillFixed.length} fix(es) on PR ${prUrl}; ${stillManual.length} advisory/ies still need manual review.`);
}
