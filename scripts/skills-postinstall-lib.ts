/* eslint-disable import-x/no-nodejs-modules, no-undef -- This postinstall script runs in Node via tsx outside the React Native runtime. */
...
export function readSkillsLocal(readFile?: ReadFileSync): Record<string, string> {
...
export function autoUpdateSkills(deps?: PostinstallDeps): boolean {
  const spawn = deps?.spawn ?? spawnSync;
  const stderr = deps?.stderr ?? process.stderr;
  const autoUpdate = run('yarn', ['skills'], spawn, 'inherit');
