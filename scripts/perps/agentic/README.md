...
Cache lives in `$MM_BUILD_CACHE_DIR` (default `~/Library/Caches/mm-mobile-builds` on macOS, `~/.cache/mm-mobile-builds` on Linux), keyed by an agentic-local `@expo/fingerprint` hash computed by `scripts/perps/agentic/lib/compute-cache-fp.js`. This is intentionally a narrower fingerprint than the project-wide `scripts/generate-fingerprint.js` (which EAS Build / OTA depend on): we ignore per-worktree build artifacts (`ios/build/`, `.gradle/`, env-populated `xcconfig`, Xcode `xcuserdata`, etc.) so two slots on the same commit produce the same key and share a cached artifact. Parallel worktrees at the same fingerprint share one artifact through a per-fingerprint mutex: Linux uses `flock(1)` (auto-released by the kernel on process death); macOS, where `flock` is not in base, uses an atomic `mkdir <fp>.lock.d` fallback that is released by the script's `EXIT` trap. If a script is killed with `kill -9` between `mkdir` and the trap, the mutex dir can be left behind — delete it manually under `$MM_BUILD_CACHE_DIR/<plat>/`. Override retention with `BUILD_CACHE_RETAIN=N` (default 5 per platform).
