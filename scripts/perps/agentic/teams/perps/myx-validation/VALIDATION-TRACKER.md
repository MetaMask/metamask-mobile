...
8. **Broker revenue model**: MYX uses referral rebates, not a direct fee. Broker address tags trades (done). Revenue requires `setUserFeeData()` — this is an on-chain call to the broker contract (NOT a backend API). Requires EIP-712 signature from broker owner (`0xAdA1c1...`). **EIP-712 domain spec now known** (MYX team confirmed 2026-03-27): `name: "Broker"`, `version: "1.0"`, `chainId`, `verifyingContract: brokerAddress`. Previous failure was due to using `name: "Metamask Broker"` (display name) and missing `version`. Tested on Linea Sepolia testnet (2026-03-27): still reverts with `NotBrokerSigner(0x315BC4...)`. The domain change shifted the recovered address (different from previous "Metamask Broker" attempt), confirming it affects the hash. Remaining unknowns: struct types may differ on testnet, or `verifyingContract` may not be the broker address. Need MYX team to provide a working testnet example.
