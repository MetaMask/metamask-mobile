...
buildIosSimulator(){
	...
	if [ -n "$IOS_SIMULATOR" ]; then
		npx expo run:ios --no-install --configuration Debug --port $WATCHER_PORT --device "$IOS_SIMULATOR"
	else
		npx expo run:ios --no-install --configuration Debug --port $WATCHER_PORT
	fi
}
