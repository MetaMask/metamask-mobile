...
buildAndroidQAE2E(){
	prebuild_android
	cd android && ./gradlew assembleQaRelease app:assembleQaAndroidTest -PminSdkVersion=26 -DtestBuildType=release
}
...
buildAndroidRunE2E(){
	...
	cd android && ./gradlew assembleProdDebug app:assembleProdAndroidTest -DtestBuildType=debug && cd ..
}
