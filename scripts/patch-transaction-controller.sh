...
COMMENT='+PATCH GENERATED FROM MetaMask/core branch: patch/mobile-transaction-controller-33-0-1\
+This patch backports various transaction controller features from the main branch of MetaMask/core\
+Steps to update patch:\
+* Create a new core branch from: patch/mobile-transaction-controller-33-0-1\
+* Run "yarn build" in the core monorepo\
+* Run "yarn patch:tx <core-directory>" in the mobile repo\
+* Once the new patch is merged, add your changes to: patch/mobile-transaction-controller-33-0-1'
