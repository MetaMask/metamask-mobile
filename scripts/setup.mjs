    ...
    const tasks = [
      {
        ...
        task: async (_, task) => {
          if (GITHUB_CI) {
            ...
            // Use the base $ (not $$) so stderr is captured and can be
            // inspected — $$ uses stdio:'inherit' in CI which leaves
            // error.stderr undefined, making the "already installed" guard
            // always throw.
            try {
