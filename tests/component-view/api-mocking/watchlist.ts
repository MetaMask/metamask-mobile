...
import { clearAllNockMocks } from './nockHelpers';
import type { MockTrendingToken } from './trending';
...
const TOKEN_SEARCH_ORIGIN = 'https://token.api.cx.metamask.io';

/**
 * Overrides the token search endpoint for watchlist search tests.
 * Call after setupTrendingApiFetchMock so search returns specific results.
 */
export function setupWatchlistSearchApiMock(
  results: MockTrendingToken[],
): Scope {
  return nock(TOKEN_SEARCH_ORIGIN)
    .get('/tokens/search')
    .query(true)
    .reply(200, { count: results.length, data: results })
    .persist();
}
