...
const logger = createLogger({
  name: 'WebSocketRegistry',
  level: LogLevel.INFO,
});

/**
 * Mock message type used by WebSocket mock services.
 * Each mock matches incoming messages by substring and replies with a canned response.
 */
 ...
 * explicitly in FixtureHelper — no hidden side effects.
 *
 * Adding a new websocket service:
 * 1. Create `websocket/<name>-mocks.ts`, export a `WebSocketServiceConfig`
 * 2. Import and register it in FixtureHelper
 * 3. Add WSS forwarding rule in MockServerE2E
 */
class WebSocketRegistry {
  ...
  static register(config: WebSocketServiceConfig): void {
    if (WebSocketRegistry.entries.has(config.name)) {
      logger.debug(
        `WebSocket service '${config.name}' is already registered, skipping.`,
      );
      return;
    }
    ...
    logger.info(`Registered WebSocket service: ${config.name}`);
  }
  ...
  static async startAll(
    ...
    await Promise.all(
      allEntries.map(async ([name, entry]) => {
        ...
        logger.info(`Started WebSocket service: ${name} on port ${entry.port}`);
      }),
    );
  }
  ...
  static async stopAll(): Promise<void> {
    ...
    logger.info('All WebSocket services stopped');
  }
   ...
   * Useful for test isolation between spec files.
   */
  static reset(): void {
    WebSocketRegistry.entries.clear();
    logger.debug('WebSocket registry reset');
  }
}
