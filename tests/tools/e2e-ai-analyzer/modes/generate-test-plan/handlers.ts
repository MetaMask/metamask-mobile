...
/**
 * Testing resources - dApps, tools, and utilities for manual testing
 */
interface TestingResource {
  name: string;
  url: string;
  description: string;
  category: 'dapp' | 'snaps' | 'utility' | 'sdk';
}

const TESTING_RESOURCES: TestingResource[] = [
  {
  ...
  {
    name: 'MetaMask SDK React Demo',
    url: 'https://metamask.github.io/metamask-sdk/release-114.0.0/packages/examples/react-demo/build/index.html',
    description: 'SDK connection testing, deeplinks, mobile-web interactions',
    category: 'sdk',
  },
  {
    name: 'MMI Test dApp',
    url: 'https://consensys.gitlab.io/codefi/products/mmi/test-dapp/',
    description: 'Institutional features testing',
    category: 'dapp',
  },
  {
    name: 'Deeplink Generator',
    url: 'https://metamask.github.io/metamask-deeplinks/#',
    description: 'Generate and test MetaMask deeplinks',
    category: 'utility',
  },
  {
    name: 'Snap Simple Keyring',
    url: 'https://metamask.github.io/snap-simple-keyring/latest/',
    description: 'Test snap-based account management',
    category: 'snaps',
  },
  {
    name: 'Snap Account Abstraction Keyring',
    url: 'https://metamask.github.io/snap-account-abstraction-keyring/latest/',
    description: 'Test account abstraction via snaps',
    category: 'snaps',
  },
  {
    name: 'Snaps Directory',
    url: 'https://snaps.metamask.io/',
    description: 'Browse and install snaps for testing',
    category: 'snaps',
  },
];

/**
 * Map feature areas to relevant testing resources
 */
const FEATURE_AREA_RESOURCES: Record<string, string[]> = {
  ...
  SmokePerps: ['Deeplink Generator'],
  SmokePredictions: ['Deeplink Generator'],
};
 ...
 */
export function getAllTestingResources(): TestingResource[] {
...
/**
 * Generates delta markdown for build comparison
 * Used by the fast path when comparing commits
 * Formatted for Slack (no # headers, emojis per item)
 */
export function generateDeltaMarkdown(
  ...
  const fromLabel = fromCommit.substring(0, 7);
  const toLabel = toCommit.substring(0, 7);

  lines.push(`🍒 *Build Delta* (${fromLabel} → ${toLabel})`);
  lines.push(`Release ${version} | Generated: ${generatedDate}`);
  lines.push('');

  // Map cherry-picks to feature areas
  const cherryPicksByArea = new Map<string, CherryPickInfo[]>();
  for (const cp of cherryPicks) {
    const area = extractFeatureAreaFromMessage(cp.message);
    if (area) {
      const existing = cherryPicksByArea.get(area) || [];
      existing.push(cp);
      cherryPicksByArea.set(area, existing);
    }
  }

  // Get sign-off status per team
  const signOffMap = new Map<string, boolean>();
  for (const signOff of teamSignOffs) {
    signOffMap.set(signOff.team.toLowerCase(), signOff.signedOff);
  }

  // Areas needing re-testing
  const areasNeedingRetest = Array.from(cherryPicksByArea.keys());

  if (areasNeedingRetest.length > 0) {
    lines.push(`*${areasNeedingRetest.length} area(s) need re-testing*`);
    lines.push('');

    for (const areaTag of areasNeedingRetest) {
      const displayName = getFeatureAreaDisplayName(areaTag);
    ...
    lines.push('*No areas need re-testing*');
    lines.push('');
    lines.push('No cherry-picks detected between these commits.');
    lines.push('');
  }
