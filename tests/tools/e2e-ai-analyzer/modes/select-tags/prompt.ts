...
SmokeSeedlessOnboarding is a thin device suite (new-user Google/Apple OAuth to home, QR sync SRP, fixture lock/unlock and reset-wallet). Select it for OAuth / SeedlessOnboardingController / social-login sheet / Add Device QR sync / seedless lock-reset device paths, or changes under tests/smoke-appium/seedless/. Do NOT select it solely for Account Already Exists UI, Wallet Setup Completed attribution props, or Import SRP screen validation — those are covered by component-view / unit tests.`;
