...
```bash
yarn performance:swaps:scenario:001
```

Use a different Metro port when needed:

```bash
yarn performance:swaps:scenario:001 --metro-port 8082
```
...
The final status must be `clean`. Preparation and cleanup use exact `SWAPS_PERF_ANALYSIS` markers and refuse partial or ambiguous source changes.
...
The artifact includes driver wall-clock phase durations, sanitized JS `fetch` metadata, targeted render counts, and sanitized console warnings/errors. URLs are reduced to host plus a normalized path; request bodies, response bodies, headers, query strings, wallet addresses, and API keys are not stored.
