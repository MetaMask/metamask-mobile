...
/** Generous until BrowserStack TTC baselines exist. */
export const ONBOARDING_APP_TTC_THRESHOLD: PlatformThreshold = {
  ios: 15_000,
  android: 15_000,
};
...
export async function waitForAppScreenTtc(
  ...
  const timeoutMs = options?.timeoutMs ?? DEFAULT_REQUIRED_TIMEOUT_MS;
 ...
 *
 * Defaults to **optional** (soft-fail): missing probes are logged and skipped so
 * seedless OAuth/nav timers still succeed. Pass `required: true` for cold-start
 * landing where TTC is the primary signal.
 */
export async function addAppScreenTtcTimer(options: {
  ...
  const timeoutMs =
    options.timeoutMs ??
    (required ? DEFAULT_REQUIRED_TIMEOUT_MS : DEFAULT_OPTIONAL_TIMEOUT_MS);
  ...
  try {
    ...
    const timer = new TimerHelper(
      ...
      options.threshold ?? ONBOARDING_APP_TTC_THRESHOLD,
      options.platform,
    );
    ...
    console.warn(
      `[TTC] Skipping optional probe for [${options.screenId}]: ${message}`,
    );
    return null;
  }
}
