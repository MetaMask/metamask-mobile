  ...
  test(
    ...
    async ({ device, performanceTracker }, testInfo) => {
      ...
      const timer1 = new TimerHelper(
        'Google (existing): Tap "Create new wallet" → OnboardingSheet visible',
        { ios: 1500, android: 2000 },
        device,
      );
      const timer2 = new TimerHelper(
        'Google (existing): Tap Google login → account found visible',
        { ios: 15000, android: 15000 },
        device,
      );
      const timer3 = new TimerHelper(
        'Google (existing): Tap account-found login → password screen visible',
        { ios: 4000, android: 4000 },
        device,
      );
      const timer4 = new TimerHelper(
        'Google (existing): Submit password → wallet main visible',
