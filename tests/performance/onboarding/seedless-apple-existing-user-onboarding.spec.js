...
test.describe(`${PerformanceOnboarding} seedless existing user`, () => {
  ...
  test(
    'Seedless Onboarding: Apple Login Returning User (cold start after onboarding)',
    ...
    async ({ device, performanceTracker }, testInfo) => {
      ...
      await onboardingFlowSeedlessNewUser(device, 'apple');
      ...
      timer1.changeName(
        'Apple (returning): After unlock until wallet menu button visible (cold start after seedless onboarding)',
