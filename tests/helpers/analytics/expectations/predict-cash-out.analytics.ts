 ...
 *
 * Note: "Predict Position Viewed" is not asserted here. It is only emitted when
 * `PredictHomePositions` is visible on screen. The wallet uses the sections-based homepage
 * via `Homepage` + `PredictionsSection` — that path does not call `trackPositionViewed`.
 */
