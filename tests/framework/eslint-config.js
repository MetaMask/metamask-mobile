...
// Every glob is rooted at `tests/` because these entries used to be reached
// through an `extends` nested inside a `tests/**/*.{js,ts}` override, which
// intersected the outer and inner file globs.
module.exports = [
  {
    files: ['tests/**/*.{js,ts}'],
    rules: {
  ...
  {
    files: ['tests/**/specs/**/*.{js,ts}'],
    ignores: ['tests/**/specs/**/*.failing.{js,ts}'],
    rules: {
  ...
  // MMQA-2230: ban dual-framework imports in new PO/flow files (error)
  {
    ...
    rules: {
      'no-restricted-imports': ['error', dualFrameworkRestrictedImportOptions],
    },
  },
