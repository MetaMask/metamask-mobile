...
// The Detox-injected `device` global (absent outside Detox jest runs).
declare const device: unknown;

/**
 * adb serial of the Detox-assigned device (e.g. 'emulator-15710'), read from
 * the Detox-injected `device` global. Returns undefined outside Detox runs.
 *
 * Detox runs have no worker-pool assignment (that is Appium CI infra), so
 * without this fallback `adb` runs bare and fails with "more than one
 * device/emulator" whenever an extra device is attached to the host.
 */
function detoxDeviceSerial(): string | undefined {
  try {
...
export function adbDeviceArgs(
  ...
  let serial = resolveWorkerAndroidSerial(env);
  if (!serial && FrameworkDetector.isDetox()) {
    serial = detoxDeviceSerial();
  }
  if (!serial) {
    return [];
  }
