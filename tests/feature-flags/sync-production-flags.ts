...
// Simple ANSI color helpers (avoids chalk dependency)
const c = {
...
export function compareProductionFlagsToRegistry(
  ...
  fullRegistryOverride?: Record<string, unknown>,
): SyncResult {
...
function formatSyncReport(result: SyncResult): string {
  const lines: string[] = [];
  const header = (s: string) => c.bold(c.green(s));
  const sectionTitle = (s: string) => c.bold(c.green(s));
  ...
  if (!result.hasDrift) {
    lines.push(c.green('✓ Registry is in sync with production.'));
    return lines.join('\n');
  }
  ...
  if (result.newInProduction.length > 0) {
    ...
    for (const { name, value } of result.newInProduction) {
      lines.push(` - ${c.red(name)}: ${JSON.stringify(value)}`);
    }
    lines.push('');
  }
  ...
  if (result.hasDrift) {
    ...
    lines.push(
      ` ${c.green('New flags in production')}: ${result.newInProduction.length}`,
    );
    lines.push(
      ` ${c.green('Flags removed from production')}: ${result.removedFromProduction.length}`,
    );
    lines.push(
      ` ${c.green('Value mismatches')}: ${result.valueMismatches.length}`,
    );
    lines.push(
      ` ${c.green('inProd mismatches')}: ${result.inProdMismatches.length}`,
    );
    lines.push(
      ` ${c.green('Total drift')}: ${result.newInProduction.length + result.removedFromProduction.length + result.valueMismatches.length + result.inProdMismatches.length}`,
    );
    lines.push('');
  }
  ...
  lines.push(
    c.bold(
      c.red(
        'To fix: Update tests/feature-flags/feature-flag-registry.ts and run yarn feature-flags:sync to verify.',
      ),
    ),
  );
  lines.push('');
...
export async function updateRegistryFile(result: SyncResult): Promise<void> {
  ...
  /* eslint-disable @typescript-eslint/no-var-requires, @typescript-eslint/no-require-imports -- dynamic require for ts-node compat */
  const prettier = require('prettier') as {
    default: {
      format: (content: string, options?: object) => Promise<string>;
      resolveConfig: (filePath: string) => Promise<object | null>;
    };
  };
  /* eslint-enable @typescript-eslint/no-var-requires, @typescript-eslint/no-require-imports */
  const prettierApi = prettier.default ?? prettier;
  const prettierOptions = await prettierApi.resolveConfig(REGISTRY_FILE_PATH);
  const formatted = await prettierApi.format(content, {
    ...prettierOptions,
...
// JEST_WORKER_ID is set when running under Jest
