...
/* eslint-enable import-x/no-nodejs-modules, import-x/no-namespace */
...
const SCAN_DIRS = ['tests/smoke', 'tests/regression'];
...
export function generateReport(repoRoot: string): CoverageReport {
  ...
  const deprecatedFlags = flagResults.filter(
    (f) => f.status === FeatureFlagStatus.Deprecated,
  );
  const inProdFlags = flagResults.filter((f) => f.inProd);
  ...
  return {
    ...
    summary: {
      ...
      deprecatedFlags: deprecatedFlags.length,
      inProdFlags: inProdFlags.length,
...
function printReport(report: CoverageReport): void {
  ...
  console.log('');
  console.log(
    '  State detection considers the production default (served by remoteFeatureFlagsHelper)',
  );
...
function main(): void {
  ...
  if (!fs.existsSync(pkgPath)) {
    ...
    console.error(
      'Make sure you are running this from the metamask-mobile repo root.',
    );
    process.exit(1);
  }
