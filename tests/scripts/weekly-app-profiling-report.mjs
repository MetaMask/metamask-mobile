...
function buildSlackMarkdown(report) {
  ...
  lines.push('');
  lines.push(
    `**Window:** ${meta.since.slice(0, 10)} → ${meta.until.slice(0, 10)} (${meta.days}d) · **Device:** ${meta.device}`,
  );
  lines.push(
    `**PRs with perf results:** ${prSummary.withResults} · **All passed:** ${prSummary.allPassed} · **With failures:** ${prSummary.withFailures}`,
  );
  lines.push(
    `**Profiling samples matched:** ${meta.profilingSamplesMatched} across ${meta.artifactPrs} artifact downloads`,
  );
  lines.push('');
  lines.push(`### Top ${scenarios.length} scenarios — profiling averages`);
  lines.push('');
  lines.push(
    `| # | Scenario | Runs | Fail% | CPU avg | Mem avg | Mem max | Slow frames | Issues | n |`,
  );
  lines.push(`|---|---|---|---|---|---|---|---|---|---|`);
  for (const s of scenarios) {
    lines.push(
      `| ${s.rank} | ${s.scenario.replaceAll('|', '\\|')} | ${s.executions} | ${s.failRatePct}% | ${fmtAvg(s.profiling.cpuAvg, '%')} | ${fmtAvg(s.profiling.memAvgMb, ' MB')} | ${fmtAvg(s.profiling.memMaxMb, ' MB')} | ${fmtAvg(s.profiling.slowFramesPct, '%')} | ${fmtAvg(s.profiling.issues)} | ${s.profiling.samples} |`,
    );
  }
  ...
  lines.push('');
  lines.push(
    `_Generated by \`tests/scripts/weekly-app-profiling-report.mjs\`_`,
  );
  return `${lines.join('\n')}\n`;
}
