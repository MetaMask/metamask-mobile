  ...
  return [...scenarios]
    ...
    .sort((a, b) => {
      const score = (scenario) => {
        ...
        const fail = scenario.failRatePct ?? 0;
        const memory = scenario.profiling.memMaxMb?.avg ?? 0;
        return slow * 2 + fail + Math.max(0, memory - 850) / 10;
      };
...
function buildSlackMarkdown(report) {
  ...
  lines.push('');
  lines.push(
    `_Full scenario metrics and merged PR details remain available in report.json._`,
  );
  lines.push('');
