...
const SLACK_TEAM_IDS = {
  '@swap-bridge-dev-team': 'S04NGHK3U9Z',
  '@metamask-onboarding-team': 'S090QC71NQ2',
  '@metamask-mobile-platform': 'S04EF225J1M',
  '@mm-perps-engineering-team': 'S094DMAQNCV',
  '@accounts-team': 'S05NSFC03GF',
  '@assets-dev-team': 'S09C9U4K953',
  '@team-predict': 'S095BEYMASG',
};
...
function selectFeaturedScenarios(scenarios) {
  return [...scenarios]
    .filter((scenario) => scenario.profiling.samples >= 2)
    .sort((a, b) => {
      const score = (scenario) => {
        const slow = scenario.profiling.slowFramesPct?.avg ?? 0;
        const memory = scenario.profiling.memMaxMb?.avg ?? 0;
        const issues = scenario.profiling.issues?.avg ?? 0;
        return slow * 2 + issues + Math.max(0, memory - 850) / 10;
      };
...
function getScenarioTeams(prs) {
  const teamsByScenario = new Map();
  for (const pr of prs) {
    for (const test of pr.passedTests || []) {
      if (!test.team) continue;
      const teams = teamsByScenario.get(test.testName) || new Set();
      teams.add(test.team);
      teamsByScenario.set(test.testName, teams);
    }
  }
  return teamsByScenario;
}

function formatTeamTags(teams = []) {
  return teams
    .map((team) => {
      const id = SLACK_TEAM_IDS[team];
      return id ? `<!subteam^${id}|${team.replace(/^@/, '')}>` : team;
    })
    .join(' ');
}
...
function formatScenarioSlackCard(s) {
  const p = s.profiling;
  const lines = [];
  const teamTag = s.teams?.length ? `${formatTeamTags(s.teams)} ` : '';
  lines.push(`*${s.rank}. ${teamTag}${s.scenario}*`);
  lines.push(`Profiling samples n=${p.samples}`);
  lines.push(
...
function buildSlackMarkdown(report) {
  const lines = [];
  const { meta, scenarios, leads } = report;
  const featuredScenarios = selectFeaturedScenarios(scenarios);
  const topLeads = selectTopLeads(leads);

  lines.push(`*Weekly App Profiling Report*`);
  lines.push('');
  lines.push(`*Window:* ${meta.since.slice(0, 10)} → ${meta.until.slice(0, 10)}`);
  lines.push('');
  lines.push(`*Executive summary*`);
  lines.push('');
  lines.push(`_AI-generated summary grouped by team._`);
  lines.push('');
  lines.push(`*Data details*`);
  lines.push('');
  for (const s of featuredScenarios) {
  ...
  if (report.aiInsights) {
    ...
    lines.push(`_Pending AI pass._`);
  }

  lines.push('');
  lines.push(`*Priority actions*`);
  lines.push('');
  lines.push(`_AI-generated: maximum 3 actions._`);
  lines.push('');
  lines.push(
    `_Full app-profiling metrics remain available in report.json._`,
  );
  lines.push('');
  lines.push(`_Generated by tests/scripts/weekly-app-profiling-report.mjs_`);
  lines.push('');
  lines.push(`*Setup:* Weekly BrowserStack app-profiling rollup (last 7 days)`);
  lines.push(
    `*Disclaimer:* This report is for TESTING purposes only and should not be treated as a production alert.`,
  );
  lines.push(`*CC:* <@UEYQL2PEV>`);
  lines.push('');
  return `${lines.join('\n')}\n`;
}
...
function buildAiBriefing(report) {
  ...
  lines.push(`Produce an executive summary grouped by team, followed by **3 actionable investigation insights** for the QA/performance audience.`);
  lines.push('');
  lines.push(`## Rules`);
  lines.push(`- Focus on what is worth investigating next (not generic advice).`);
  lines.push(`- Use only app-profiling metrics: CPU, memory, slow frames, issues, app size, ANRs, and frozen frames.`);
  lines.push(`- Do not mention test errors, quality gates, failure rates, or test stability.`);
  ...
  lines.push(`### Executive summary`);
  lines.push(`- <@TEAM_ID> [Profiling insight supported by metrics]`);
  lines.push(`- <@TEAM_ID> [Profiling insight supported by metrics]`);
  lines.push(`- <@TEAM_ID> [Profiling insight supported by metrics]`);
  lines.push('');
  lines.push(`### Data details`);
...
async function main() {
  ...
  const scenarios = top.map((t, idx) => {
    ...
    const scenarioTeams = getScenarioTeams(prs).get(t.test);
    return {
      rank: idx + 1,
      scenario: t.test,
      teams: scenarioTeams ? [...scenarioTeams] : [],
      executions: t.count,
      pass: t.pass,
      fail: t.fail,
  ...
  report.leads = buildInvestigationLeads({ scenarioRows: scenarios });
