...
export function getEvmAssetId(token: typeof ETH_TOKEN) {
...
function getTokenAmount(token: typeof ETH_TOKEN) {
  if (token.symbol === 'ETH') return '10';
  if (token.symbol === 'DAI') return '5000';
  return '10000';
}
...
export function createHomepageTokensFilterFixture() {
