...
const multiSrpFixture = () =>
  new FixtureBuilder().withMultiSRPKeyringController().build();

async function withGetEntropySnap(
...
appiumTest.describe(SmokeSnaps('Get Entropy Snap Tests'), () => {
  appiumTest(
    'signs a message with default Snap entropy',
    async ({ driver: _driver, currentDeviceDetails }) => {
      await withGetEntropySnap(currentDeviceDetails, async () => {
  ...
  appiumTest(
    'shows an error for an invalid entropy source',
    async ({ driver: _driver, currentDeviceDetails }) => {
      await withGetEntropySnap(currentDeviceDetails, async () => {
