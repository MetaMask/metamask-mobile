...
/**
 * Setup Multichain Provider session.
 * Each test re-runs this since Appium tears down the WebDriver session per test.
 */
async function setupMultichainProviderSession(): Promise<void> {
...
async function runMultichainChainTest(
  ...
  await withSnapsFixtures(
    ...
    async () => {
      await setupMultichainProviderSession();
      await exerciseMultichainChain(chain);
    },
  );
}
...
appiumTest.describe(SmokeSnaps('Multichain Provider Snap Tests'), () => {
  appiumTest(
    ...
    async ({ driver: _driver, currentDeviceDetails }) => {
      await runMultichainChainTest(currentDeviceDetails, CHAINS[0]);
    },
  );
  ...
  appiumTest(
    ...
    async ({ driver: _driver, currentDeviceDetails }) => {
      await runMultichainChainTest(currentDeviceDetails, CHAINS[1]);
    },
  );
  ...
  appiumTest(
    ...
    async ({ driver: _driver, currentDeviceDetails }) => {
      await runMultichainChainTest(currentDeviceDetails, CHAINS[2]);
    },
  );
});
