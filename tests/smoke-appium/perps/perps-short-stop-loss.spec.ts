...
appiumTest.describe(SmokePerps('Perps Pro - Short stop loss'), () => {
  appiumTest(
    'closes a short in Pro mode when mark price crosses the stop loss trigger',
    async ({ driver: _driver, currentDeviceDetails }) => {
      await withFixtures(
        {
          fixture: buildPerpsSmokeFixture(),
        ...
        async ({ commandQueueServer }: TestSuiteParams) => {
          ...
          await PerpsProMarketView.selectDirection('short');
          ...
          await Utilities.executeWithRetry(
            ...
            },
            {
              ...
              timeout: 30000,
              description:
                'wait for Pro short position to close after stop loss trigger',
            },
          );
        },
      );
