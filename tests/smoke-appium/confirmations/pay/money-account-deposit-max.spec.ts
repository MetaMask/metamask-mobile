...
const MONEY_DEPOSIT_HOLDINGS: TokenHolding[] = [
  { ...PREDEFINED_TOKENS.ETHEREUM.USDC, amount: '50' },
...
appiumTest.describe(
  SmokeConfirmations('MM Pay - Money Account deposit max'),
  () => {
    appiumTest.describe.configure({ timeout: 250_000 });

    appiumTest(
      'deposits max $50 with Mainnet USDC to mUSD on Monad, confirms, and sees it in Money activity',
        ...
        await withFixtures(
          ...
          async () => {
            ...
            await TransactionPayConfirmation.verifyCustomAmount(
              '50',
              'Max of $50 USDC should set amount to $50',
            );
