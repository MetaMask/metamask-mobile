...
import { Assertions, Matchers, Gestures, Utilities } from '../../../framework/index.js';
...
appiumTest.describe(SmokeConfirmations('MM Pay - Perps deposit'), () => {
  ...
  appiumTest('deposits $80, verifies the MM Pay quote, confirms the transaction, and sees it in perps activity', async ({
    driver: _driver,
    ...
    await withFixtures(
      ...
      async () => {
        ...
        // The keypad
        const depositKeyboard = Matchers.getElementByID('deposit-keyboard');
        await Assertions.expectElementToBeVisible(depositKeyboard, { timeout: 10000 });
        
        await Gestures.waitAndTap(Matchers.getElementByID('keypad-key-8'));
        await Gestures.waitAndTap(Matchers.getElementByID('keypad-key-0'));
        await Gestures.waitAndTap(Matchers.getElementByID('deposit-keyboard-done-button'));

        // Transaction Fee
        const transactionFee = Matchers.getElementByID('transaction-fee');
        await Assertions.expectElementToBeVisible(transactionFee, { timeout: 10000 });

        // Confirm
        const confirmButton = Matchers.getElementByID('confirm-button');
        await Gestures.waitAndTap(confirmButton);

        // Wait for it to close/transition
        await Assertions.expectElementToNotBeVisible(confirmButton, {
          timeout: 25000,
