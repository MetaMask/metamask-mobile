...
appiumTest.describe(SmokeMMConnect('RN legacy EVM connect'), () => {
    ...
    async ({ driver: _driver, currentDeviceDetails }) => {
      await withFixtures(
        ...
        async () => {
          ...
          const signResponse =
            await RNPlaygroundDapp.getLegacyEvmResponseText();
          console.log(`personal_sign response: ${signResponse}`);
          console.log(
            `personal_sign contains 0x: ${signResponse.includes('0x')}`,
          );
          ...
          console.log(
            `eth_sendTransaction (cancelled) response: ${txResponse}`,
          );
          console.log(
          ...
          console.log(
            `Chain ID after dapp switch to Polygon: ${polygonChainId}`,
          );
          console.log(
            `Chain ID contains 0x89: ${polygonChainId.includes('0x89')}`,
          );
        },
      );
    },
