...
const smokeTags = {
  ...
  smokeSeedlessOnboarding: {
    ...
    description:
      'Tests seedless onboarding with social login providers (Google and Apple) and post-onboard device flows. Covers new user wallet creation via Google and Apple OAuth (password creation, MetaMetrics opt-in, wallet home). Also covers QR sync SRP (new-user and existing-user mobile ↔ extension), and lock/unlock and reset-wallet after Google social login (may be skipped when flaky). Specs live in tests/smoke-appium/seedless/. Account Already Exists UI, Wallet Setup Completed attribution props, and Import SRP screen validation are covered by component-view / unit tests — do not re-add those as Appium-only coverage. When changes touch OAuth, SeedlessOnboardingController, social login UI, the onboarding sheet, Add Device / QR sync, or seedless lock/reset, select this tag. Related to SmokeWalletPlatform for wallet lifecycle, SmokeAccounts for multi-SRP import, and wallet analytics smokes for Wallet Setup Completed attribution.',
  },
  smokeBrowser: {
    tag: 'SmokeBrowser:',
    description:
