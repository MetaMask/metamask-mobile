...
const smokeTags = {
  smokeAccounts: {
    tag: 'SmokeAccounts:',
    description:
      'Tests account security, multi-account management, and profile sync within the wallet. Specs live in tests/smoke-appium/accounts/ and run via Appium smoke CI (appium-smoke-tests-{android,ios}); the same SmokeAccounts tag also gates any remaining Detox coverage. Covers Secret Recovery Phrase (SRP) protection flows including the reveal quiz validation in Settings, SRP export from both Settings and account action menus, and wallet details credential display. Also tests multi-account workflows: creating new HD wallet accounts, adding QR-based hardware wallet accounts, importing accounts via private key, account switching and selection, account renaming, and managing account visibility in the account list. Covers profile sync via the Profile Sync Controller: account sync toggle, multi-SRP synchronization, automatic account discovery, imported-account exclusion, contact/address book sync toggle, contact creation and sync propagation, and verifying synced data persists after restart. Integrates with the AccountSelector and RevealPrivateCredential components. When changes touch multi-SRP architecture, account list, SRP export, or sync flows, also select SmokeWalletPlatform.',
  },
  smokeConfirmations: {
  ...
  smokeWalletPlatform: {
    ...
    description:
      'Tests core wallet platform features and services. Covers the Trending discovery tab: search functionality, browsing content feeds (Predictions, Tokens, Perps, Sites sections), and browser navigation integration. Trending is the connecting point for all subsections—changes to Perps, Predictions, or Tokens views (headers, lists, full views) that are embedded in Trending affect this tag. Tests transaction history: displaying incoming/outgoing ETH transactions, token transfer details, and privacy mode support to hide sensitive balances. Validates wallet lifecycle analytics tracking for new wallet creation and SRP import events. Tests multi-SRP wallet architecture: importing additional Secret Recovery Phrases, adding accounts to different SRPs, exporting SRP from Settings and account action menus, and managing separate account hierarchies per SRP. Covers account deletion flows and EVM provider event handling (accountsChanged, chainChanged) for dApp communication. Integrates with SmokeAccounts for account management and sync, SmokeSwap/SmokeStake/SmokeMoney for activity display, and SmokePerps (Perps section inside Trending).',
  },
  smokeMoney: {
    tag: 'SmokeMoney:',
    description:
