...
- **`Utilities.ts`** - Core utilities, configuration management, and helper functions
  - Key methods: `waitForElementToStopMoving()`, `waitForElementToBeEnabled()`, `waitForReadyState()`, `executeWithRetry()`
- **`types.ts`** - TypeScript type definitions for all framework options

#### Key Features:
- ✅ **Auto-retry with exponential backoff** - Handles flaky network/UI conditions
...
```typescript
// Configure timeouts for test suite
beforeAll(() => {
  Utilities.configure({ timeout: 20000, retryInterval: 500 });
});
...
- **"Element not found"** → Use framework retry mechanisms and element stability checking
- **"Deprecated method warning"** → Check `@deprecated` tag for modern replacement
- **"Test is slow"** → Optimize timeouts, avoid unnecessary delays, use parallel operations
