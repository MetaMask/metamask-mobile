/**
 * Request allowlist/blocklist configuration for e2e test mocking
 *
 * This configuration controls which external requests are allowed to pass through
 * to real servers vs being mocked during e2e tests.
 */

/**
 * Hosts that are always allowed to receive real requests
 * These should be local development servers or critical services
 */
...
/**
 * Specific URLs that are allowed to pass through to real servers
 * Use this for granular control over specific endpoints
 */
export const ALLOWLISTED_URLS = [
  // Temporarily allow existing live requests during migration
  // TODO: Create proper mocks and remove these URLs over time (our goal is to have no live requests in e2e tests)
...
/**
 * Check if a host should be allowed to pass through
 */
export const isHostAllowed = (host: string) => ALLOWLISTED_HOSTS.includes(host);

/**
 * Check if a URL should be allowed to pass through
 */
export const isUrlAllowed = (url: string) => ALLOWLISTED_URLS.includes(url);
