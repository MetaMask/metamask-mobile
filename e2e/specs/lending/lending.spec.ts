...
**/
  ...
  // ${action.dp}   | ${depositAmt}  | ${'USDC'}      | ${'ABASUSDC'}   | ${CustomNetworks.Tenderly.Base}
  // ${action.wt}   | ${withdrawAmt} | ${'ABASUSDC'}  | ${'USDC'}       | ${CustomNetworks.Tenderly.Base}
  // ${action.dpm}  | ${depositAmt}  | ${'USDC'}      | ${'ABASUSDC'}   | ${CustomNetworks.Tenderly.Base}
      ...
      await withFixtures(
        {
          ...
          fixture: new FixtureBuilder()
            ...
            .withRandomImportedAccountKeyringController(wallet)
            .withPreferencesController({ smartTransactionsOptInStatus: false })
            .withTokens(
              [srcTokenDetails, dstTokenDetails],
              chainId,
        ...
        async () => {
          ...
          if (type === action.dp) {
            ...
            await Assertions.checkIfVisible(
              WalletView.tokenInWallet(`${tokenBalance} ${srcTokenSymbol}`),
            );
          ...
          if (type === action.dp) {
            ...
            await Assertions.checkIfVisible(EarnView.depositText);
          } else if (type === action.wt) {
            ...
            await Assertions.checkIfVisible(EarnView.withdrawText);
          } else if (type === action.dpm) {
            console.log(`Tapping on ${srcTokenSymbol}`);
            await WalletView.tapTokenById(srcTokenSymbol);
            console.log('Scrolling down');
          ...
          if (type === action.dp || type === action.dpm) {
            ...
            await Assertions.checkIfVisible(EarnView.spendCapText);
            await Assertions.checkIfVisible(
              EarnView.tokenInReview(`${quantity} ${srcTokenSymbol}`),
            );
            ...
            await Assertions.checkIfVisible(EarnView.completeStepProgress);
            console.log('Tapping on Confirm button');
          ...
          await Assertions.checkIfVisible(EarnView.txtRequest);
          console.log('Tapping on Confirm button');
          await TestHelpers.delay(1000);
          await EarnView.tapConfirm();
          console.log('Checking on transaction confirmation');
          await Assertions.checkIfVisible(EarnView.txtSubmittedToast);
          await Assertions.checkIfNotVisible(EarnView.txtSubmittedToast);

          // 7. Check transaction history
          console.log('Checking on transaction history');
          await Assertions.checkIfVisible(EarnView.txtHistory);
          if (type === action.dp || type === action.dpm) {
            console.log('Checking on approve transaction');
            await Assertions.checkIfVisible(EarnView.txtApprove);
            console.log('Checking on lending deposit transaction');
            await Assertions.checkIfVisible(EarnView.txtLending);
          } else if (type === action.wt) {
            console.log('Checking on lending withdrawal transaction');
            await Assertions.checkIfVisible(EarnView.txtWithdraw);
          }
