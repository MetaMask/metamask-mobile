...
HTTP Daemon (createServer)
   ├─ Package: tools, knowledge store, MobilePlatformDriver
   └─ Consumer: MetaMaskMobileSessionManager, capabilities, Metro attach
    │
    ▼
iOS Simulator + @metamask/device-mcp (IdbBackend)
```
...
| **Platform Driver** | `MobilePlatformDriver`          | Low-level driver that communicates with the simulator via `DeviceBackend`.          |
| **Device Backend**  | `DeviceBackend (IdbBackend)`    | iOS device automation backend provided by `@metamask/device-mcp`.                   |
...
5. The **@metamask/device-mcp (IdbBackend)** executes the action and returns the new UI state.
...
5. **idb-companion**: Install via `brew install idb-companion`
...
| `capabilities/`        | Mobile-specific capabilities like `MobilePlatformDriver` and `MetroAttach`.               |
