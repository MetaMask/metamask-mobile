...
describe('readAppBundleMetadata', () => {
  ...
  it('throws IOSLaunchError when CFBundleIdentifier cannot be read', () => {
    mockedExecFileSync.mockImplementation((file, args) => {
      const argsArr = args as string[];
      if (
        file === 'defaults' &&
    ...
    expect(() => readAppBundleMetadata('/tmp/MetaMask.app')).toThrow(
      IOSLaunchError,
    );
    expect(() => readAppBundleMetadata('/tmp/MetaMask.app')).toThrow(
      'Cannot read CFBundleIdentifier',
    );
  });

  it('throws IOSLaunchError for non-existent app path with unreadable plist', () => {
    mockedExecFileSync.mockImplementation((file, args) => {
      const argsArr = args as string[];
      if (file === 'defaults' && argsArr[0] === 'read') {
        throw new Error('read failed');
    ...
    expect(() => readAppBundleMetadata('/nonexistent/MetaMask.app')).toThrow(
      IOSLaunchError,
    );
    expect(() => readAppBundleMetadata('/nonexistent/MetaMask.app')).toThrow(
      'Cannot read CFBundleIdentifier',
