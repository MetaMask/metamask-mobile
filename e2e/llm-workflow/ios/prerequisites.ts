...
export async function validateIOSPrerequisites(input: {
  ...
  return {
    simulatorDeviceId,
    appBundlePath,
    appBundleId: readAppBundleId(appBundlePath),
...
function readAppBundleId(appBundlePath: string): string {
  return readPlistKey(appBundlePath, 'CFBundleIdentifier') ?? 'io.metamask.MetaMask';
}
