...
export default class MultichainUtilities {
     ...
     */
    static validateSessionStructure(sessionResult: unknown): sessionResult is SessionResult {
     ...
     */
    static validateSessionContainsChains(sessionResult: SessionResult, expectedChainIds: string[]): boolean {
        if (!this.validateSessionStructure(sessionResult) || !sessionResult.success) {
            return false;
        }
        ...
        const expectedScopes = expectedChainIds.map(chainId => this.getEIP155Scope(chainId));

        return expectedScopes.every(scope => {
            const scopeData = sessionScopes[scope];
            return scopeData?.accounts &&
    ...
    static getEVMChainCount(sessionResult: SessionResult): number {
        ...
        return Object.keys(sessionScopes).filter(scope => scope.startsWith('eip155:')).length;
    }
     ...
     */
    static generateSessionAssertions(sessionResult: unknown, expectedChainIds: string[]): SessionAssertions {
        ...
        assertions.chainsValid = this.validateSessionContainsChains(validSessionResult, expectedChainIds);
    ...
    static logSessionDetails(sessionResult: SessionResult, testName: string = 'Session Test'): void {
        // eslint-disable-next-line no-console
        ...
        if (sessionResult.success && sessionResult.sessionScopes) {
            ...
            chainIds.forEach(chainId => {
                ...
                // eslint-disable-next-line no-console
                console.log(`  • ${networkName} (${scope}): ${scopeData?.accounts?.length || 0} accounts`);
            });
        }
        // eslint-disable-next-line no-console
        console.log('');
