...
import { AppProfilingDataHandler } from '../services/providers/browserstack/AppProfilingDataHandler';
import type { DeviceConfig, WebDriverConfig } from '../types';
import {
  getProjectFromTest,
  type DeviceInfo,
  type DeviceMetricsGroup,
  type MetricsEntry,
  type ProfilingDataOrError,
  type ProfilingSummaryOrError,
  type SessionData,
} from './reporter.types';
import fs from 'fs';
...
/**
 * Extract device info from DeviceConfig
 */
function deviceConfigToInfo(config: DeviceConfig): DeviceInfo {
  return {
...
class PerformanceReporter implements Reporter {
  ...
  onTestEnd(test: TestCase, result: TestResult): void {
    ...
    const project = getProjectFromTest(test);
    ...
    if (sessionAttachment?.body) {
      try {
        ...
        console.log(`❌ Error parsing session data: ${errorMessage}`);
      }
    }
  ...
  getDeviceInfo(test: TestCase, _result: TestResult): DeviceInfo {
    // Get project config by traversing the suite hierarchy
    const project = getProjectFromTest(test);
    const projectUse = project?.use as WebDriverConfig | undefined;

    // Path 1: project.use.device (standard Playwright project config)
    if (projectUse?.device) {
      return deviceConfigToInfo(projectUse.device);
    }
   ...
   * @param path - Dot-separated path (e.g., 'cpu.avg')
   * @param defaultValue - Default value if path doesn't exist
   * @returns The value at the path or default value
   */
  getNestedProperty<T = unknown>(
    obj: unknown,
    propertyPath: string,
    defaultValue: T = 'N/A' as T,
  ): T {
   ...
   * @param profilingData - The profiling data object
   * @param profilingSummary - The profiling summary object
   * @returns True if data is valid
   */
  isValidProfilingData(
    profilingData: ProfilingDataOrError | null | undefined,
    profilingSummary: ProfilingSummaryOrError | null | undefined,
  ): boolean {
    return Boolean(
      profilingData &&
        !profilingData.error &&
  ...
  async onEnd(): Promise<void> {
    ...
    if (this.sessions.length > 0 && hasCredentials && isBrowserStackRun) {
      ...
      for (const session of this.sessions) {
        try {
          // Fetch video URL
          const videoURL = await browserStackAPI.getVideoURL(
            session.sessionId,
            60,
            3000,
          );
          ...
          try {
            ...
            if (profilingResult.error) {
              ...
              session.profilingData =
                profilingResult.profilingData ?? undefined;
              session.profilingSummary =
                profilingResult.profilingSummary ?? undefined;
              console.log(
            ...
            const errorMessage =
              error instanceof Error ? error.message : String(error);
            console.log(
              `⚠️ Failed to fetch profiling data for ${session.testTitle}: ${errorMessage}`,
            );
            session.profilingData = {
    ...
    const testName = this.metrics[0].testName
      ...
      .substring(0, 30);
    try {
      // Ensure reports directory exists - use e2e/reports/performance for better organization
      const e2eRoot = path.resolve(__dirname, '..', '..');
      const reportsDir = path.join(e2eRoot, 'reports', 'performance');
      ...
      if (this.metrics.length > 0) {
        ...
        const metricsByDevice: Record<string, DeviceMetricsGroup> = {};
        metricsWithVideo.forEach((metric) => {
          const metricDevice = metric.device ?? {
            name: 'Unknown',
            osVersion: 'Unknown',
          };
          const deviceKey = `${metricDevice.name}-${metricDevice.osVersion}`;
        ...
        Object.entries(metricsByDevice).forEach(([_deviceKey, deviceData]) => {
        ...
        const html = `
          ...
          <body>
            <h1>Performance Report - ${
              this.metrics[0].device?.name ?? 'Unknown'
            } - OS version: ${this.metrics[0].device?.osVersion ?? 'Unknown'}</h1>
