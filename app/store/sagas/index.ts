...
import { authenticationSaga } from '../../core/redux/slices/authentication/sagas';

// ==========================================================================
// OLD AUTH SAGAS REMOVED
// These have been replaced by the new authenticationSaga in app/core/redux/slices/authentication/sagas.ts
// - authStateMachine → manageAuthenticationLifecycleSaga
// - appLockStateMachine → handleAppLockSaga
// - biometricsStateMachine → handleBiometricsSaga
// - lockKeyringAndApp → lockKeyringAndAppSaga
// ==========================================================================
...
export function* rootSaga() {
  ...
  // OLD: yield fork(authStateMachine); ← REMOVED - Replaced by authenticationSaga
  yield fork(basicFunctionalityToggle);
  yield fork(handleDeeplinkSaga);
  ///: BEGIN:ONLY_INCLUDE_IF(preinstalled-snaps,external-snaps)
  yield fork(handleSnapsRegistry);
  ///: END:ONLY_INCLUDE_IF
  yield fork(authenticationSaga); // NEW - Full authentication control
}
