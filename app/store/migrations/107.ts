...
export default async function migrate(state: unknown) {
  ...
  try {
    ...
    if (
      ...
      for (const controllerName of Object.keys(controllers)) {
        try {
          ...
          captureException(
            new Error(
              `Migration 107: Failed to migrate ${controllerName} to individual storage: ${String(
                error,
              )}`,
            ),
          );
      ...
      if (failedControllers > 0) {
        captureException(
          new Error(
            `Migration 107: ${failedControllers} controllers failed to migrate, preserving their state in redux-persist`,
          ),
        );
      }
    ...
    captureException(
      new Error(
        `Migration 107: Failed to migrate from redux-persist to individual controller storage: ${String(
          error,
        )}`,
      ),
    );
