...
/**
 * Migration 81: set completedOnboarding based on the state of the KeyringController.
 */
const migration = (state: unknown): unknown => {
  const migrationVersion = 81;
  ...
  try {
    ...
    captureException(
      new Error(
        `Migration 081: setting completedOnboarding failed with error: ${error}`,
      ),
    );
    return state;
  }
