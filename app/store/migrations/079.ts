...
/**
 * Migration 79: removes from the TokensController state all tokens that belong to an EVM account that has been removed.
...
const migration = (state: unknown): unknown => {
  ...
  if (
    ...
    captureException(
      new Error(
        `FATAL ERROR: Migration 79: Invalid TokenBalancesController state error: '${typeof tokenBalancesControllerState}'`,
      ),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(
        `FATAL ERROR: Migration 79: Invalid TokensController state error: '${typeof tokensControllerState}'`,
      ),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(
        `FATAL ERROR: Migration 79: Invalid AccountsController state error: '${typeof accountsControllerState}'`,
      ),
    );
    return state;
  }
