...
export default async function migrate(stateAsync: unknown) {
  ...
  if (!isObject(networkControllerState)) {
    captureException(
      new Error(
        `Migration 36: Invalid NetworkController state: '${typeof networkControllerState}'`,
      ),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(
        `Migration 36: Invalid NetworkController networkDetails state: '${typeof networkControllerState.networkDetails}'`,
      ),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(
        `Migration 36: Invalid NetworkController providerConfig state: '${typeof networkControllerState.providerConfig}'`,
      ),
    );
    return state;
  }
