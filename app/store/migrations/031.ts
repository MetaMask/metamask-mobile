...
export default function migrate(state: unknown) {
  if (!isObject(state)) {
    captureException(
      new Error(`Migration 31: Invalid state error: '${typeof state}'`),
    );
    return state;
  }
  ...
  if (!isObject(state.engine)) {
    captureException(
      new Error(
        `Migration 31: Invalid engine state error: '${typeof state.engine}'`,
      ),
    );
    return state;
  }
  ...
  if (!isObject(state.engine.backgroundState)) {
    captureException(
      new Error(
        `Migration 31: Invalid engine backgroundState error: '${typeof state
  ...
  if (!isObject(networkControllerState)) {
    captureException(
      new Error(
        `Migration 31: Invalid NetworkController state error: '${typeof networkControllerState}'`,
      ),
    );
    return state;
  }
  ...
  if (!networkControllerState.providerConfig.chainId) {
    captureException(
      new Error(
        `Migration 31: NetworkController providerConfig chainId not found: '${JSON.stringify(
