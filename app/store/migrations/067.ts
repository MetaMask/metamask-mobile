...
export default async function migrate(stateAsync: unknown) {
  ...
  if (!isObject(state)) {
    captureException(
      new Error(`Migration 62: Invalid root state: '${typeof state}'`),
    );
    return state;
  }
  ...
  if (!isObject(state.engine) || !hasProperty(state, 'engine')) {
    captureException(
      new Error(
        `Migration 62: Invalid or missing 'engine' in state: '${typeof state.engine}'`,
      ),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(
        `Migration 62: Invalid or missing 'backgroundState' in engine: '${typeof state
