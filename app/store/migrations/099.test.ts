...
describe('Migration 99: Update bridge txHistory for solana to use txHash as key and txMetaId', () => {
  ...
  it('should update txHistory keys for solana txs and preserve other txs', async () => {
    ...
    const newState = (await migrate(oldState)) as {
      engine: {
        backgroundState: {
    ...
    expect(
      newState.engine.backgroundState.BridgeStatusController,
    ).toStrictEqual({
      txHistory: {
        '4ad572c0-5817-11f0-9a9f-65cbc1ba7703': evmTxHistoryItem,
        '5Cs9btTX3G7a4H4mmbuYLPpEzyyj2t64aSnCCMyy1AKQZvBrLprbmcT7TF2mcTQNKGik9JTLRkqHTtkKjHUUi4af':
    ...
    const {
      ...
      engine: {
        backgroundState: { BridgeStatusController: _, ...otherOldStates },
      },
    } = oldState;
    const {
      // eslint-disable-next-line no-unused-vars
  ...
  it.each([
    ...
    'should not change state if BridgeStatusController state is invalid',
    async (bridgeStatusControllerState) => {
      mockedEnsureValidState.mockReturnValue(true);
  ...
  it('captures exception but returns state unchanged when txHistory update fails', async () => {
    const state = {
      engine: {
        backgroundState: {
          BridgeStatusController: {
            txHistory: {
              '4731011b-c021-46c8-9dbd-706d21ef6709': null,
            },
          },
        },
      },
