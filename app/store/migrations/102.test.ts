...
describe('Migration 102: Convert deposit order string IDs to objects', () => {
  beforeEach(() => {
    jest.resetAllMocks();
  });
    ...
    expect(mockedCaptureException).toHaveBeenCalledWith(
      expect.objectContaining({
        message: expect.stringContaining(
          "FATAL ERROR: Migration 102: Invalid state error: 'string'",
        ),
      }),
    );
  });
  ...
  it('handles migration errors gracefully and returns original state', () => {
    ...
    expect(mockedCaptureException).toHaveBeenCalledWith(
      expect.objectContaining({
        message: expect.stringContaining(
          'Migration 102: Failed to migrate deposit order data',
        ),
      }),
    );
