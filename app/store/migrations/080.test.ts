...
describe('Migration 80: Update profile sync state properties to backup and sync ones', () => {
  beforeEach(() => {
    jest.resetAllMocks();
  });
  ...
  it.each([
    {
      ...
      expectedError:
        "FATAL ERROR: Migration 80: Invalid UserStorageController state error: 'string'",
    },
    {
      state: {
        engine: {
      ...
      test: 'invalid UserStorageController state',
      expectedError:
        "FATAL ERROR: Migration 80: Invalid UserStorageController state error: 'undefined'",
    },
  ])(
