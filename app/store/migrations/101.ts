...
/**
 * Migration 101: Remove old and unused UserStorageController properties
...
const migration = (state: unknown): unknown => {
  ...
  if (!isObject(userStorageControllerState)) {
    captureException(
      new Error(
        `FATAL ERROR: Migration 101: Invalid UserStorageController state error: '${typeof userStorageControllerState}'`,
      ),
    );
    return state;
  }
