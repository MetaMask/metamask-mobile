...
describe('Migration 105: Reset PhishingController urlScanCache', () => {
  beforeEach(() => {
    jest.resetAllMocks();
  });
  ...
  it('captures exception if PhishingController state is invalid', () => {
    ...
    expect(mockedCaptureException.mock.calls[0][0].message).toContain(
      'Migration 105: Invalid PhishingController state',
    );
  });
  ...
  it('handles error during migration', () => {
    ...
    expect(mockedCaptureException).toHaveBeenCalledWith(expect.any(Error));
    expect(mockedCaptureException.mock.calls[0][0].message).toContain(
      'Migration 105: cleaning PhishingController state failed with error',
    );
  });
});
