...
export default async function migrate(stateAsync: unknown) {
  ...
  if (!isObject(state)) {
    captureException(
      new Error(`Migration 37: Invalid root state: '${typeof state}'`),
    );
    return state;
  }
  ...
  if (!isObject(state.engine)) {
    captureException(
      new Error(
        `Migration 37: Invalid root engine state: '${typeof state.engine}'`,
      ),
    );
    return state;
  }
  ...
  if (!isObject(state.engine.backgroundState)) {
    captureException(
      new Error(
        `Migration 37: Invalid root engine backgroundState: '${typeof state
  ...
  if (!isObject(state.engine.backgroundState.PreferencesController)) {
    captureException(
      new Error(
        `Migration 37: Invalid PreferencesController state: '${typeof state
