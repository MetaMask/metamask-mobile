...
import { RealmPersistentStorage } from './realmInstance';
...
export const verifyRealmData = async () => {
  try {
    // Basic health check for critical controllers
    ...
    for (const controller of criticalControllers) {
      await checkControllerData(controller);
    }
    
  } catch (error) {
    Logger.error(error as Error, 'Error during verification');
  }
};
...
export const checkControllerData = async (controllerName: string) => {
  try {
    ...
    Logger.error(error as Error, `Error checking ${controllerName}`);
    return false;
  }
};
