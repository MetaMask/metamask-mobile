 ...
 * Uses the controller's selector which computes enabled state from opt-in flags
 *
...
/**
 * Selects whether user opted in for regular account
 * Uses the controller's selector to ensure logic is controlled by the controller
 *
 * @returns True if opted in, false if not, undefined if state not available
 */
export const selectAnalyticsOptedInForRegularAccount = createSelector(
  selectAnalyticsControllerState,
  (state) =>
    state && analyticsControllerSelectors.selectOptedInForRegularAccount(state),
);

/**
 * Selects whether user opted in for social account
 * Uses the controller's selector to ensure logic is controlled by the controller
 *
 * @returns True if opted in, false if not, undefined if state not available
 */
export const selectAnalyticsOptedInForSocialAccount = createSelector(
  selectAnalyticsControllerState,
  (state) =>
    state && analyticsControllerSelectors.selectOptedInForSocialAccount(state),
);
