...
describe('accounts selectors', () => {
  ...
  describe('selectAccountGroupByGroupId', () => {
    it('returns null when wallets object is empty', () => {
      const result = selectAccountGroupByGroupId(
        {} as unknown as TestWallets,
        ACCOUNT_GROUP_ID_1,
      );
      expect(result).toBeNull();
    });

    it('returns null when group ID does not exist in any wallet', () => {
      const wallets = {
        [WALLET_ID_1]: {
          id: WALLET_ID_1,
      ...
      const result = selectAccountGroupByGroupId(
        wallets as unknown as TestWallets,
        ACCOUNT_GROUP_ID_1,
      );
      expect(result).toBeNull();
    });

    it('returns the correct group when it exists in a wallet', () => {
      const expectedGroup = {
      ...
      const wallets = {
        [WALLET_ID_1]: {
          ...
          groups: {
            [ACCOUNT_GROUP_ID_1]: expectedGroup,
          },
        },
      };
    ...
    it('finds group in the correct wallet when multiple wallets exist', () => {
      const group1 = {
        accounts: [ACCOUNT_ID_1],
      };
      ...
      const wallets = {
        ...
        [WALLET_ID_2]: {
          ...
          groups: {
            [ACCOUNT_GROUP_ID_2]: group2,
          },
        },
      };

      const result1 = selectAccountGroupByGroupId(
        wallets as unknown as TestWallets,
        ACCOUNT_GROUP_ID_1,
      );
      expect(result1).toEqual(group1);

      const result2 = selectAccountGroupByGroupId(
        wallets as unknown as TestWallets,
        ACCOUNT_GROUP_ID_2,
      );
    ...
    it('returns the first matching group when group ID exists in multiple wallets', () => {
      const group1 = {
        accounts: [ACCOUNT_ID_1],
      };
      ...
      const sameGroupId = ACCOUNT_GROUP_ID_1;
      ...
      // Should return the first one it finds (group1 from wallet1)
      expect(result).toEqual(group1);
    });

    it('handles wallets with multiple groups correctly', () => {
      const targetGroup = {
        accounts: [ACCOUNT_ID_3, ACCOUNT_ID_4],
      };
      ...
      const wallets = {
        [WALLET_ID_1]: {
          ...
          groups: {
            ...
            [ACCOUNT_GROUP_ID_2]: targetGroup,
            [ACCOUNT_GROUP_ID_3]: {
              accounts: [ACCOUNT_ID_2],
            },
          },
    ...
    it('handles entropy wallet groups correctly', () => {
      const entropyGroup = {
        accounts: [ENTROPY_EVM_ACCOUNT_ID, ENTROPY_SOLANA_ACCOUNT_ID],
      };
      ...
      const wallets = {
        [ENTROPY_WALLET_ID]: {
          ...
          type: AccountWalletType.Entropy,
          metadata: {
            name: 'Entropy Test Wallet',
            entropy: {
              id: 'testEntropy',
            },
          },
          groups: {
            [ENTROPY_GROUP_ID]: entropyGroup,
          },
        },
      };
    ...
    it('returns null when searching for non-existent group in complex wallet structure', () => {
      const wallets = {
        [WALLET_ID_1]: {
          id: WALLET_ID_1,
          type: AccountWalletType.Keyring,
        ...
        [ENTROPY_WALLET_ID]: {
          ...
          groups: {
            [ENTROPY_GROUP_ID]: {
              accounts: [ENTROPY_EVM_ACCOUNT_ID],
            },
          },
        },
      };

      const nonExistentGroupId = 'nonexistent:group/id' as AccountGroupId;
      const result = selectAccountGroupByGroupId(
        wallets as unknown as TestWallets,
        nonExistentGroupId,
      );
