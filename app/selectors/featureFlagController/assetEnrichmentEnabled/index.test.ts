import { selectAssetEnrichmentEnabled } from '.';
...
describe('selectAssetEnrichmentEnabled', () => {
  it('returns true when stellarAccounts flag is enabled and version meets minimum', () => {
    ...
    expect(selectAssetEnrichmentEnabled(mockedState)).toBe(true);
  });
  ...
  it('returns false when stellarAccounts flag is disabled', () => {
    ...
    expect(selectAssetEnrichmentEnabled(mockedState)).toBe(false);
  });

  it('returns false when stellarAccounts flag is undefined', () => {
    expect(selectAssetEnrichmentEnabled(mockedUndefinedFlagsState)).toBe(false);
  });

  it('returns false when stellarAccounts flag is empty', () => {
    expect(selectAssetEnrichmentEnabled(mockedEmptyFlagsState)).toBe(false);
  });
