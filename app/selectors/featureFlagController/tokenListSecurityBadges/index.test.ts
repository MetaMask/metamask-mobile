import {
  ...
  TOKEN_LIST_SECURITY_BADGES_FLAG_KEY,
} from '.';
...
describe('selectTokenListSecurityBadgesEnabled', () => {
  ...
  it('exposes the client-config flag key for registry alignment', () => {
    expect(TOKEN_LIST_SECURITY_BADGES_FLAG_KEY).toBe('tokenListSecurityBadges');
  });

  it('returns true when enabled is true and minimum version passes', () => {
    const result = selectTokenListSecurityBadgesEnabled.resultFunc({
      [TOKEN_LIST_SECURITY_BADGES_FLAG_KEY]: {
  ...
  it('returns false when flag is missing', () => {
    expect(selectTokenListSecurityBadgesEnabled.resultFunc({})).toBe(false);
  });
