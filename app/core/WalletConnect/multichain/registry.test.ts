...
describe('multichain registry', () => {
  ...
  it('returns the input unchanged for namespaces without a normalizer', () => {
    expect(normalizeCaipChainId('eip155:1')).toBe('eip155:1');
    expect(normalizeCaipChainId('solana:mainnet')).toBe('solana:mainnet');
  });
});
