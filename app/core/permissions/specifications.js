	...
	[PermissionKeys.eth_accounts]: {
		...
		factory: (permissionOptions, requestData) => {
			...
			return constructPermission({
				id: random(),
				...permissionOptions,
				caveats: [CaveatFactories[CaveatTypes.restrictReturnedAccounts](requestData.approvedAccounts)],
			});
		},
		...
		validator: (permission, _origin, _target) => {
			...
			if (!caveats || caveats.length !== 1 || caveats[0].type !== CaveatTypes.restrictReturnedAccounts) {
				throw new Error(
