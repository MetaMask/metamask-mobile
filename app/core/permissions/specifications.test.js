...
import { getCaveatSpecifications, getPermissionSpecifications, unrestrictedMethods } from './specifications';
...
describe('PermissionController specifications', () => {
	describe('caveat specifications', () => {
		it('getCaveatSpecifications returns the expected specifications object', () => {
			...
			expect(caveatSpecifications[CaveatTypes.restrictReturnedAccounts].type).toStrictEqual(
	...
	describe('permission specifications', () => {
		it('getPermissionSpecifications returns the expected specifications object', () => {
			...
			expect(permissionSpecifications[RestrictedMethods.eth_accounts].targetKey).toStrictEqual(
		...
		describe('eth_accounts', () => {
			describe('factory', () => {
				it('constructs a valid eth_accounts permission', () => {
					...
					expect(
						factory({ invoker: 'foo.bar', target: 'eth_accounts' }, { approvedAccounts: ['0x1'] })
					).toStrictEqual({
						caveats: [
							{
								type: CaveatTypes.restrictReturnedAccounts,
			...
			describe('methodImplementation', () => {
				it('returns the keyring accounts in lastSelected order', async () => {
					...
					const getAllAccounts = jest.fn().mockImplementationOnce(() => ['0x1', '0x2', '0x3', '0x4']);
					...
					expect(await methodImplementation()).toStrictEqual(['0x3', '0x4', '0x1', '0x2']);
				});
				...
				it('throws if a keyring account is missing an address (case 1)', async () => {
					...
					const getAllAccounts = jest.fn().mockImplementationOnce(() => ['0x1', '0x2', '0x3']);
					...
					await expect(() => methodImplementation()).rejects.toThrow('Missing identity for address: "0x1".');
				});
				...
				it('throws if a keyring account is missing an address (case 2)', async () => {
					...
					await expect(() => methodImplementation()).rejects.toThrow('Missing identity for address: "0x2".');
				});
			});
			...
			describe('validator', () => {
				...
				it('rejects invalid caveats', () => {
					...
					[null, [], [1, 2], [{ type: 'foobar' }]].forEach((invalidCaveatsValue) => {
						expect(() =>
							validator({
