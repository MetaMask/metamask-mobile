...
async function handleUniversalLink({
  ...
  /**
   * Fire-and-forget Branch.io params fetch for analytics. Resolves to the
   * params if Branch returns valid data within 500 ms, or to `undefined`
   * otherwise. This is passed into the analytics context as a promise so
   * the interstitial / handler flow is never blocked on Branch — only the
   * (already fire-and-forget) `trackDeepLinkAnalytics` awaits the result.
   */
  const branchParamsPromise: Promise<BranchParams | undefined> = Promise.race([
    // Wrap in Promise.resolve to tolerate mocks / implementations that may
    // synchronously return a non-Promise value.
