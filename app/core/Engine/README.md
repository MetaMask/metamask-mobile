...
- Create a messenger callback function for creating the controller's messenger
  ...
  - It should exist in the `Engine/messengers` folder and follow the pattern of existing controllers such as `Engine/messengers/phishing-controller-messenger`
...
- Create an initialization function for the controller
  ...
  - It should exist in the `Engine/controllers` folder and follow the pattern of existing controllers such as `Engine/controllers/phishing-controller-init`
  - The initialization function should return an object with a `controller` property that is the controller instance
  - Import the initialization function in `Engine.ts` and add it to the `initFunctions` object passed to `initMessengerClients`
