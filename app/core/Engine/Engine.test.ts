...
describe('Engine', () => {
  ...
  describe('AccountActivityService:balanceUpdated — Stellar trustline extra refresh', () => {
    const STELLAR_ACCOUNT_ID = 'stellar-account-uuid';
    ...
    it('calls refreshAccountAssetInfo for Stellar classic assets when assetsUnifyState is enabled', async () => {
      (store.getState as jest.Mock).mockReturnValue(makeStoreMock(true));
      ...
      Object.defineProperty(engine.context.AssetsController, 'state', {
        get: () => ({
          assetsBalance: {
            [STELLAR_ACCOUNT_ID]: {
              [STELLAR_CLASSIC_ASSET_ID]: { amount: '0' },
              [STELLAR_NATIVE_ASSET_ID]: { amount: '10' },
            },
          },
        }),
        configurable: true,
      ...
      expect(refreshSpy).toHaveBeenCalledWith(STELLAR_ACCOUNT_ID, [
        STELLAR_CLASSIC_ASSET_ID,
      ]);
    });

    it('does not call refreshAccountAssetInfo when assetsUnifyState is disabled', async () => {
      (store.getState as jest.Mock).mockReturnValue(makeStoreMock(false));
    ...
    it('does not call refreshAccountAssetInfo for non-Stellar chain pushes', async () => {
      (store.getState as jest.Mock).mockReturnValue(makeStoreMock(true));
    ...
    it('does not throw when refreshAccountAssetInfo rejects', async () => {
      (store.getState as jest.Mock).mockReturnValue(makeStoreMock(true));
