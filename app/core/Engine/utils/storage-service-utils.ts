 ...
 *
 * We use URI-style encoding for these characters.
 * Note: We only encode the key part, not the namespace (for backward compatibility).
 */

/**
 * Encode the key part to avoid issues with redux-persist-filesystem-storage.
 *
 * The library has two problematic behaviors:
 * 1. `/` in keys creates subdirectories, making keys unreachable via getAllKeys
 * 2. `-` gets converted to `:` by fromFileName, corrupting the key
 *
 * We use URI-style encoding for these characters.
 ...
 * @returns The encoded key safe for filesystem storage.
 */
 ...
 */
export const decodeStorageKey = (encodedKey: string): string => encodedKey
    .replace(/%2D/gi, '-')
    .replace(/%2F/gi, '/')
    .replace(/%25/g, '%');
