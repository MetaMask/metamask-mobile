...
export interface TronAccountProviderConfig {
  discovery: {
    maxAttempts: number;
    timeoutMs: number;
    backOffMs: number;
...
export const TRON_ACCOUNT_PROVIDER_NAME = 'Tron' as const;

export class TronAccountProvider extends SnapAccountProvider {
  static NAME = TRON_ACCOUNT_PROVIDER_NAME;

  static TRON_SNAP_ID = 'npm:@metamask/tron-wallet-snap' as SnapId;

  readonly #client: KeyringClient;

  readonly #config: TronAccountProviderConfig;

  constructor(
    messenger: MultichainAccountServiceMessenger,
    config: TronAccountProviderConfig = {
      discovery: {
        timeoutMs: 2000,
        maxAttempts: 3,
        backOffMs: 1000,
    ...
    super(TronAccountProvider.TRON_SNAP_ID, messenger);
    this.#client = this.#getKeyringClientFromSnapId(
      TronAccountProvider.TRON_SNAP_ID,
    );
    this.#config = config;
  }

  getName(): string {
    return TronAccountProvider.NAME;
  }
  ...
  isAccountCompatible(account: Bip44Account<InternalAccount>): boolean {
    return (
      ...
      Object.values<string>(TrxAccountType).includes(account.type)
    );
  }
  ...
  async createAccounts({
    ...
    const createTronAccount = async (addressType: TrxAccountType) =>
      await createAccount({
        entropySource,
        index,
        addressType,
    ...
    const tronAccount = await createTronAccount(TrxAccountType.Eoa);

    assertIsBip44Account(tronAccount);
