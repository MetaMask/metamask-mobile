...
export const createSubscribeToPushNotifications = () => async () =>
  FCMService.listenToPushNotificationsReceived(
    ...
    async (rawPayload) => {
      ...
      if (!title) {
        logPushEvent('FCM_PLATFORM_HANDLER_NO_TITLE', 'No title in notification payload — skipped');
        return;
      }
      await NotificationsService.displayNotification({
        pressActionId: PressActionId.OPEN_NOTIFICATIONS_VIEW,
      ...
      logPushEvent('FCM_PLATFORM_HANDLER_DISPLAYED', `Platform notification displayed: "${title}"`);
    },
  );
