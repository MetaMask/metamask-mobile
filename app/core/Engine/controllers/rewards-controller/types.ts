...
export interface EstimatePerpsContextDto {
   ...
   */
  perpsType?: 'OPEN_POSITION' | 'CLOSE_POSITION' | 'STOP_LOSS' | 'TAKE_PROFIT';
  ...
  /**
   * Is buy order (true = BUY order, false = SELL order)
   * @example true
   */
  isBuy: boolean;
  ...
  /**
   * Slippage tolerance for market orders (e.g., 0.01 = 1%)
   * @example 0.01
   */
  slippage?: number;

  /**
   * Override grouping (defaults: "na" without TP/SL, "normalTpsl" with TP/SL)
   * @example 'normalTpsl'
   */
  grouping?: string;

  /**
   * Current market price (avoids extra API call if provided)
   * @example 10000
   */
  currentPrice?: number;
}
