...
const mockEnableAgenticCliNotificationsIfDisabled = jest
  .fn()
...
jest.mock(
  ...
  () => ({
    enableAgenticCliNotificationsIfDisabled: (...args: unknown[]) =>
      mockEnableAgenticCliNotificationsIfDisabled(...args),
  }),
);
...
describe('AgenticCliQrLoginService', () => {
  ...
  it('exchanges the Hydra token, opens prod dashboard, sends CLI token, and cleans up', async () => {
    ...
    expect(mockEnableAgenticCliNotificationsIfDisabled).toHaveBeenCalledTimes(
      1,
    );
