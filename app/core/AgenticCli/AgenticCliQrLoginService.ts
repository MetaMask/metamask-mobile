...
import { enableAgenticCliNotificationsIfDisabled } from '../../util/notifications/ensureAgenticCliNotificationPreferencesMigrated';
...
export async function handleAgenticCliQrLogin({
  ...
  try {
    ...
    await enableAgenticCliNotificationsIfDisabled().catch((error) => {
      logger.error('Failed to auto-enable agentic CLI notifications:', error);
    });
