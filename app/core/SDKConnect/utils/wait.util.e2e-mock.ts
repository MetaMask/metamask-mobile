...
export const waitForCondition = async ({
  ...
  while (!fn()) {
    i += 1;
    if (i > 5 && i % 10 === 0) {
      DevLogger.log(`[wait.util - waitForCondition] Waiting for fn context=${context} to return true`);
    }
    await wait(waitTime);
  }
};
...
export const waitForAsyncCondition = async ({
  ...
  while (!(await fn())) {
    i += 1;
    if (i > 5 && i % 10 === 0) {
      DevLogger.log(`[wait.util - waitForAsyncCondition] Waiting for fn context=${context} to return true`);
    }
    await wait(waitTime);
  }
};
