 ...
 * sign-in branch would fire a `changeUser` (session start + identify) for a
 * wallet that never reaches the user — a burst of Braze requests that is
 * pure noise. Sign-out (`clearBrazeUser`) is intentionally left ungated so
 * the previous user's session-end event is preserved.
 *
