...
export class Currency {
	...
	format(_options) {
		...
		const amount = options.commas ? commify(this.round(options.decimals)) : this.round(options.decimals);
	...
	_round = decStr => this._floor(this.fromWad(this.toWad(decStr).add(this.toWad('0.5'))).toString());
