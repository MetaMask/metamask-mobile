import { SessionRequest, WalletClient } from '@metamask/mobile-wallet-protocol-wallet-client';
import { IKeyManager, SessionStore, WebSocketTransport } from '@metamask/mobile-wallet-protocol-core';
...
export class Connection {
	...
	private constructor(id: string, dappMetadata: DappMetadata, client: WalletClient, bridge?: BackgroundBridge) {
		this.id = id;
	...
	public static async create(connreq: ConnectionRequest, keymanager: IKeyManager, relayURL: string): Promise<Connection> {
		const id = connreq.sessionRequest.id;
		const dappMetadata = connreq.dappMetadata;
		const transport = await WebSocketTransport.create({ url: relayURL, kvstore: new KVStore(`mwp/transport/${id}`) });
		const sessionstore = new SessionStore(new KVStore(`mwp/session-store/${id}`));
