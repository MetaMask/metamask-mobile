...
class NavigationService {
  ...
  static set navigation(navRef: NavigationContainerRef) {
    ...
    // Lazy require keeps AgenticService (and its Engine/Redux deps) out of the
    // module graph when __DEV__ is false (e.g. production builds and tests).
    if (__DEV__) {
      /* eslint-disable @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires */
      const AgenticService =
        require('../AgenticService/AgenticService').default;
