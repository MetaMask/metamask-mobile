 ...
 * env vars (see `.js.env.example`). This mirrors `MM_BACKEND_WEBSOCKET_URL`
 * for the Backend WebSocket service: each REST service URL can be pointed at
 * a dev/local backend without code changes.
 */
...
export function isBackendAuthDisabled(): boolean {
  return process.env.MM_BACKEND_DISABLE_AUTH === 'true';
}
