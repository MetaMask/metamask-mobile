...
export const HardwareWalletProvider: React.FC<HardwareWalletProviderProps> = ({
  ...
  // Mirrors the logic from useBluetooth.ts to detect when Bluetooth is on/off
  // ========================================================================
  const [isBluetoothOn, setIsBluetoothOn] = useState(false);
  const [bluetoothConnectionError, setBluetoothConnectionError] =
    useState(false);

  // Monitor Bluetooth adapter state (exactly like useBluetooth.ts)
  useEffect(() => {
    let subscription: Subscription | undefined;

    // Start monitoring Bluetooth state
    // This uses the same approach as useBluetooth.ts
    subscription = TransportBLE.observeState({
      next: (e: { available: boolean; type: BleState }) => {
        console.log(
          '[HardwareWalletProvider] Bluetooth state changed:',
          e.type,
          'available:',
          e.available,
        );
        if (e.available && e.type === BleState.PoweredOn) {
          setIsBluetoothOn(true);
          setBluetoothConnectionError(false);
        }
        if (!e.available && e.type === BleState.PoweredOff) {
      ...
      error: (error: Error) => {
        console.error(
          '[HardwareWalletProvider] Bluetooth state observation error:',
          error,
        );
        setBluetoothConnectionError(true);
      },
  ...
  // ========================================================================
  // REACTIVE BLUETOOTH STATE MONITORING
  // If Bluetooth gets disabled during ANY active operation, show error immediately
  // ========================================================================
  useEffect(() => {
    // Only react if we're in an active state that requires Bluetooth
    const activeStates = [
    ...
    const requiresBT = effectiveWalletType
      ? requiresBluetooth(effectiveWalletType)
      : false;

    // If Bluetooth just went off while we're in an active BT-requiring state, show error
    if (!isBluetoothOn && isInActiveState && requiresBT) {
      console.log(
        '[HardwareWalletProvider] Bluetooth disabled during active operation - showing error',
      );
  ...
  useEffect(() => {
    ...
    refs.adapterRef.current = createAdapter(effectiveWalletType, {
      onDisconnect: (error) => {
        if (error) {
          handleError(error);
        } else {
  ...
  // Update permissions state with internal Bluetooth state
  useEffect(() => {
    setters.setPermissionState((prev) => ({
      ...prev,
      isBluetoothEnabled: isBluetoothOn,
    }));
  }, [isBluetoothOn, setters]);
  ...
  // ========================================================================
  // BLE SCANNING EFFECT
  // Mirrors the logic from useBluetoothDevices.ts
  // ========================================================================
  useEffect(() => {
    let subscription: Subscription | undefined;
    let timeoutId: ReturnType<typeof setTimeout> | undefined;

    // Only attempt scanning when in Scanning state with Ledger wallet type
    if (
      connectionState.status === ConnectionStatus.Scanning &&
      effectiveWalletType === HardwareWalletType.Ledger
    ) {
      console.log(
        '[HardwareWalletProvider] Scanning state entered, isBluetoothOn:',
        isBluetoothOn,
      );

      // Check if Bluetooth is enabled BEFORE attempting to scan
      // This is a fallback check - openDeviceSelection should catch this first
      if (!isBluetoothOn) {
        console.log(
          '[HardwareWalletProvider] Bluetooth is OFF - transitioning to error state',
        );
        const btError = createHardwareWalletError(
          ErrorCode.BluetoothDisabled,
      ...
      console.log(
        '[HardwareWalletProvider] Starting TransportBLE.listen subscription',
      );

      // Start BLE scanning (same approach as useBluetoothDevices.ts)
      subscription = new Observable(TransportBLE.listen).subscribe({
        next: (event: {
          ...
          console.log(
            '[HardwareWalletProvider] BLE event:',
            event.type,
          ...
          if (event.type === 'add') {
            ...
            setDeviceSelectionState((prev) => {
              // Avoid duplicates (same logic as useBluetoothDevices.ts)
              const exists = prev.devices.some((d) => d.id === device.id);
              if (exists) return prev;
              console.log(
                '[HardwareWalletProvider] Adding device:',
                device.name,
                device.id,
              );
              return {
        ...
        error: (error: Error) => {
          console.error('[HardwareWalletProvider] BLE scan error:', error);
          // Parse the error to detect specific cases (e.g., permission denied)
          const scanError = parseErrorByType(error, HardwareWalletType.Ledger);
          // Transition to error state so ErrorContent is displayed
          updateConnectionState(ConnectionState.error(scanError));
        },
        complete: () => {
          console.log('[HardwareWalletProvider] BLE scan completed');
          setDeviceSelectionState((prev) => ({
            ...prev,
            isScanning: false,
          }));
      ...
      // Stop scanning after timeout and show timeout error if no devices found
      timeoutId = setTimeout(() => {
        console.log('[HardwareWalletProvider] Scan timeout reached');
        setDeviceSelectionState((prev) => {
          // Only show timeout error if no devices were found
          if (prev.devices.length === 0 && prev.isScanning) {
            const timeoutError = createHardwareWalletError(
              ErrorCode.ConnectionTimeout,
              HardwareWalletType.Ledger,
              'Scan timeout: No Ledger devices found. Make sure your Ledger is unlocked and Bluetooth is enabled on the device.',
            );
            // Transition to error state so ErrorContent is displayed
            updateConnectionState(ConnectionState.error(timeoutError));
            return {
              ...prev,
              isScanning: false,
            };
      ...
      }, 30000); // 30 second timeout (same as before)

      return () => {
        console.log(
          '[HardwareWalletProvider] Cleaning up BLE scan subscription',
        );
        subscription?.unsubscribe();
  ...
  const openDeviceSelection = useCallback(
    (walletTypeToConnect: HardwareWalletType, onSuccess?: () => void) => {
      ...
      // Check if this wallet type requires Bluetooth
      if (requiresBluetooth(walletTypeToConnect)) {
        // Check Bluetooth state FIRST before scanning
        if (!isBluetoothOn) {
          console.log(
          ...
          // Go directly to error state instead of scanning
          updateConnectionState(ConnectionState.error(btError));
          return;
        }
      }

      // Bluetooth is on (or not needed), proceed to scanning
      console.log(
        '[HardwareWalletProvider] Bluetooth is ON - transitioning to scanning state',
      );
      updateConnectionState(ConnectionState.scanning());
    },
    [setters, updateConnectionState, isBluetoothOn],
  );
  ...
  const openSigningModal = useCallback(
    async (
      ...
      // Check Bluetooth requirement
      if (requiresBluetooth(walletTypeToSign) && !isBluetoothOn) {
    ...
    [setters, isBluetoothOn, handleError, updateConnectionState, refs],
  );
  ...
  const connect = useCallback(
    async (targetDeviceId: string): Promise<void> => {
      ...
      // Check Bluetooth requirement (use effective wallet type)
      if (requiresBluetooth(effectiveWalletType) && !isBluetoothOn) {
        const error = createHardwareWalletError(
          ErrorCode.BluetoothDisabled,
          effectiveWalletType ?? HardwareWalletType.Ledger,
          'Bluetooth is not enabled. Please enable Bluetooth in your device settings.',
  ...
  const ensureDeviceReady = useCallback(
    async (targetDeviceId?: string): Promise<boolean> => {
      ...
      // If there's already a pending check, cancel it
      if (pendingReadyResolveRef.current) {
        console.log(
          '[HardwareWalletProvider] Cancelling previous pending readiness check',
        );
        pendingReadyResolveRef.current(false);
        pendingReadyResolveRef.current = null;
      }
