"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var transaction_controller_1 = require("@metamask/transaction-controller");
var rpc_errors_1 = require("@metamask/rpc-errors");
var ppom_util_1 = require("../../lib/ppom/ppom-util");
/**
 * A type guard for {@link RuntimeObject}.
 *
 * TODO: replace this with `isObject` from `@metamask/utils`
 *
 * @param value - The value to check.
 * @returns Whether the specified value has a runtime type of `object` and is
 * neither `null` nor an `Array`.
 */
function isObject(value) {
    return Boolean(value) && typeof value === 'object' && !Array.isArray(value);
}
/**
 * A type guard for ensuring an object has a property.
 *
 * TODO: replace this with `hasProperty` from `@metamask/utils`
 *
 * @param objectToCheck - The object to check.
 * @param name - The property name to check for.
 * @returns Whether the specified object has an own property with the specified
 * name, regardless of whether it is enumerable or not.
 */
var hasProperty = function (objectToCheck, name) {
    return Object.hasOwnProperty.call(objectToCheck, name);
};
/**
 * Handle a `eth_sendTransaction` request.
 *
 * @param args - Named arguments.
 * @param args.hostname - The hostname associated with this request.
 * @param args.req - The JSON-RPC request.
 * @param args.res - The JSON-RPC response.
 * @param args.sendTransaction - A function that requests approval for the given transaction, then
 * signs the transaction and broadcasts it.
 * @param args.validateAccountAndChainId - A function that validates the account and chain ID
 * used in the transaction.
 */
function eth_sendTransaction(_c) {
    var hostname = _c.hostname, req = _c.req, res = _c.res, sendTransaction = _c.sendTransaction, validateAccountAndChainId = _c.validateAccountAndChainId;
    return __awaiter(this, void 0, void 0, function () {
        var transactionParameters, _d, result, transactionMeta, _e;
        return __generator(this, function (_f) {
            switch (_f.label) {
                case 0:
                    if (!Array.isArray(req.params) &&
                        !(isObject(req.params) && hasProperty(req.params, 0))) {
                        throw rpc_errors_1.rpcErrors.invalidParams({
                            message: "Invalid parameters: expected an array"
                        });
                    }
                    transactionParameters = req.params[0];
                    if (!isObject(transactionParameters)) {
                        throw rpc_errors_1.rpcErrors.invalidParams({
                            message: "Invalid parameters: expected the first parameter to be an object"
                        });
                    }
                    return [4 /*yield*/, validateAccountAndChainId({
                            from: req.params[0].from,
                            chainId: req.params[0].chainId
                        })];
                case 1:
                    _f.sent();
                    return [4 /*yield*/, sendTransaction(req.params[0], {
                            deviceConfirmedOn: transaction_controller_1.WalletDevice.MM_MOBILE,
                            origin: hostname
                        })];
                case 2:
                    _d = _f.sent(), result = _d.result, transactionMeta = _d.transactionMeta;
                    ppom_util_1["default"].validateRequest(req, transactionMeta === null || transactionMeta === void 0 ? void 0 : transactionMeta.id);
                    _e = res;
                    return [4 /*yield*/, result];
                case 3:
                    _e.result = _f.sent();
                    return [2 /*return*/];
            }
        });
    });
}
exports["default"] = eth_sendTransaction;
