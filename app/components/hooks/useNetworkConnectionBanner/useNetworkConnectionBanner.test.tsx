...
describe('useNetworkConnectionBanner', () => {
  ...
  describe('useEffect', () => {
    ...
    it('classifies the failure using the user-selected RPC endpoint, not the chain default (custom override fires)', () => {
      // Chain 0x1 has two endpoints: Infura (default) and a custom one the
      // user has selected. Only the custom endpoint is failing while chain
      // 0x89 stays available — so the all-enabled-failed escape hatch does
      // NOT apply. The hook must derive isInfuraEndpoint from the SELECTED
      // endpoint; otherwise the failure is misclassified as Infura and the
      // single-domain rule suppresses the banner.
      const CUSTOM_CLIENT_ID = 'mainnet-custom';
      const INFURA_CLIENT_ID = 'mainnet-infura';
      ...
      const findClientId = jest.fn((chainId: Hex) =>
        chainId === '0x1' ? CUSTOM_CLIENT_ID : NETWORK_CLIENT_ID_89,
      );
      const getConfig = jest.fn((clientId: string) => {
        if (clientId === CUSTOM_CLIENT_ID) {
          return {
            chainId: '0x1',
            name: 'Ethereum Mainnet',
            defaultRpcEndpointIndex: 0,
            ...
            rpcEndpoints: [
              {
                url: 'https://mainnet.infura.io/v3/test-infura-project-id',
                networkClientId: INFURA_CLIENT_ID,
                type: RpcEndpointType.Infura,
              },
              {
                url: 'https://eth-mainnet.alchemyapi.io/v2/key',
                networkClientId: CUSTOM_CLIENT_ID,
                type: RpcEndpointType.Custom,
              },
            ],
          };
        }
        return mockNetworkConfigurationByChainId['0x89'];
      });
      ...
      Engine.context = {
        NetworkController: {
          ...mockNetworkController,
          findNetworkClientIdByChainId: findClientId,
          getNetworkConfigurationByNetworkClientId: getConfig,
          state: {
            networksMetadata: {
              [CUSTOM_CLIENT_ID]: { status: NetworkStatus.Unavailable },
              [NETWORK_CLIENT_ID_89]: { status: NetworkStatus.Available },
            },
          },
        },
      ...
      expect(actions[0]).toStrictEqual({
        ...
        isInfuraEndpoint: false,
        infuraNetworkClientId: INFURA_CLIENT_ID,
      });
    });
