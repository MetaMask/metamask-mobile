...
import { CaveatTypes, RestrictedMethods } from '../../../core/permissions/constants';
 ...
 */
class AccountListPermissions extends PureComponent {
	static propTypes = {
		/**
		...
		permittedAccountsMap: PropTypes.array,
	};
	...
	onAccountRevoke = async (newIndex) => {
		try {
			const { keyrings, hostname } = this.props;

			const allKeyrings =
				keyrings && keyrings.length ? keyrings : Engine.context.KeyringController.state.keyrings;
			const accountsOrdered = allKeyrings.reduce((list, keyring) => list.concat(keyring.accounts), []);

			this.removePermittedAccount(hostname, accountsOrdered[newIndex]);
	...
	onAccountConnect = async (newIndex) => {
		const previousIndex = this.state.selectedAccountIndex;
		const { PreferencesController } = Engine.context;
		const { keyrings, accounts, hostname, isRequest } = this.props;
		...
		requestAnimationFrame(async () => {
			try {
				...
				if (isRequest) {
					return this.props.onAccountChange(accountsOrdered[newIndex]);
				}
				this.addPermittedAccount(hostname, accountsOrdered[newIndex]);
	...
	renderItem = ({ item, index }) => {
		...
		return (
			<AccountElement
				onConnect={this.onAccountConnect}
				onRevoke={this.onAccountRevoke}
	...
	getAccounts() {
		const { accounts, identities, selectedAddress, keyrings, getBalanceError, hostname, permittedAccountsMap } =
			this.props;
		...
		let accountsOrdered = allKeyrings.reduce((list, keyring) => list.concat(keyring.accounts), []);
		accountsOrdered = accountsOrdered.filter((address) => !!identities[toChecksumAddress(address)]);

		return orderBy(
			accountsOrdered.map((addr, index) => {
				...
				const isConnected = Boolean(
					permittedAccountsMap[hostname]?.find(
						(item) => toChecksumAddress(item) === identityAddressChecksummed
					)
				);
...
const mapStateToProps = (state) => ({
	...
	permittedAccountsMap: getPermittedAccountsByOrigin(state.engine.backgroundState.PermissionController),
