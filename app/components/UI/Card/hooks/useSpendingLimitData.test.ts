...
describe('useSpendingLimitData', () => {
  ...
  describe('Network Filtering', () => {
    ...
    it('includes linea network for international users', () => {
        ...
        data: createMockDelegationSettings({
          networks: [
            ...
            },
            {
              ...
              environment: 'production',
              chainId: '59144',
              delegationContract: '0xlineaUsDelegation',
              tokens: {
                usdc: { symbol: 'usdc', decimals: 6, address: '0xlineaUsUsdc' },
              },
            },
          ],
        }),
      ...
      expect(result.current.availableTokens[0].address).toBe('0xlineaUsdc');
    });

    it('includes linea-us network for US users', () => {
      mockUserCardLocation = 'us';
      ...
      expect(result.current.availableTokens).toHaveLength(1);
      expect(result.current.availableTokens[0].address).toBe('0xlineaUsUsdc');
    });
  });
