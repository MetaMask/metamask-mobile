...
const Transactions = ({
  ...
  const [cancelConfirmDisabled, setCancelConfirmDisabled] =
  ...
  const [speedUpConfirmDisabled, setSpeedUpConfirmDisabled] =
    useState<boolean>(false);
  const [rpcBlockExplorer, setRpcBlockExplorer] = useState<string | undefined>(
    undefined,
  );
  ...
  const [isQRHardwareAddress, setIsQRHardwareAddress] =
    useState<boolean>(false);
  ...
  const onSpeedUpAction = (speedUpAction: boolean, gas: any, tx: any) => {
    ...
    if (gas.isEIP1559Transaction) {
      ...
      const isValidBalance = validateTransactionActionBalance(
        tx,
  ...
  const renderList = () => {
    ...
    const renderSpeedUpGas = () => {
      ...
      if (!existingGas.isEIP1559Transaction)
        return `${renderFromWei(
          Math.floor(existingGas.gasPrice * SPEED_UP_RATE),
        )} ${strings('unit.eth')}`;
    };
    ...
    const renderCancelGas = () => {
      ...
      if (!existingGas.isEIP1559Transaction)
        return `${renderFromWei(
          Math.floor(existingGas.gasPrice * CANCEL_RATE),
        )} ${strings('unit.eth')}`;
    };
  ...
  return (
    <SafeAreaView style={styles.wrapper} testID={'txn-screen'}>
      ...
      {(speedUp1559IsOpen || cancel1559IsOpen) &&
        renderUpdateTxEIP1559Gas(cancel1559IsOpen)}
    </SafeAreaView>
  );
};
