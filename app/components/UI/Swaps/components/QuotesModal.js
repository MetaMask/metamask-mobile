						...
						<View onStartShouldSetResponder={() => true}>
							{!!selectedDetailsQuote && !!selectedDetailsQuoteValues && (
								<>
									...
									<View style={styles.detailsRow}>
										<Text small>{strings('swaps.quote_details_max_slippage')}</Text>
									...
									<View style={styles.detailsRow}>
										...
										<Text primary>
											...
											{selectedDetailsQuote?.priceSlippage?.calculationError?.length === 0 &&
												selectedDetailsQuote?.priceSlippage?.destinationAmountInETH && (
													<Text>
														{' '}
														(~
														{weiToFiat(
