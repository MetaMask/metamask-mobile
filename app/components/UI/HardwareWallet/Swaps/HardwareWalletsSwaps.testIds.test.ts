import { HardwareWalletsSwapsSelectorsIDs } from './HardwareWalletsSwaps.testIds';

describe('HardwareWalletsSwapsSelectorsIDs', () => {
  it('exports stable test IDs for the hardware wallet swaps flow', () => {
    expect(HardwareWalletsSwapsSelectorsIDs).toEqual({
      CONTAINER: 'hardware-wallets-swaps',
      RIVE_ANIMATION: 'hardware-wallets-swaps-rive-animation',
      TITLE: 'hardware-wallets-swaps-title',
      CANCEL_BUTTON: 'hardware-wallets-swaps-cancel-button',
      TRY_AGAIN_BUTTON: 'hardware-wallets-swaps-try-again-button',
      RECONNECT_BUTTON: 'hardware-wallets-swaps-reconnect-button',
      DONE_BUTTON: 'hardware-wallets-swaps-done-button',
      STEP_ROW: 'hardware-wallets-swaps-step-row',
      STEP_CONNECTOR: 'hardware-wallets-swaps-step-connector',
      SIGNING_SPINNER: 'hardware-wallets-swaps-signing-spinner',
      SCAN_NEXT_QR_BUTTON: 'hardware-wallets-swaps-scan-next-qr-button',
      INLINE_QR_CODE: 'hardware-wallets-swaps-inline-qr-code',
    });
  });
});
