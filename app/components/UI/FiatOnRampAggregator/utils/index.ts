...
export const timeToDescription = (timeArr: number[]) => {
	...
	if (lower === 0) {
		if (isOverADay(upper)) {
			if (toDay(upper) === 1) {
				return [TimeDescriptions.less_than, toDay(upper).toString(), TimeDescriptions.business_day];
			}
			return [TimeDescriptions.less_than, toDay(upper).toString(), TimeDescriptions.business_days];
		} else if (isOverAnHour(upper)) {
			if (toHour(upper) === 1) {
				return [TimeDescriptions.less_than, toHour(upper).toString(), TimeDescriptions.hour];
			}
			return [TimeDescriptions.less_than, toHour(upper).toString(), TimeDescriptions.hours];
		}
		if (upper === 1) {
			return [TimeDescriptions.less_than, upper.toString(), TimeDescriptions.minute];
		}
		return [TimeDescriptions.less_than, upper.toString(), TimeDescriptions.minutes];
		...
		return [toHour(lower).toString(), TimeDescriptions.separator, toHour(upper).toString(), TimeDescriptions.hours];
	}
...
export function getPaymentMethodIcon(id: string) {
	switch (id) {
