...
import { type UseQuickBuyControllerResult } from './useQuickBuyController';
import {
...
const mockUseIsInsufficientBalance =
  ...
  >;
const mockUseHasSufficientGas = useHasSufficientGas as jest.MockedFunction<
  typeof useHasSufficientGas
>;
const mockUseQuickBuySetup = useQuickBuySetup as jest.MockedFunction<
  typeof useQuickBuySetup
>;
const mockUsePayWithTokens = usePayWithTokens as jest.MockedFunction<
  typeof usePayWithTokens
>;
const mockUseReceiveTokens = useReceiveTokens as jest.MockedFunction<
  typeof useReceiveTokens
>;
const mockUsePositionTokenBalance =
  usePositionTokenBalance as jest.MockedFunction<
    typeof usePositionTokenBalance
  >;
const mockUseDestTokenExchangeRate =
...
const { mockShowToast, toastRef: mockToastRef } = jest.requireMock(
...
const setupDefaultMocks = () => {
  ...
  } as ReturnType<typeof useQuickBuyAnalytics>);
  ...
  mockUsePositionTokenBalance.mockReturnValue(undefined);
  mockUseDestTokenExchangeRate.mockReturnValue(undefined);
  ...
  mockUseHasSufficientGas.mockReturnValue(true);
  (
...
export const runQuickBuyControllerCases = ({
  ...
  renderHook: (
    ...
    rerender: (props?: { target: QuickBuyTarget; onClose: () => void }) => void;
    unmount: () => void;
  };
  name: string;
  setupQuoteSourceMock: (
  ...
  mockQuoteSource: jest.MockedFunction<typeof useQuickBuyQuotes>;
}) => {
  describe(name, () => {
    beforeEach(() => {
      jest.clearAllMocks();
    ...
    describe('handleAmountChange', () => {
      it('ignores input with more than one decimal point', () => {
        const { result } = renderHook(createTarget(), jest.fn());

        act(() => {
          result.current.handleAmountChange('1.2.3');
        });

        expect(result.current.fiatAmount).toBe('');
      });

      it('ignores fiat input past two decimal places', () => {
        const { result } = renderHook(createTarget(), jest.fn());

        act(() => {
          result.current.handleAmountChange('20.123');
        });
    ...
    describe('handleQuickAmountPress', () => {
      ...
      it('enables Add Funds when a pill exceeds the available balance', () => {
        ...
        const sourceWithRate = createSourceToken({
          currencyExchangeRate: 2000,
        });
        (usePayWithTokens as jest.Mock).mockReturnValue({
          options: [sourceWithRate],
      ...
      it('routes to Ramp buy when Add Funds is confirmed', async () => {
        ...
        const { result } = renderHook(createTarget(), onClose);
      ...
      it('exposes usdToCurrentCurrencyRate from native currency rates', () => {
        ...
        expect(result.current.usdToCurrentCurrencyRate).toBeCloseTo(
          1000 / 1200,
        );
      });
    });
    ...
    describe('max ("sell all") source amount', () => {
      ...
      const renderWithStablecoin = () => {
        ...
        return renderHook(createTarget(), jest.fn());
      };
    ...
    describe('handleSelectSourceToken', () => {
      ...
      it('does not auto-select a pay-with token after a manual selection is cleared', () => {
        const eth = createSourceToken({ symbol: 'ETH' });
        const usdc = createSourceToken({
          symbol: 'USDC',
          address: '0xusdc',
        });
        ...
        act(() => {
          result.current.handleSelectSourceToken(usdc);
        });
        act(() => {
          result.current.setSelectedSourceToken(undefined);
        });

        expect(result.current.selectedSourceToken).toBeUndefined();
      });

      it('tracks pay_with_selected with an empty previous token when none is selected', () => {
        const eth = createSourceToken({ symbol: 'ETH' });
        const usdc = createSourceToken({
          symbol: 'USDC',
          address: '0xusdc',
        ...
        mockTrackPayWithSelected.mockClear();
        act(() => {
          result.current.handleSelectSourceToken(usdc);
        });

        expect(mockTrackPayWithSelected).toHaveBeenCalledWith('USDC', '');
      });
    });
    ...
    describe('quoteOverride wiring', () => {
      it('passes null to useIsInsufficientBalance when there is no active quote', () => {
        renderHook(createTarget(), jest.fn());
    ...
    describe('isConfirmDisabled', () => {
      ...
      it('is enabled after quote loading settles for the entered amount', () => {
        ...
        setupQuoteSourceMock(quoteState);
      ...
      it('does not block hardware Solana when no selected address is set', () => {
        (
          selectSelectedInternalAccountFormattedAddress as unknown as jest.Mock
        ...
        expect(result.current.isHardwareSolanaBlocked).toBe(false);
    ...
    describe('live available balances (TSA-632)', () => {
      ...
      describe('source (Pay with) balance', () => {
        it('updates the displayed source balance when the underlying balance changes (external transfer)', () => {
          ...
          const { result, rerender } = renderHook(createTarget(), jest.fn());
          // 1.0 ETH * $2000 rate.
        ...
        it('re-keys the on-chain fetch off the live balance so any change re-fetches', () => {
          ...
          const { rerender } = renderHook(createTarget(), jest.fn());
      ...
      describe('dest (Receive) balance', () => {
        const renderSellMode = () => {
          const utils = renderHook(createTarget(), jest.fn());
          act(() => {
    ...
    describe('selectedQuoteRequestId', () => {
      ...
      it('clears manual selection when requestId is not in the current quote batch', () => {
        const quoteState: UseQuickBuyQuotesResult = {
          activeQuote: quoteWithRequestId('quote-a'),
          sortedQuotes: [quoteWithRequestId('quote-a')],
          destTokenAmount: '1',
          isQuoteLoading: false,
          isNoQuotesAvailable: false,
          quoteFetchError: null,
        ...
        quoteState.sortedQuotes = [quoteWithRequestId('quote-b')];
        rerender(props);
      ...
      it('never asks the quotes hook to pause auto-refresh', () => {
        ...
        const lastCall = mockQuoteSource.mock.calls.at(-1)?.[0];
    ...
    describe('quick buy interacted analytics', () => {
      ...
      it('tracks receive_token_selected with an empty previous token when none is selected', () => {
        const usdc = createSourceToken({
          symbol: 'USDC',
          address: '0xusdc',
          currencyExchangeRate: 1,
    ...
    describe('handleConfirm', () => {
      ...
      describe('trade-level analytics', () => {
        ...
        it('omits amount_usd from trade submitted when fiat amount is zero', async () => {
          mockSuccessfulSubmit();
          ...
          expect(mockTrackTradeSubmitted).toHaveBeenCalledWith(
            expect.not.objectContaining({ amount_usd: expect.anything() }),
          );
        });

        it('includes amount_usd on trade submitted when fiat amount is positive', async () => {
          mockSuccessfulSubmit();
          ...
          expect(mockTrackTradeSubmitted).toHaveBeenCalledWith(
            expect.objectContaining({ amount_usd: 20 }),
          );
        });
        ...
        it('includes amount_token on trade completed when dest amount is numeric', async () => {
          ...
          expect(mockTrackTradeCompleted).toHaveBeenCalledWith(
            expect.objectContaining({ amount_token: 1.5 }),
          );
        });
        ...
        it('omits amount_token from trade completed when dest amount is not numeric', async () => {
          ...
          expect(mockTrackTradeCompleted).toHaveBeenCalledWith(
            expect.objectContaining({ amount_token: undefined }),
          );
        });

        it('omits amount_usd from trade submitted when currency rates cannot convert fiat', async () => {
          (selectCurrencyRates as unknown as jest.Mock).mockReturnValue({});
          (selectNetworkConfigurations as unknown as jest.Mock).mockReturnValue(
            {},
          );
        ...
        it('omits pay_with_token and includes receive_token on trade submitted in sell mode', async () => {
          mockUsePositionTokenBalance.mockReturnValue(
            createSourceToken({
              address: '0xDEST',
              symbol: 'TARGET',
            }),
          ...
          expect(mockTrackTradeSubmitted).toHaveBeenCalledWith(
            expect.objectContaining({ receive_token: 'USDC' }),
          );
          expect(mockTrackTradeSubmitted).toHaveBeenCalledWith(
            expect.not.objectContaining({ pay_with_token: expect.anything() }),
          );
        });
      });
      ...
      describe('stay-on-screen swap toasts', () => {
        ...
        it('uses an empty counter token symbol when the source has no symbol', async () => {
          mockUsePayWithTokens.mockReturnValue({
            options: [createSourceToken({ symbol: undefined })],
            isLoading: false,
          });
          mockUsableQuote();
          (
        ...
        it('skips terminal toast reconcile when the toast ref is missing', async () => {
    ...
    describe('add funds disabled state', () => {
      it('keeps pay-with options unfiltered when dest chain id is missing', () => {
        mockUseQuickBuySetup.mockReturnValue({
          chainId: undefined,
          destToken: undefined,
          isLoading: false,
      ...
      it('disables confirm when the dest native asset cannot be resolved', () => {
        mockUsePayWithTokens.mockReturnValue({
          options: [],
          isLoading: false,
        });
    ...
    describe('currency rates', () => {
      it('uses an empty currency-rate map when the selector returns undefined', () => {
        (selectCurrencyRates as unknown as jest.Mock).mockReturnValue(
          undefined,
        );
        ...
        expect(result.current.usdToCurrentCurrencyRate).toBeUndefined();
      });

      it('returns undefined usdToCurrentCurrencyRate when conversion rates are missing', () => {
        (selectCurrencyRates as unknown as jest.Mock).mockReturnValue({});
        (selectNetworkConfigurations as unknown as jest.Mock).mockReturnValue(
          {},
        );
      ...
      it('uses a fallback currency-rate entry when the source chain has none', () => {
        (selectNetworkConfigurations as unknown as jest.Mock).mockReturnValue({
          '0x1': { nativeCurrency: 'UNKNOWN' },
        });
        (selectCurrencyRates as unknown as jest.Mock).mockReturnValue({
    ...
    describe('source chain client lookup', () => {
      it('keeps sourceChainId when NetworkController has no client for the chain', () => {
        ...
        expect(result.current.sourceChainId).toBe('0x1');
      });
    });

    describe('quote display fields', () => {
      const quotedDisplayState = (): UseQuickBuyQuotesResult => ({
        activeQuote: createActiveQuote({
          quote: {
            gasIncluded: true,
            src: { amount: '10000000000000000', normalizedAmount: '0.01' },
            dest: { minAmountNormalized: '1', normalizedAmount: '0.01' },
            feeData: {
              txFee: [{ valueInCurrency: '1.5' }],
            },
            priceData: { priceImpact: { amount: '0.0123' } },
          },
        }),
        destTokenAmount: '20',
        isQuoteLoading: false,
      ...
      it('formats minimum received from dest.minAmountNormalized', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '1',
          atomicBalance: undefined,
        });
        ...
        expect(result.current.formattedMinimumReceived).toBe('1 TARGET');
      });
      ...
      it('formats price impact as a percent', () => {
        ...
        expect(result.current.formattedPriceImpact).toBe('1.23%');
      });
      ...
      it('formats a rate at or below 1 with significant digits', () => {
        ...
        expect(result.current.formattedRate).toBe('1 ETH = 0.50 TARGET');
      });
      ...
      it('returns no formatted rate when the quote source amount is zero', () => {
        setupQuoteSourceMock({
          ...
          activeQuote: createActiveQuote({
            quote: {
              src: { amount: '0', normalizedAmount: '0' },
            },
          }),
        });
        ...
        expect(result.current.formattedRate).toBeUndefined();
      });
      ...
      it('shows Auto slippage when slippage is unset', () => {
        ...
        expect(result.current.formattedSlippage).toBe('Auto');
      });
      ...
      it('shows a dash price impact when priceData is missing', () => {
        ...
        expect(result.current.formattedPriceImpact).toBe('-');
      });
      ...
      it('shows a dash minimum received when dest min amount is missing', () => {
        setupQuoteSourceMock({
          ...
          destTokenAmount: undefined,
          activeQuote: createActiveQuote({
            quote: { dest: undefined },
          }),
        });

        const { result } = renderHook(createTarget(), jest.fn());

        expect(result.current.formattedMinimumReceived).toBe('-');
      });
    });

    describe('slider', () => {
      it('ignores a slider tick that rounds to the same percent', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '100',
          atomicBalance: undefined,
        });
        mockUsePayWithTokens.mockReturnValue({
          options: [createSourceToken({ currencyExchangeRate: 1 })],
          isLoading: false,
        });
        ...
        act(() => {
          result.current.handleSliderChange(50);
          result.current.handleSliderChange(50.2);
        });

        expect(result.current.sliderPercent).toBe(50);
      });

      it('sets sourceAmountTokens from the slider when the source is unpriced', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '10',
          atomicBalance: undefined,
        });
        mockUsePayWithTokens.mockReturnValue({
          options: [createSourceToken({ currencyExchangeRate: undefined })],
          isLoading: false,
        });
        ...
        expect(result.current.sourceAmountTokens).toBe('5');
      });

      it('spends the full token balance at 100% when the source is unpriced', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '10',
          atomicBalance: undefined,
        });
        ...
        expect(result.current.sourceTokenAmount).toBe('10');
      });

      it('clears the unpriced slider amount at 0%', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '10',
          atomicBalance: undefined,
        });
        ...
        expect(result.current.sourceAmountTokens).toBe('');
      });

      it('clears the priced slider fiat at 0%', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '100',
          atomicBalance: undefined,
        });
      ...
      it('clears the unpriced slider amount when there is no token balance', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: undefined,
          atomicBalance: undefined,
        });
      ...
      it('tracks slider release against the dest token in sell mode', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '100',
          atomicBalance: undefined,
        });
        ...
        mockUseReceiveTokens.mockReturnValue([
          createSourceToken({ symbol: 'USDC', address: '0xusdc' }),
        ]);
      ...
      it('commits an empty fiat amount when the priced slider ends at 0%', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '100',
          atomicBalance: undefined,
        });
        ...
        act(() => {
          result.current.handleSliderDragEnd(0);
        });
      ...
      it('does not track slider release after a committed amount is cleared to 0%', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '100',
          atomicBalance: undefined,
        });
      ...
      it('clears fiat when the priced slider has no spendable balance', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '0',
          atomicBalance: undefined,
        });
        ...
        act(() => {
          result.current.handleSliderChange(25);
        });

        expect(result.current.fiatAmount).toBe('');
      });

      it('does not track slider release when there is no spendable balance', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '0',
          atomicBalance: undefined,
        });
    ...
    describe('quick amounts and display mode', () => {
      it('ignores a zero quick amount press', () => {
        const { result } = renderHook(createTarget(), jest.fn());

        act(() => {
          result.current.handleQuickAmountPress(0);
        });

        expect(result.current.fiatAmount).toBe('');
      });

      it('leaves source amount unset when an unpriced token has no entered tokens', () => {
        mockUsePayWithTokens.mockReturnValue({
          options: [createSourceToken({ currencyExchangeRate: undefined })],
          isLoading: false,
        });
        ...
        expect(result.current.hasSourcePrice).toBe(false);
        expect(result.current.sourceTokenAmount).toBeUndefined();
      });

      it('leaves source amount unset when an unpriced token amount is zero', () => {
        mockUsePayWithTokens.mockReturnValue({
          options: [createSourceToken({ currencyExchangeRate: undefined })],
          isLoading: false,
        });
        ...
        expect(result.current.sourceAmountTokens).toBe('0');
        expect(result.current.sourceTokenAmount).toBeUndefined();
      });

      it('sets slider percent to 0 when a quick amount is pressed with no spendable balance', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: '0',
          atomicBalance: undefined,
        });
      ...
      it('tracks a quick buy amount without a pay-with token symbol', () => {
        mockUsePayWithTokens.mockReturnValue({
          options: [createSourceToken({ symbol: undefined })],
          isLoading: false,
        });
        ...
        act(() => {
          result.current.handleQuickAmountPress(20, 20);
        });
      ...
      it('tracks a quick sell amount without a receive token', () => {
        mockUsePositionTokenBalance.mockReturnValue(
          createSourceToken({
            address: '0xDEST',
            symbol: 'TARGET',
      ...
      it('does not track a custom amount of zero', () => {
        jest.useFakeTimers();
      ...
      it('ignores a NaN quick amount press', () => {
        const { result } = renderHook(createTarget(), jest.fn());

        act(() => {
          result.current.handleQuickAmountPress(Number.NaN);
        });

        expect(result.current.fiatAmount).toBe('');
      });

      it('focuses the hidden input when the amount area is pressed', () => {
        const { result } = renderHook(createTarget(), jest.fn());
        const focus = jest.fn();
        (
          result.current.hiddenInputRef as { current: { focus: () => void } }
        ).current = { focus };

        act(() => {
          result.current.handleAmountAreaPress();
        });

        expect(focus).toHaveBeenCalled();
      });

      it('sets crypto display mode when the amount area is pressed without a price', () => {
        mockUsePayWithTokens.mockReturnValue({
          options: [createSourceToken({ currencyExchangeRate: undefined })],
          isLoading: false,
        });
        ...
        expect(result.current.amountDisplayMode).toBe('crypto');
      });

      it('hides the source balance when displayBalance is not a number', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: 'abc',
          atomicBalance: undefined,
        });
        ...
        expect(result.current.sourceBalanceDisplay).toBeUndefined();
        expect(result.current.sourceBalanceFiat).toBe('$0.00');
      });

      it('hides the source balance when displayBalance is NaN', () => {
        (useLatestBalance as jest.Mock).mockReturnValue({
          displayBalance: 'NaN',
          atomicBalance: undefined,
        });
      ...
      it('toggles amount display from crypto back to fiat', () => {
        const { result } = renderHook(createTarget(), jest.fn());

        act(() => {
          result.current.handleToggleAmountDisplay();
          result.current.handleToggleAmountDisplay();
        });
      ...
      it('toggles amount display from fiat to crypto', () => {
        const { result } = renderHook(createTarget(), jest.fn());
      ...
      it('converts an entered token amount to fiat when a price arrives', () => {
        const unpriced = createSourceToken({ currencyExchangeRate: undefined });
        mockUsePayWithTokens.mockReturnValue({
          options: [unpriced],
          isLoading: false,
        });
        ...
        mockUsePayWithTokens.mockReturnValue({
          options: [createSourceToken({ currencyExchangeRate: 10 })],
          isLoading: false,
        });
        rerender(props);
      ...
      it('tracks a custom sell amount against the dest token', () => {
        jest.useFakeTimers();
      ...
      it('tracks custom amount entry after the typing debounce', () => {
        jest.useFakeTimers();
      ...
      it('does not re-track a custom amount that already matches the last tracked usd', () => {
        jest.useFakeTimers();

        try {
          const { result } = renderHook(createTarget(), jest.fn());
          const lastTrackedAmountRef = mockUseQuickBuyAnalytics.mock.results.at(
            -1,
          )?.value.refs.lastTrackedAmountRef as { current: string };
          lastTrackedAmountRef.current = '20';
    ...
    describe('button labels and errors', () => {
      it('returns the submitting label while a transaction is in flight', () => {
        (selectIsSubmittingTx as unknown as jest.Mock).mockReturnValue(true);
      ...
      it('does not track slippage changes when the user did not override slippage', () => {
        (selectIsSlippageUserOverride as unknown as jest.Mock).mockReturnValue(
          false,
        );
        const target = createTarget();
        const onClose = jest.fn();
        const { rerender } = renderHook(target, onClose);

        (selectSlippage as unknown as jest.Mock).mockReturnValue('1');
        rerender({ target, onClose });

        expect(mockTrackSlippageChanged).not.toHaveBeenCalled();
      });

      it('tracks slippage as Auto when the value is cleared', () => {
        const target = createTarget();
        const onClose = jest.fn();
        const { rerender } = renderHook(target, onClose);
        ...
        expect(mockTrackSlippageChanged).toHaveBeenCalledWith('Auto', '0.5');
      });

      it('returns the sell label in sell mode', () => {
        mockUsePositionTokenBalance.mockReturnValue(
          createSourceToken({
            address: '0xDEST',
            symbol: 'TARGET',
        ...
        expect(result.current.getButtonLabel()).toBe(
          'social_leaderboard.trader_position.sell',
        );
      });
      ...
      it('sets buttonError to no_quotes when the quotes hook reports none', () => {
        ...
        expect(result.current.buttonError).toBe('no_quotes');
      });
    });

    describe('quote selection and confirm guards', () => {
      it('disables confirm when token decimals cannot be converted to atomic units', () => {
        mockUsePayWithTokens.mockReturnValue({
          options: [
            createSourceToken({
              decimals: {
                valueOf() {
                  throw new Error('cannot convert decimals');
                },
      ...
      it('does not track quote_selected when the request id is not in the batch', () => {
        ...
        act(() => {
          result.current.handleSelectQuote('missing');
        });

        expect(mockTrackQuoteSelected).not.toHaveBeenCalled();
      });

      it('does not submit when wallet address is missing', async () => {
        (selectSourceWalletAddress as unknown as jest.Mock).mockReturnValue(
          undefined,
        );
        setupQuoteSourceMock({
      ...
      it('omits trade submitted analytics when caip19 cannot be resolved', async () => {
        ...
        expect(mockTrackTradeSubmitted).not.toHaveBeenCalled();
      });
      ...
      it('omits trade completed analytics when submit throws and caip19 cannot be resolved', async () => {
        ...
        expect(mockTrackTradeCompleted).not.toHaveBeenCalled();
      });

      it('tags submit failures with unknown chain ids when tokens omit chainId', async () => {
        mockUsePayWithTokens.mockReturnValue({
          options: [createSourceToken({ chainId: undefined })],
          isLoading: false,
        });
        mockUseQuickBuySetup.mockReturnValue({
          chainId: '0x1',
      ...
      it('does not track receive_token_selected when the same symbol is re-selected', () => {
        const usdc = createSourceToken({
          symbol: 'USDC',
          address: '0xusdc',
          currencyExchangeRate: 1,
        });
        mockUseReceiveTokens.mockReturnValue([usdc]);
        mockUsePositionTokenBalance.mockReturnValue(
          createSourceToken({
            address: '0xDEST',
            symbol: 'TARGET',
