...
class SardineWebView extends PureComponent {
		...
		if(isSuccess && orderData) {
			...
			InteractionManager.runAfterInteractions(() => {
				AnalyticsV2.trackEvent(AnalyticsV2.ANALYTICS_EVENTS.ONRAMP_PURCHASE_SUBMITTED, {
					...
					'on-ramp_provider': { value: FIAT_ORDER_PROVIDERS.SARDINE, anonymous: true },
				});
			...
			InteractionManager.runAfterInteractions(() => {
				AnalyticsV2.trackEvent(AnalyticsV2.ANALYTICS_EVENTS.ONRAMP_PURCHASE_FAILED, {
					'on-ramp_provider': { value: FIAT_ORDER_PROVIDERS.SARDINE, anonymous: true },
				});
	...
	render() {
		...
		const userAgent = "Mozilla/5.0 (Linux; Android 8.0.0; Pixel 2 XL Build/OPD1.170816.004; Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.97 Safari/537.36";
