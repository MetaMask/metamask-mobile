...
const TRANSAK_API_KEY_SECRET_STAGING =
  process.env.TRANSAK_API_KEY_SECRET_STAGING;
const TRANSAK_API_KEY_PRODUCTION = process.env.TRANSAK_API_KEY_PRODUCTION;
const TRANSAK_API_KEY_SECRET_PRODUCTION =
  process.env.TRANSAK_API_KEY_SECRET_PRODUCTION;
...
const TRANSAK_API_BASE_URL = `${
  isDevelopment ? TRANSAK_API_URL_STAGING : TRANSAK_API_URL_PRODUCTION
}api/v2/`;
const TRANSAK_API_KEY = isDevelopment
  ? TRANSAK_API_KEY_STAGING
  : TRANSAK_API_KEY_PRODUCTION;
const TRANSAK_API_KEY_SECRET = isDevelopment
...
const getOrderStatus = (orderId) =>
  transakApi.get(`partners/order/${orderId}`, {
    params: { partnerAPISecret: TRANSAK_API_KEY_SECRET },
  });
...
export async function processTransakOrder(order) {
  try {
    ...
    Logger.error(error, {
      message: 'FiatOrders::TransakProcessor error while processing order',
      order,
    });
    return order;
  }
...
export const useTransakFlowURL = (address, chainId) => {
  const params = useMemo(() => {
    const selectedChainId = isTransakAllowedToBuy(chainId)
      ? chainId
      : NETWORKS_CHAIN_ID.MAINNET;
    const [network, defaultCryptoCurrency, cryptoCurrencyList] =
      TRANSAK_NETWORK_PARAMETERS[selectedChainId];
    return qs.stringify({
