...
const SARDINE_API_KEY_SECRET_STAGING = process.env.SARDINE_API_KEY_SECRET_STAGING;
const SARDINE_API_KEY_PRODUCTION = process.env.SARDINE_API_KEY_PRODUCTION;
const SARDINE_API_KEY_SECRET_PRODUCTION = process.env.SARDINE_API_KEY_SECRET_PRODUCTION;
...
export const isSardineAllowedToBuy = (chainId) => SARDINE_ALLOWED_NETWORKS.includes(chainId);

//* Constants

const { SARDINE_URL, SARDINE_URL_STAGING, SARDINE_API_URL_STAGING, SARDINE_API_URL_PRODUCTION, SARDINE_REDIRECT_URL } =
	AppConstants.FIAT_ORDERS;

const isDevelopment = process.env.NODE_ENV !== 'production';

const SARDINE_API_BASE_URL = `${isDevelopment ? SARDINE_API_URL_STAGING : SARDINE_API_URL_PRODUCTION}`;
const SARDINE_API_KEY = isDevelopment ? SARDINE_API_KEY_STAGING : SARDINE_API_KEY_PRODUCTION;
const SARDINE_API_KEY_SECRET = isDevelopment ? SARDINE_API_KEY_SECRET_STAGING : SARDINE_API_KEY_SECRET_PRODUCTION;
...
const getClientToken = (orderId) =>
	sardineApi.get(`auth/client-token`, { params: { partnerAPISecret: SARDINE_API_KEY_SECRET } });
const getOrderStatus = (orderId) =>
	sardineApi.get(`trade/${orderId}`, { params: { partnerAPISecret: SARDINE_API_KEY_SECRET } });
 ...
 */
 export const sardineCallbackOrderToFiatOrder = (sardineRedirectOrder) => ({
	...
	cryptocurrency: sardineRedirectOrder.fiat_type || sardineRedirectOrder.asset_type,
...
export async function processSardineOrder(order) {
	try {
		...
		Logger.error(error, { message: 'FiatOrders::SardineProcessor error while processing order', order });
		return order;
	}
}
...
export const useSardineFlowURL = (address, chainId) => {
	const params = useMemo(() => {
		const selectedChainId = isSardineAllowedToBuy(chainId) ? chainId : NETWORKS_CHAIN_ID.MAINNET;
		const [network, defaultCryptoCurrency, cryptoCurrencyList] = SARDINE_NETWORK_PARAMETERS[selectedChainId];
		return qs.stringify({
