...
| `usePredictBuyConditions`       | Derives boolean flags (`canPlaceBet`, `isBelowMinimum`, `isInsufficientBalance`, `isInsufficientPayTokenBalance`, `isRateLimited`, `isPayFeesLoading`, `isBalancePulsing`, etc.) from the rounded all-in order cost, preview state, and pay-system state. Includes stale-quote detection to bridge `TransactionPayController` timing gaps |
...
| `usePredictBuyInfo`             | Computes display values (`toWin`, `metamaskFee`, `providerFee`, `exchangeFee`, `depositFee`, `depositAmount`, `total`, `rewardsFeeAmount`) from preview, `TransactionPayController` totals, and Predict balance                                                                                                                           |
