...
describe('usePredictBalanceTokenFilter', () => {
  ...
  it('shows the formatted balance as the row description', () => {
    mockPredictBalance = 42.5;
    mockHasTransactionType.mockReturnValue(true);
    const tokens = [createMockToken()];
  ...
  it('uses pUSD as the fiat description metadata', () => {
    mockHasTransactionType.mockReturnValue(true);
    const tokens = [createMockToken()];
    ...
    expect((filteredTokens[0] as HighlightedItem).fiat_description).toBe(
      'pUSD',
    );
  });
