...
describe('PredictController', () => {
  ...
  describe('getClaimablePositions', () => {
    it('get claimable positions successfully with default provider', async () => {
      // Given
      ...
      mockPolymarketProvider.getPositions.mockResolvedValue(mockPositions);

      await withController(async ({ controller }) => {
        // When calling getClaimablePositions
        const result = await controller.getClaimablePositions({});

        // Then it should return the positions
        expect(result).toEqual(mockPositions);

        // And provider should be called with claimable: true and default address
        expect(mockPolymarketProvider.getPositions).toHaveBeenCalledWith({
          address: '0x1234567890123456789012345678901234567890',
          claimable: true,
        });
    ...
    it('get claimable positions successfully with custom provider and address', async () => {
      // Given
      const mockPositions: PredictPosition[] = [
        {
          id: 'pos-1',
      ...
      await withController(async ({ controller }) => {
        // When calling getClaimablePositions with custom params
        const result = await controller.getClaimablePositions({
          providerId: 'polymarket',
        ...
        // And provider should be called with claimable: true and custom address
        expect(mockPolymarketProvider.getPositions).toHaveBeenCalledWith({
          providerId: 'polymarket',
          address: '0x9876543210987654321098765432109876543210',
          claimable: true,
    ...
    it('merge claimable positions from multiple providers', async () => {
      // Given positions from two providers
      const mockPositions1: PredictPosition[] = [
        {
          id: 'pos-1',
      ...
      const mockPositions2: PredictPosition[] = [
        {
          id: 'pos-2',
      ...
      const mockProvider2 = {
        getPositions: jest.fn().mockResolvedValue(mockPositions2),
        providerId: 'provider2',
      };

      await withController(async ({ controller }) => {
        // Add second provider to controller
        (controller as any).providers.set('provider2', mockProvider2 as any);

        mockPolymarketProvider.getPositions.mockResolvedValue(mockPositions1);

        // When calling getClaimablePositions without providerId
        const result = await controller.getClaimablePositions({});

        // Then it should return merged positions from both providers
        expect(result).toEqual([...mockPositions1, ...mockPositions2]);
        expect(result).toHaveLength(2);
      });
    });

    it('handle error when provider is not available', async () => {
      await withController(async ({ controller }) => {
        // When calling getClaimablePositions with invalid provider
        // Then it should throw an error
        await expect(
          controller.getClaimablePositions({
            providerId: 'invalid-provider',
          }),
        ).rejects.toThrow('PROVIDER_NOT_AVAILABLE');
      });
    ...
    it('handle error when getPositions throws', async () => {
      // Given
      mockPolymarketProvider.getPositions.mockRejectedValue(
        new Error('Network error'),
      );
    ...
    it('filter out undefined positions from providers', async () => {
      // Given
      const mockPositions: PredictPosition[] = [
        {
          id: 'pos-1',
      ...
      await withController(async ({ controller }) => {
        ...
        // Then it should filter out undefined positions
        expect(result).toEqual(mockPositions);
        expect(result).toHaveLength(1);
      });
    });
