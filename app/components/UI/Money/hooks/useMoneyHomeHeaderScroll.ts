...
export interface UseMoneyHomeHeaderScrollParams {
  /**
...
export interface UseMoneyHomeHeaderScrollResult {
...
/**
 * Drives Money home's scroll-linked native header title.
 *
 * At rest the large in-content "Money" title is visible and the native header
 * title is empty (so only the liquid-glass more button shows). Once scroll
 * passes the title section, the large title fades out and the native header
 * title is set to "Money" (centered).
 *
 ...
 */
export const useMoneyHomeHeaderScroll = ({
  onCompactTitleVisibilityChange,
}: UseMoneyHomeHeaderScrollParams): UseMoneyHomeHeaderScrollResult => {
