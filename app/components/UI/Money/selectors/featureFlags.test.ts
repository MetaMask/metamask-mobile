...
import {
  ...
  selectCardEducationAnimationEnabledFlag,
...
describe('selectCardEducationAnimationEnabledFlag', () => {
  const originalEnv = process.env;
  ...
  it('returns true when remote flag is enabled and version requirement is met', () => {
    ...
    const state = createState({
      earnCardEducationAnimationEnabled: {
        enabled: true,
        minimumVersion: '1.0.0',
      },
    });

    const result = selectCardEducationAnimationEnabledFlag(state as never);
  ...
  it('defaults to true when remote flag returns undefined and env is unset', () => {
    mockedValidate.mockReturnValue(undefined);
    delete process.env.MM_CARD_EDUCATION_ANIMATION_ENABLED;
  ...
  it('returns false when env var is set to false and remote is undefined', () => {
    mockedValidate.mockReturnValue(undefined);
    process.env.MM_CARD_EDUCATION_ANIMATION_ENABLED = 'false';
  ...
  it('remote flag takes precedence over env var', () => {
    mockedValidate.mockReturnValue(false);
    process.env.MM_CARD_EDUCATION_ANIMATION_ENABLED = 'true';
