...
const MoneyRemoveSocialSheet = () => {
  const sheetRef = useRef<BottomSheetRef>(null);
  const navigation = useNavigation<AppNavigationProp>();
  const { passkeyCount } = useMoneyFinishSetup();
  const { isAuthenticatorAdded, isSmsAdded, removeSocial } =
    useMoneySecurityMethods();
  const canRemove = passkeyCount > 0 || isAuthenticatorAdded || isSmsAdded;
  ...
  const handleRemove = useCallback(() => {
    ...
    sheetRef.current?.onCloseBottomSheet(() => {
      ...
      showSuccessToast(strings('money.social.removed_toast'));
    });
  }, [navigation, removeSocial, showSuccessToast]);
      ...
      <Box style={styles.content}>
        ...
        <Box alignItems={BoxAlignItems.Center} twClassName="px-2 pt-6 gap-4">
          ...
          >
            {strings(
              canRemove
                ? 'money.social.remove_title'
                : 'money.social.cannot_remove_title',
            )}
          </Text>
          <Text
          ...
          >
            {strings(
              canRemove
                ? 'money.social.remove_description'
                : 'money.social.cannot_remove_description',
            )}
          </Text>
        </Box>
        ...
        {canRemove ? (
          <Box gap={3} twClassName="mt-6">
            <Button
              ...
              testID={MoneyRemoveSocialSheetTestIds.CONFIRM_BUTTON}
            ...
            >
              ...
              >
                {strings('money.social.confirm_remove')}
              </Text>
            </Button>
            <Button
...
export default MoneyRemoveSocialSheet;
