...
/**
 * SnapshotCommitmentView allows users to enter the amount of points to commit to a snapshot.
 * Features a numeric keypad, percentage buttons, and displays the user's available balance.
 */
const SnapshotCommitmentView: React.FC = () => {
  ...
  const route =
    useRoute<
      RouteProp<
        { params: { snapshotName: string; hasExistingCommitment: boolean } },
        'params'
      >
    >();
  const { snapshotName, hasExistingCommitment } = route.params;
  ...
  const headerTitle = useMemo(() => {
    if (hasExistingCommitment) {
      return strings('rewards.snapshots.add_points_for', {
        snapshotName,
      });
    }
    return strings('rewards.snapshots.enter_snapshot', { snapshotName });
  }, [hasExistingCommitment, snapshotName]);
  ...
  return (
    <ErrorBoundary navigation={navigation} view="SnapshotCommitmentView">
      <View
        style={tw.style('flex-1 bg-background-default')}
        testID="snapshot-commitment-view"
      >
        ...
        >
          ...
          >
            {strings('rewards.snapshots.points_available', {
        ...
        <Box twClassName="bg-background-alternative rounded-t-2xl pt-4 pb-6">
          ...
          <Box twClassName="px-4 mt-4">
            <Button
              ...
              label={strings('rewards.snapshots.commit')}
...
export default SnapshotCommitmentView;
