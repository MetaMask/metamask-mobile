...
describe('useReferralDetails', () => {
  ...
  const mockGetReferralDetails = jest.fn();
  ...
  beforeEach(() => {
    ...
    (
      Engine as unknown as {
        context: { RewardsController: { getReferralDetails: jest.Mock } };
      }
    ).context = {
      RewardsController: {
        getReferralDetails: mockGetReferralDetails,
      },
    };

    // Mock successful API call by default
    mockGetReferralDetails.mockResolvedValue(mockReferralDetailsResponse);
  ...
  describe('successful fetch', () => {
    it('fetches referral details and updates Redux store', async () => {
      ...
      // Assert
      expect(mockGetReferralDetails).toHaveBeenCalledWith('subscription-123');
    ...
    it('logs success message when fetch completes', async () => {
      // Arrange
      const { result } = renderHook(() => useReferralDetails());
  ...
  describe('loading state management', () => {
    it('sets loading to true during fetch and false after completion', async () => {
      ...
      mockGetReferralDetails.mockImplementation(
        () =>
          new Promise((resolve) => {
            setTimeout(() => {
    ...
    it('resets error state when starting new fetch', async () => {
      ...
      mockGetReferralDetails.mockRejectedValueOnce(new Error('Network error'));
      await act(async () => {
        await result.current.fetchReferralDetails();
      });
      ...
      mockGetReferralDetails.mockResolvedValueOnce(mockReferralDetailsResponse);
  ...
  describe('error handling', () => {
    it('handles API errors and sets error message', async () => {
      ...
      mockGetReferralDetails.mockRejectedValue(new Error(errorMessage));
      const { result } = renderHook(() => useReferralDetails());
    ...
    it('handles non-Error objects and sets unknown error message', async () => {
      // Arrange
      mockGetReferralDetails.mockRejectedValue('string error');
      const { result } = renderHook(() => useReferralDetails());
    ...
    it('logs error details when fetch fails', async () => {
      ...
      mockGetReferralDetails.mockRejectedValue(error);
      const { result } = renderHook(() => useReferralDetails());
  ...
  describe('subscription ID validation', () => {
    it('returns early when no subscription ID is available', async () => {
      ...
      // Assert
      expect(mockGetReferralDetails).not.toHaveBeenCalled();
      expect(mockDispatch).not.toHaveBeenCalled();
      expect(mockLogger.log).toHaveBeenCalledWith(
  ...
  describe('data mapping', () => {
    it('correctly maps API response to Redux action payload', async () => {
      ...
      mockGetReferralDetails.mockResolvedValue(customResponse);
      const { result } = renderHook(() => useReferralDetails());
