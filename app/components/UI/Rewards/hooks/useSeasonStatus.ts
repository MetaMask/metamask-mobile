  ...
  const fetchSeasonStatus = useCallback(
    async (isRefresh = false): Promise<void> => {
      ...
      try {
        ...
        // Get the RewardsController from Engine and call the public method
        const controller = Engine.context.RewardsController;
        const statusData = await controller.getSeasonStatus(
          seasonId,
          subscriptionId,
        );
