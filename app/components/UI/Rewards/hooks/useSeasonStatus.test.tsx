...
const mockRewardsController = Engine.context.RewardsController as jest.Mocked<
  typeof Engine.context.RewardsController
>;
...
describe('useSeasonStatus', () => {
  ...
  beforeEach(() => {
    ...
    mockRewardsController.getSeasonStatus.mockResolvedValue(mockSeasonStatus);
  });
  ...
  describe('Successful data fetching', () => {
    it('fetches season status successfully on mount', async () => {
      ...
      expect(result.current.error).toBeNull();
      expect(mockRewardsController.getSeasonStatus).toHaveBeenCalledTimes(1);
      expect(mockRewardsController.getSeasonStatus).toHaveBeenCalledWith(
        mockSeasonId,
        mockSubscriptionId,
      );
    ...
    it('skips initial fetch when skipInitialFetch is true', async () => {
      ...
      expect(result.current.isLoading).toBe(false);
      expect(result.current.seasonStatus).toBeNull();
      expect(mockRewardsController.getSeasonStatus).not.toHaveBeenCalled();
    });
    ...
    it('updates season status when data changes', async () => {
      ...
      mockRewardsController.getSeasonStatus.mockResolvedValue(newSeasonStatus);
  ...
  describe('Error handling', () => {
    it('handles fetch errors with null season status', async () => {
      ...
      mockRewardsController.getSeasonStatus.mockRejectedValue(
    ...
    it('handles unknown error types', async () => {
      // Arrange
      mockRewardsController.getSeasonStatus.mockRejectedValue('String error');
  ...
  describe('Refresh functionality', () => {
    it('refreshes data when refresh function is called', async () => {
      ...
      mockRewardsController.getSeasonStatus.mockClear();
    ...
    it('sets isRefreshing to true during refresh', async () => {
      ...
      mockRewardsController.getSeasonStatus.mockReturnValue(hangingPromise);
    ...
    it('clears error on successful refresh', async () => {
      // Arrange
      mockRewardsController.getSeasonStatus.mockRejectedValueOnce(
  ...
  describe('Polling functionality', () => {
    ...
    it('continues polling multiple times', async () => {
      ...
      // Assert
      expect(mockRewardsController.getSeasonStatus).toHaveBeenCalledTimes(2);
    });
  ...
  describe('Edge cases', () => {
    it('handles concurrent refresh calls', async () => {
      ...
      expect(mockRewardsController.getSeasonStatus).toHaveBeenCalledTimes(3);
    });
