...
- **Composable UI.** Seven compound components replace 45 directories. One `EventCard` renders a stable `EventDisplayModel` prepared by widgets, so every variant stays behind one presentation module. One `OutcomeButton` is the single place you bet in the entire app.
...
| ----- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------- |
| 1     | Foundation                                                | Types, query descriptor contracts, `VenueAdapter` contract + derived `PredictClient` type, session-service contract, error model, translation layer                                                                                                                                                                    | 2-3      | None                                |
...
| 3     | Read Services                                             | Query descriptors, MarketDataService, PortfolioService (BaseDataService), read-model writer interfaces, old controller delegates reads                                                                                                                                                                                 | 3-4      | Phase 2                             |
| 4     | Write Services                                            | TradingService (Stateful, owns active-order workflow slice), TransactionService (Runtime, public user-intent actions) + shared lifecycle-aware `TransactionExecutor` primitive, LiveDataService (Runtime), `predictAnalytics` helper (injected, not a service); old controller delegates write methods to new services | 4-5      | Phase 2 + writer-interface contract |
...
Note: Phases 3 and 4 can run in parallel because read services and write services are independently testable. Both depend on the `VenueAdapter` contract (with derived `PredictClient`), `PolymarketAdapter`, and `PredictSessionService` from Phase 2. Phase 4 also depends on the narrow read-model writer interface contract; concrete writer implementations can arrive with Phase 3 while Phase 4 uses test doubles.
...
- Phase 3 builds query descriptors, read services, read-model writer interfaces, and hollows out the old controller's read methods.

### Stream B: Write path

`Phase 1 → Phase 2 → Phase 4`

- Phase 4 can start as soon as the `VenueAdapter` + session seam from Phase 2 is stable and the read-model writer interface contract is agreed.
- Write services (trading, transactions, live data, analytics) are independent of read-service implementation; their cache coordination uses narrow read-model writer interfaces that can be stubbed until the concrete Phase 3 read services land.
