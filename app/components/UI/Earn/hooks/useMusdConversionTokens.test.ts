...
describe('useMusdConversionTokens', () => {
  ...
  const mockAUsdcMainnet: AssetType = {
  ...
  const mockAUsdtMainnet: AssetType = {
    ...
    logo: 'https://example.com/ausdt.png',
    isETH: false,
    aggregators: [],
    image: 'https://example.com/ausdt.png',
  };

  const mockADaiMainnet: AssetType = {
    ...
    logo: 'https://example.com/adai.png',
    isETH: false,
    aggregators: [],
    image: 'https://example.com/adai.png',
  };
  ...
  describe('hasConvertibleTokensByChainId', () => {
    it('returns true when a convertible token exists on the given chain', () => {
      mockUseAccountTokens.mockReturnValue([mockUsdcMainnet]);
      mockIsTokenAllowed.mockReturnValue(true);
      ...
      expect(
        result.current.hasConvertibleTokensByChainId(CHAIN_IDS.MAINNET),
      ).toBe(true);
    });

    it('returns false when no convertible token exists on the given chain', () => {
      mockUseAccountTokens.mockReturnValue([mockUsdcMainnet]);
      mockIsTokenAllowed.mockReturnValue(true);
      ...
      expect(
        result.current.hasConvertibleTokensByChainId(CHAIN_IDS.LINEA_MAINNET),
      ).toBe(false);
    });
  });

  describe('min balance branches', () => {
    it('filters out token when fiat balance is null', () => {
      ...
      mockUseAccountTokens.mockReturnValue([usdcNullFiat]);
      mockIsTokenAllowed.mockReturnValue(true);
    ...
    it('filters out token when fiat balance is not finite', () => {
      const usdcNanFiat = {
        ...mockUsdcMainnet,
        fiat: {
      ...
      mockUseAccountTokens.mockReturnValue([usdcNanFiat]);
      mockIsTokenAllowed.mockReturnValue(true);
  ...
  describe('isConversionToken chainId guard', () => {
    it('returns false when token has no chainId', () => {
      ...
      expect(result.current.isConversionToken(tokenWithoutChainId)).toBe(false);
    });
  });

  describe('aToken inclusion', () => {
    it('includes held aUSDC, aUSDT, and aDAI in tokens when all tokens are allowed', () => {
      mockUseAccountTokens.mockReturnValue([
        mockAUsdcMainnet,
        mockAUsdtMainnet,
        mockADaiMainnet,
      ...
      expect(result.current.tokens).toContainEqual(mockAUsdcMainnet);
      expect(result.current.tokens).toContainEqual(mockAUsdtMainnet);
      expect(result.current.tokens).toContainEqual(mockADaiMainnet);
      expect(result.current.tokens).toHaveLength(3);
    });

    it('includes aTokens alongside stablecoins, ordering stablecoins first', () => {
      mockUseAccountTokens.mockReturnValue([
        mockUsdcMainnet,
        mockUsdtMainnet,
        mockDaiMainnet,
      ...
      // Stablecoins (by fiat desc) come first, then aTokens (by fiat desc).
      expect(result.current.tokens).toEqual([
        mockDaiMainnet,
        mockUsdtMainnet,
        mockUsdcMainnet,
