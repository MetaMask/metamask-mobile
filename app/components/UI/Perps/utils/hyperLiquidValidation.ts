...
import { HYPERLIQUID_ASSET_CONFIGS, getSupportedAssets } from '../constants/hyperLiquidConfig';
 ...
 */
export function createErrorResult<T extends { success: boolean; error?: string }>(
  error: unknown,
  defaultResponse: T
): T {
  return {
...
export function validateWithdrawalParams(params: {
  ...
  if (params.destination && !isValidHexAddress(params.destination)) {
    return { isValid: false, error: `Invalid destination address format: ${params.destination}` };
  }
 ...
 */
export function applyPathFilters(assets: CaipAssetId[], params?: GetSupportedPathsParams): CaipAssetId[] {
  if (!params) {
  ...
  if (params.symbol && params.symbol in HYPERLIQUID_ASSET_CONFIGS) {
    const config = HYPERLIQUID_ASSET_CONFIGS[params.symbol as keyof typeof HYPERLIQUID_ASSET_CONFIGS];
 ...
 */
export function getSupportedPaths(params?: GetSupportedPathsParams): CaipAssetId[] {
  const isTestnet = params?.isTestnet ?? false;
...
export function validateOrderParams(params: {
  ...
  if (params.price && parseFloat(params.price) <= 0) {
    return { isValid: false, error: 'Price must be a positive number if provided' };
  }
