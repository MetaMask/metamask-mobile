 ...
 * configures itself when the hasOverrides flag is detected.
 */
...
function registerE2EPerpsDeepLinkHandler(): void {
  if (hasRegisteredDeepLinkHandler || !hasOverrides) {
    return;
  }
...
/**
 * Auto-configure E2E bridge when hasOverrides is true
 */
function autoConfigureE2EBridge(): void {
  if (!hasOverrides || e2eBridgePerps.mockStreamManager) {
...
export function applyE2EControllerMocks(controller: unknown): void {
  // Use unified hasTestOverrides flag so CI/local runs with either HAS_OVERRIDES=true
  if (hasOverrides) {
