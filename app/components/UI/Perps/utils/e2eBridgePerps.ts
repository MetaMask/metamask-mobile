...
function startE2EPerpsCommandPolling(): void {
  ...
  const port = process.env.E2E_API_PORT || '2446';
  ...
  async function pollOnce(): Promise<void> {
    try {
      ...
      const url = `${baseUrl}/e2e/perps/commands`;
      DevLogger.log('[E2E Bridge] Poll URL', url);
      ...
      if ((response as AxiosResponse).status !== 200) {
        ...
        if (consecutivePollFailures >= MAX_CONSECUTIVE_POLL_FAILURES) {
          ...
          DevLogger.log(
            '[E2E Bridge] Disabling polling due to repeated non-200 responses',
          );
          return;
        }
        scheduleNext(pollIntervalMs);
      ...
      consecutivePollFailures = 0;
      DevLogger.log('[E2E Bridge] Poll data', data);
      ...
      DevLogger.log('[E2E Bridge] Poll error', err);
      consecutivePollFailures += 1;
      if (consecutivePollFailures >= MAX_CONSECUTIVE_POLL_FAILURES) {
        pollingDisabled = true;
        DevLogger.log('[E2E Bridge] Disabling polling due to repeated errors');
        return;
      }
      scheduleNext(pollIntervalMs);
    }
  ...
  DevLogger.log('[E2E Bridge] Starting E2E perps HTTP polling');
  scheduleNext(0);
}
