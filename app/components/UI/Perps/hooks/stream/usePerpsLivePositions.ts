...
export function usePerpsLivePositions(
  ...
  // Derive enriched positions synchronously from raw positions and price data.
  // Using useMemo instead of useEffect+setState eliminates a one-render gap
  // where isInitialLoading could be false while positions were still empty,
  // causing Long/Short buttons to flash before Modify/Close buttons appeared.
