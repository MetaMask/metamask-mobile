...
describe('usePerpsChaseOrders', () => {
  ...
  it('keeps a newer Chase child Active after the canceled child rotates', async () => {
    const childA = {
      ...activeOrder,
    ...
    expect(hook.result.current.chaseOrders).toEqual([childB]);
    hook.unmount();
  });

  it('keeps newer Filled child truth after the canceled child rotates', async () => {
    const childA = {
      ...activeOrder,
      restingOrderId: 'child-a',
    };
  ...
  it('ignores a pre-Chase order updated after the current child cancellation', async () => {
    const childA = {
      ...activeOrder,
      restingOrderId: 'child-current',
      startedAt: 100,
