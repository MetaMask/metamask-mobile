...
describe('PerpsTabView', () => {
  ...
  // Market mocks for bug regression: explore must include all categories (crypto, stocks, forex, commodities)
  ...
  describe('Homepage Redesign V1 Feature', () => {
    ...
    it("'See all perps' button navigates to perp market list", () => {
      const mockUsePerpsTabExploreData =
        jest.requireMock('../../hooks').usePerpsTabExploreData;
      mockUsePerpsTabExploreData.mockReturnValue({
        exploreMarkets: [mockMarket, mockMarketBTC],
      ...
      const seeAllPerpsButton = screen.getByText(
        strings('perps.home.see_all_perps'),
      );
      act(() => {
        fireEvent.press(seeAllPerpsButton);
      });
    ...
    it('Explore section includes all market categories (crypto, stocks, forex, commodities) and does not filter out any category', () => {
      const mockUsePerpsTabExploreData =
        jest.requireMock('../../hooks').usePerpsTabExploreData;
      mockUsePerpsTabExploreData.mockReturnValue({
        exploreMarkets: [
      ...
      expect(screen.getByText('ETH')).toBeOnTheScreen();
      expect(screen.getByText('XYZ100')).toBeOnTheScreen();
      expect(screen.getByText('EURUSD')).toBeOnTheScreen();
      expect(screen.getByText('GOLD')).toBeOnTheScreen();
    });

    it('Explore section in wallet home (perps tab) includes commodities when provided by explore data', () => {
      const mockUsePerpsTabExploreData =
        jest.requireMock('../../hooks').usePerpsTabExploreData;
      mockUsePerpsTabExploreData.mockReturnValue({
        exploreMarkets: [mockMarket, mockMarketCommodity],
        watchlistMarkets: [],
        isLoading: false,
      });
    ...
    it('when positions or orders exist, positions/orders sections are shown (tradeInfoContainer), not explore section', () => {
      mockUsePerpsLivePositions.mockReturnValue({
        positions: [mockPosition],
        isInitialLoading: false,
      });
      ...
      expect(
        screen.queryByText(strings('perps.home.see_all_perps')),
      ).not.toBeOnTheScreen();
      expect(
        screen.queryByText(strings('perps.home.explore_markets')),
      ).not.toBeOnTheScreen();
    });
  });

  describe('Connection state: connected vs disconnected vs loading', () => {
    it('when disconnected (isConnected false), view still renders control bar and content', () => {
      mockUsePerpsConnection.mockReturnValue({
        isConnected: false,
        isInitialized: true,
        isConnecting: false,
    ...
    it('when loading (isInitialLoading true with positions), view renders and shows positions section loading state', () => {
      mockUsePerpsLivePositions.mockReturnValue({
        positions: [mockPosition],
      ...
      // Explore section not shown when we have positions (loading branch uses tradeInfoContainer)
      expect(
        screen.queryByText(strings('perps.home.see_all_perps')),
      ).not.toBeOnTheScreen();
    });
