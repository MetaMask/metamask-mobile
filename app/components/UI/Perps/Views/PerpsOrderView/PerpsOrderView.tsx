...
const PerpsOrderViewContentBase: React.FC<PerpsOrderViewContentProps> = ({
  ...
  const orderValidation = usePerpsOrderValidation({
    ...
    availableBalance: balanceForValidation,
  ...
  const amountTimesLeverage = Math.floor(
    balanceForValidation * orderForm.leverage,
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      >
        ...
        <PerpsAmountDisplay
          amount={orderForm.amount}
          showWarning={!isLoadingAccount && balanceForValidation === 0}
          onPress={handleAmountPress}
          isActive={isInputFocused}
          tokenAmount={positionSize}
          tokenSymbol={getPerpsDisplaySymbol(orderForm.asset)}
          hasError={balanceForValidation > 0 && !!filteredErrors.length}
      ...
      <PerpsLeverageBottomSheet
        ...
        onConfirm={(leverage, inputMethod) => {
          ...
          const currentAmount = parseFloat(orderForm.amount || '0');
          const newMaxAmount = balanceForValidation * leverage;
