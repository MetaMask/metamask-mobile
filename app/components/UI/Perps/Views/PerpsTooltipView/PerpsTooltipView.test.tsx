...
describe('PerpsTooltipView', () => {
  ...
  describe('Different Content Keys', () => {
    const contentKeys: PerpsTooltipContentKey[] = [
      'leverage',
      'margin',
      'liquidation_price',
      'fees',
    ...
    for (const contentKey of contentKeys) {
      it(`renders correctly for ${contentKey} contentKey`, () => {
        mockUseRoute.mockReturnValue({
          params: {
            contentKey,
          },
        ...
        expect(screen.getByTestId('bottom-sheet')).toBeOnTheScreen();
        // Content is rendered (may or may not have testID depending on renderer type)
        // For custom renderers it will have testID, for default it will be text
        if (
          contentKey === 'fees' ||
          contentKey === 'market_hours' ||
          contentKey === 'after_hours_trading'
        ) {
          expect(
            screen.getByTestId(PerpsBottomSheetTooltipSelectorsIDs.CONTENT),
          ).toBeOnTheScreen();
        } else {
          // For default content, check that some content text is rendered
          // The exact text depends on the strings mock, but we verify component renders
          expect(screen.getByTestId('bottom-sheet-footer')).toBeOnTheScreen();
        }
      });
    }
  ...
  describe('Edge Cases', () => {
    it('handles undefined data prop gracefully', () => {
      ...
      expect(screen.getByText('Custom Fees Content')).toBeOnTheScreen();
      expect(screen.queryByTestId('custom-data')).toBeNull();
    });
    ...
    it('handles complex data object', () => {
      ...
      expect(screen.getByText('Custom Fees Content')).toBeOnTheScreen();
      expect(screen.getByText(JSON.stringify(complexData))).toBeOnTheScreen();
    });
  });

  describe('Callback Functions', () => {
    it('handleClose is defined and does not throw', () => {
      render(<PerpsTooltipView />);
    ...
    it('handleGotItPress calls onCloseBottomSheet', () => {
      render(<PerpsTooltipView />);

      const gotItButton = screen.getByTestId('got-it-button');

      // Pressing should not throw
      expect(() => fireEvent.press(gotItButton)).not.toThrow();
    });
  });

  describe('String Localization', () => {
    it('calls strings with correct title key', () => {
      ...
      expect(strings).toHaveBeenCalledWith('perps.tooltips.leverage.title');
    });

    it('calls strings with correct content key for default content', () => {
      const { strings } = jest.requireMock('../../../../../../locales/i18n');
      mockUseRoute.mockReturnValue({
        params: {
          contentKey: 'margin' as PerpsTooltipContentKey,
      ...
      expect(strings).toHaveBeenCalledWith('perps.tooltips.margin.content');
    });

    it('calls strings with correct button label key', () => {
      const { strings } = jest.requireMock('../../../../../../locales/i18n');
      render(<PerpsTooltipView />);

      expect(strings).toHaveBeenCalledWith('perps.tooltips.got_it_button');
    });
  });
});
