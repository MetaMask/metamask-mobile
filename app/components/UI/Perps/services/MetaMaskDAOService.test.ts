...
describe('MetaMaskDAOService', () => {
  ...
  describe('initialize', () => {
    it('should initialize successfully with valid provider', async () => {
      ...
      expect(getProviderByChainId).toHaveBeenCalledWith(
        METAMASK_DAO_CONFIG.CHAIN_ID,
      );
      expect(DevLogger.log).toHaveBeenCalledWith(
        'MetaMaskDAOService: Service initialized successfully',
      );
    });
    ...
    it('should throw error when provider is not available', async () => {
      ...
      await expect(service.initialize()).rejects.toThrow(
        'Unable to get Web3 provider for Ethereum mainnet',
      );
    });
  ...
  describe('isTokenHolder', () => {
    ...
    it('should return true for valid token holder', async () => {
      ...
      // Mock ethers BigNumber for positive balance
      const mockBalance = {
        isZero: jest.fn(() => false), // Has tokens
        toString: jest.fn(() => '1000000000000000000'), // 1 token in wei
      };
    ...
    it('should return false for non-token holder', async () => {
      const testAddress = '0x1234567890123456789012345678901234567890';
      // Mock ethers BigNumber for zero balance
      const mockBalance = {
        isZero: jest.fn(() => true), // No tokens
        toString: jest.fn(() => '0'),
      };
    ...
    it('should return false when user has zero balance (e.g., after burning tokens)', async () => {
      const testAddress = '0x1234567890123456789012345678901234567890';
      // Mock ethers BigNumber for zero balance (user may have had tokens but burned/transferred them)
      const mockBalance = {
        isZero: jest.fn(() => true), // No current tokens
        toString: jest.fn(() => '0'),
      };
    ...
    it('should use cached result when available', async () => {
      ...
      expect(mockContract.balanceOf).toHaveBeenCalledTimes(1); // Still only called once
    });
    ...
    it('should return false on error and log the error', async () => {
      ...
      mockContract.balanceOf.mockRejectedValue(mockError);
  ...
  describe('clearCache', () => {
    it('should clear the cache', () => {
      ...
      expect(DevLogger.log).toHaveBeenCalledWith(
        'MetaMaskDAOService: Cache cleared',
      );
    });
  });
