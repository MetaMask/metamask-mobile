 ...
 * `undefined` as of @metamask/design-system-react-native 0.47.0, and `Icon`
 * has no fallback for an unknown name, so referencing it crashes the render.
 *
...
export interface PerpsSentimentSatisfiedIconProps {
  /** Rendered width/height in px. Defaults to the MMDS `Md` icon size. */
  size?: number;
...
const PerpsSentimentSatisfiedIcon: React.FC<
  PerpsSentimentSatisfiedIconProps
> = ({
  size = PERPS_SENTIMENT_ICON_SIZE_MD,
  color,
  testID = 'perps-sentiment-satisfied-icon',
}) => {
  ...
  return (
    <Box testID={testID}>
      <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
        <Path d={SENTIMENT_SATISFIED_PATH} fill={fillColor} />
      </Svg>
    </Box>
  );
};
