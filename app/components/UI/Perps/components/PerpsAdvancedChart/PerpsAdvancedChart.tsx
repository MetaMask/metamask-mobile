  ...
  const side =
    positionSize !== undefined
      ? parseFloat(positionSize) >= 0
        ...
        : 'short'
      : 'long';
  const entry = parseFloat(tpslLines.entryPrice);
  if (!isFinite(entry)) return undefined;
  return {
    side,
    entryPrice: entry,
    ...(tpslLines.takeProfitPrice &&
    isFinite(parseFloat(tpslLines.takeProfitPrice))
      ? { takeProfitPrice: parseFloat(tpslLines.takeProfitPrice) }
      : {}),
    ...(tpslLines.stopLossPrice && isFinite(parseFloat(tpslLines.stopLossPrice))
      ? { stopLossPrice: parseFloat(tpslLines.stopLossPrice) }
      : {}),
    ...(tpslLines.liquidationPrice &&
    isFinite(parseFloat(tpslLines.liquidationPrice))
      ? { liquidationPrice: parseFloat(tpslLines.liquidationPrice) }
      : {}),
  };
}
