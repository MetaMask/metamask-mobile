...
const PerpsWatchOnlyBanner: React.FC<PerpsWatchOnlyBannerProps> = ({
  ...
  const handleSwitchBack = useCallback(() => {
    // A failed stop leaves the watch-only account selected, so this banner
    // stays visible and the user still sees the read-only state; log for
    // diagnosis rather than adding a second error surface to a dev tool.
    WatchOnlySession.stop().catch((error: unknown) =>
      Logger.error(
        ensureError(error, 'PerpsWatchOnlyBanner.handleSwitchBack'),
        {
          tags: {
          ...
          context: { name: 'PerpsWatchOnlyBanner', data: {} },
        },
      ),
    );
  }, []);
