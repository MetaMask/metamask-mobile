...
import { useGasIncludedQuoteParams } from '../../hooks/useGasIncludedQuoteParams';
...
// Mock useGasIncludedQuoteParams hook
jest.mock('../../hooks/useGasIncludedQuoteParams', () => ({
  useGasIncludedQuoteParams: jest.fn(),
}));
...
describe('BridgeView', () => {
  ...
  beforeEach(() => {
    ...
    (useGasIncludedQuoteParams as jest.Mock).mockReturnValue({
      gasIncluded: false,
  ...
  describe('useGasIncludedQuoteParams integration', () => {
    it('passes correct parameters to useGasIncludedQuoteParams for swap transaction', async () => {
      const testState = createBridgeTestState({
        bridgeReducerOverrides: {
          sourceToken: {
      ...
      await waitFor(() => {
        expect(useGasIncludedQuoteParams).toHaveBeenCalledWith(
          true, // isSwap
          true, // gasIncludedWithSTXSendBundleSupport
          true, // gasIncludedWith7702Support
        );
      });
    });

    it('passes correct parameters to useGasIncludedQuoteParams for bridge transaction', async () => {
      const testState = createBridgeTestState({
        bridgeReducerOverrides: {
          sourceToken: {
            address: '0x0000000000000000000000000000000000000001',
      ...
      await waitFor(() => {
        expect(useGasIncludedQuoteParams).toHaveBeenCalledWith(
          false, // isSwap (different chains)
          true, // gasIncludedWithSTXSendBundleSupport
          false, // gasIncludedWith7702Support
        );
      });
    });

    it('uses gasIncluded values from useGasIncludedQuoteParams in quote request', async () => {
      jest.useFakeTimers();
      (useGasIncludedQuoteParams as jest.Mock).mockReturnValue({
        gasIncluded: true,
    ...
    it('favors STX over 7702 when both are supported for swap', async () => {
      ...
      (useGasIncludedQuoteParams as jest.Mock).mockReturnValue({
        gasIncluded: true,
        gasIncluded7702: false, // STX takes precedence
      });
    ...
    it('disables 7702 for bridge transactions even when supported', async () => {
      ...
      (useGasIncludedQuoteParams as jest.Mock).mockReturnValue({
        gasIncluded: false,
        gasIncluded7702: false, // 7702 not supported for bridge
      });
