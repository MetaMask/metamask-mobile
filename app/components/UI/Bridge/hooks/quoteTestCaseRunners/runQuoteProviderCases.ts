...
export const mockValidateBridgeTx = jest.fn();
export const mockUseIsInsufficientBalance = jest.fn();
...
export const runQuoteProviderCases = ({
  ...
  renderWithConsumers: (state: DeepPartial<RootState>) => void;
  renderOutsideProvider: () => void;
  missingProviderError: string;
}) => {
  describe('quote provider cases', () => {
    beforeEach(() => {
      jest.clearAllMocks();
