...
const toQuoteAsset = (token: BridgeToken, name: string) => ({
  address: token.address,
  symbol: token.symbol ?? name,
  decimals: token.decimals,
  name,
  assetId: formatAddressToAssetId(token.address, token.chainId) as CaipAssetType,
  chainId: formatChainIdToDec(token.chainId),
});
...
function buildMockRecommendedQuote(
  ...
  const v1 = {
    ...
    quote: {
      ...
      srcAsset: toQuoteAsset(sourceToken, 'TOKEN1'),
      srcChainId: Number(sourceToken.chainId),
      destAsset: toQuoteAsset(destinationToken, 'TOKEN2'),
      destChainId: Number(destinationToken.chainId),
      feeData: {
        metabridge: {
          quoteBpsFee,
          amount: '0',
          asset: toQuoteAsset(usdcToken, 'TOKEN1'),
        },
      },
      ...(priceData ? { priceData } : {}),
