...
async function flushQuoteRequestDebounce(waitMs: number) {
  await act(async () => {
...
export const runBatchSellQuoteRequestCases = ({
  ...
  renderHook: (state?: DeepPartial<RootState>) => {
    result: {
      current: {
        updateBatchSellQuoteParams: (() => void) & { cancel?: () => void };
        getNewQuote: () => void;
      };
    };
  };
  ...
  describe('useBatchSellQuoteRequest', () => {
    ...
    it('returns Batch Sell quote request functions', () => {
      ...
      const { result } = renderHook(testState);
    ...
    it('returns false from hasValidBatchSellSourceAmounts when all source amounts are zero', () => {
      ...
      const isValid = hasValidBatchSellSourceAmounts(
        [ethToken, uniToken],
        sourceTokenAmounts,
        usdcToken,
      );
    ...
    it('returns no quote requests when decimal amount is below one atomic unit', () => {
    ...
    it('updates BridgeController quote request params in index order', async () => {
      ...
      await flushQuoteRequestDebounce(debounceMs);
