...
const KycPending = () => {
  ...
  // `null` means `GET /kyc/status` has not been read back yet, which is not the
  // same as a decided status: bouncing on it would close the screen during the
  // tick between the controller refreshing and Redux receiving the new state.
  const hasDecidedOtherStatus = userStatus !== null && userStatus !== 'pending';
  ...
  useEffect(() => {
    if (hasDecidedOtherStatus) {
      navigateToHome();
    }
  }, [hasDecidedOtherStatus, navigateToHome]);
