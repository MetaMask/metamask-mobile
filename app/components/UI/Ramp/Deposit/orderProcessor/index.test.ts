...
describe('processDepositOrder', () => {
  ...
  afterEach(() => {
    ...
    (DepositSDKOrders.getOrder as jest.Mock).mockClear();
  });

  it('should process an order', async () => {
    (DepositSDKOrders.getOrder as jest.Mock).mockImplementation(() => ({
      ...mockOrder.data,
      status: 'FAILED',
    }));
  ...
  it('should return the same order if response is empty', async () => {
    (DepositSDKOrders.getOrder as jest.Mock).mockImplementation(() => null);
    jest.spyOn(Date, 'now').mockImplementation(() => 1673886669600);

    const updatedOrder = await processDepositOrder(mockOrder);
    expect(DepositSDKOrders.getOrder).toHaveBeenCalledWith('test-id', '0x1234');
    expect(DepositSDKOrders.getOrder).toHaveReturnedWith(null);
