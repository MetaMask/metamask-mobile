...
- **E3 - Pre-empt the existing zero-delay focus-dismissal timer.** On re-focus, `useHeadlessSessionFocusDismissal` schedules dismissal via `setTimeout(..., 0)` (`useHeadlessSessionFocusDismissal.ts:51`) and bails if the session is gone or the focus / session id changed. Ensure a real deeplink-success terminates the session before, or takes precedence over, focus-dismissal; it does not emit `onError`, and `cancel()` plus stale-session GC remain backstops. **Decision (recorded, implemented):** no grace delay is added and the zero-delay timer is unchanged. Precedence is achieved by retention instead: the external-return correlation (`externalBrowserReturn.ts`, one record per session keyed by session id) retains `onOrderCreated` and is NOT cleared by `user_dismissed` teardown, so a success deeplink arriving after any dismissal still completes the order (E2 machinery covers the E3 race). Return deeplinks are matched to records by ORDER ID (the GUID minted at widget creation — the same key the non-headless flow's precreated stubs and provider redirects already use); an orderless record only matches while its session is live, and an unmatched deeplink takes the legacy path, so a foreign (UB1/UB2) return can never be hijacked. Double-completion is prevented by an in-flight guard keyed on session id, and a session that ends in `onError` drops its record so a late `onOrderCreated` is impossible.
