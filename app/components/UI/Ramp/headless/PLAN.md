...
- Register screen in [app/components/Nav/Main/MainNavigator.tsx](../../../../Nav/Main/MainNavigator.tsx) next to the `RampSettings` registration (lines 491–500), no env gate on the stack itself — only the entry point is gated.
...
- ~~**Flatten the `startHeadlessBuy` nested-screen descriptor.**~~ **DONE.** Root cause was that the intermediate `RootStack` slot in `routes.tsx` (the wrapper that hosts `MainRoutes`) shared the same route name `RampTokenSelection` as its child leaf screen, producing the duplicated-name nesting under `RampHeadlessEntry`. Fix: introduced `Routes.RAMP.TOKEN_SELECTION_ROOT = 'RampTokenSelectionRoot'` and renamed only the intermediate `RootStack.Navigator` slot. The leaf `RampTokenSelection` screen, the outer `MainNavigator` mount, and the public-facing route name are unchanged. All call sites that traversed the doubled `{ screen: TOKEN_SELECTION, params: { screen: TOKEN_SELECTION, ... } }` shape (`useHeadlessBuy.ts`, `createBuildQuoteNavDetails`, `TokenNotAvailableModal`, `ProviderSelectionModal`) now target `TOKEN_SELECTION_ROOT` at the intermediate level. The React Navigation "Found screens with the same name nested inside one another" warning under `RampHeadlessEntry` no longer triggers.
