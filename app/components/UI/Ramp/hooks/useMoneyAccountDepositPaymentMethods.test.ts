...
describe('useMoneyAccountDepositPaymentMethods', () => {
  ...
  describe('isSettled semantics', () => {
    it('settles (without becoming ready) when no provider resolves for the asset', async () => {
      const store = createMockStore();
      const { Wrapper } = createWrapper(store);
    ...
    it('settles when the region currency is missing so the methods query cannot run', async () => {
      const store = createMockStore({
        country: null,
        state: null,
        regionCode: 'de',
      ...
      expect(result.current.isReady).toBe(false);
      // No fiat currency -> payment-methods query is disabled, never fetched.
      expect(mockGetPaymentMethods).not.toHaveBeenCalled();
    });

    it('settles (and is ready) on a successful resolution', async () => {
      const store = createMockStore();
      const { Wrapper } = createWrapper(store);
    ...
    it('is not settled while the provider query is still in flight', () => {
      const store = createMockStore();
      const { Wrapper } = createWrapper(store);
