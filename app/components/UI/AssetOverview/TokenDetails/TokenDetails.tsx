  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Use an if/else statement (not a ternary) so the React Compiler can
        // lower this block within the try statement.
        ...
        >;
        // Guard with an if statement instead of optional chaining so the value
        // block stays compiler-lowerable inside the try.
        let marketEntry: Record<string, unknown> | undefined;
        if (response) {
          marketEntry = response[assetId];
        }
        setFetchedMarketData(marketEntry);
      } catch (error) {
