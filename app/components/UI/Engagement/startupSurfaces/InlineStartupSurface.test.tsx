...
describe('InlineStartupSurface', () => {
  ...
  it('does not render PushNotificationOnboarding when push-pre-prompt is ineligible', () => {
    const store = makeStore();
    store.dispatch(
  ...
  it('does not render PushNotificationOnboarding when a navigation-backed surface is active', () => {
    const store = makeStore();
    store.dispatch(
      surfaceStatusReported({ id: 'push-pre-prompt', status: 'ineligible' }),
    );
