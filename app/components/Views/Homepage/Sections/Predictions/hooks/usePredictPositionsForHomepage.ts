 ...
 *
 * Fetches all positions (active + claimable) in a single API call — no
 * `redeemable` filter — so that won positions are discovered regardless of
 * on-chain timing. Callers split the result client-side via `p.claimable`.
 *
