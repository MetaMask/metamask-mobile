...
describe('PerpsSection', () => {
  ...
  it('shows error state when preload rejects and no positions are cached', async () => {
    const EngineMock = jest.requireMock('../../../../../core/Engine');
    EngineMock.context.PerpsController.startMarketDataPreload.mockReturnValue(
    ...
    expect(
      await screen.findByText('Unable to load perpetuals'),
    ).toBeOnTheScreen();
  ...
  it('shows error state when preload throws synchronously', () => {
    const EngineMock = jest.requireMock('../../../../../core/Engine');
    EngineMock.context.PerpsController.startMarketDataPreload.mockImplementation(
      () => {
  ...
  it('retries preload when retry button is pressed', async () => {
    const EngineMock = jest.requireMock('../../../../../core/Engine');
    EngineMock.context.PerpsController.startMarketDataPreload.mockReturnValue(
      Promise.reject(new Error('network error')),
    );
  ...
  it('does not show error when positions exist despite preload failure', async () => {
    const EngineMock = jest.requireMock('../../../../../core/Engine');
    EngineMock.context.PerpsController.startMarketDataPreload.mockReturnValue(
      Promise.reject(new Error('network error')),
    );
    ...
    await screen.findByText('Short BTC');

    expect(
      screen.queryByText('Unable to load perpetuals'),
    ).not.toBeOnTheScreen();
    expect(screen.getByTestId('homepage-perps-positions')).toBeOnTheScreen();
  });
});
