 ...
 */
export interface UsePerpsMarketsStandaloneResult {
...
/**
 * usePerpsMarketsStandalone Hook
 *
 * Fetches perps markets in lightweight standalone mode.
 ...
 * @example
 * ```tsx
 * const { markets, isLoading, error, refresh } = usePerpsMarketsStandalone(5);
 * ```
 */
export const usePerpsMarketsStandalone = (
  limit = 5,
): UsePerpsMarketsStandaloneResult => {
  ...
  const fetchMarkets = useCallback(async () => {
    ...
    try {
      ...
      if (!controller || typeof controller.getMarkets !== 'function') {
...
export default usePerpsMarketsStandalone;
