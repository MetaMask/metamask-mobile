...
describe('Transaction Pay Utils', () => {
  ...
  describe('getAvailableTokens', () => {
    ...
    describe('with allowedPaymentTokens', () => {
      const MOCK_USDC_ADDRESS = '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48';
      const MOCK_USDT_ADDRESS = '0xdac17f958d2ee523a2206206994597c13d831ec7';
      const MOCK_DAI_ADDRESS = '0x6b175474e89094c44da98b954eedeac495271d0f';
      ...
      const mockUsdcToken = {
        ...
        balance: '1000000',
        balanceInSelectedCurrency: '$1000',
      ...
      const mockDaiToken = {
        ...
        balance: '2000000',
        balanceInSelectedCurrency: '$2000',
        chainId: MAINNET_CHAIN_ID,
        decimals: 18,
        name: 'Dai Stablecoin',
        standard: TokenStandard.ERC20,
      ...
      it('returns only tokens in the allowlist', () => {
        const allowedPaymentTokens = {
          [MAINNET_CHAIN_ID]: [MOCK_USDC_ADDRESS, MOCK_USDT_ADDRESS],
        };

        const result = getAvailableTokens({
          tokens: [mockUsdcToken, mockUsdtToken, mockDaiToken],
          allowedPaymentTokens,
        });

        expect(result).toHaveLength(2);
        expect(result[0].address.toLowerCase()).toBe(
          MOCK_USDC_ADDRESS.toLowerCase(),
        );
        expect(result[0].symbol).toBe('USDC');
        expect(result[1].address.toLowerCase()).toBe(
          MOCK_USDT_ADDRESS.toLowerCase(),
        );
        expect(result[1].symbol).toBe('USDT');
      });

      it('includes selected payToken if in the allowlist', () => {
        const allowedPaymentTokens = {
          [MAINNET_CHAIN_ID]: [MOCK_USDC_ADDRESS],
        };

        const payToken = {
          address: MOCK_USDC_ADDRESS as Hex,
          chainId: MAINNET_CHAIN_ID as Hex,
        } as TransactionPaymentToken;

        const result = getAvailableTokens({
          tokens: [mockUsdcToken, mockUsdtToken],
          payToken,
          allowedPaymentTokens,
        });
      ...
      it('excludes selected payToken if not in the allowlist', () => {
        const allowedPaymentTokens = {
          [MAINNET_CHAIN_ID]: [MOCK_USDC_ADDRESS],
        };

        const payToken = {
          address: MOCK_USDT_ADDRESS as Hex,
          chainId: MAINNET_CHAIN_ID as Hex,
        } as TransactionPaymentToken;
      ...
      it('includes required tokens if in the allowlist', () => {
        const allowedPaymentTokens = {
          [MAINNET_CHAIN_ID]: [MOCK_USDC_ADDRESS, MOCK_USDT_ADDRESS],
        };
        ...
        const mockUsdtWithZeroBalance = {
          ...mockUsdtToken,
          balance: '0',
        } as AssetType;

        const result = getAvailableTokens({
          tokens: [mockUsdcToken, mockUsdtWithZeroBalance],
          requiredTokens,
          allowedPaymentTokens,
        });

        expect(result).toHaveLength(2);
        expect(result.find((t) => t.symbol === 'USDC')).toBeDefined();
        expect(result.find((t) => t.symbol === 'USDT')).toBeDefined();
      });

      it('excludes required tokens if not in the allowlist', () => {
        const allowedPaymentTokens = {
          [MAINNET_CHAIN_ID]: [MOCK_USDC_ADDRESS],
        };
      ...
      it('returns empty array when chain has no allowed tokens', () => {
        const allowedPaymentTokens = {
          [ARBITRUM_CHAIN_ID]: [MOCK_USDC_ADDRESS],
        };
      ...
      it('performs case-insensitive address matching for allowlist', () => {
        const allowedPaymentTokens = {
          [MAINNET_CHAIN_ID]: [MOCK_USDC_ADDRESS.toUpperCase()],
        };
