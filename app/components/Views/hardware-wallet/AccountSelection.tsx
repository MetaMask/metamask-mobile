...
const AccountSelection = () => {
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      const fetchedAccounts = await getLedgerAccountsByOperation(
  ...
  }, [trackEvent, createEventBuilder, accounts, deviceModelName, walletType]);
  ...
  const updateNewLegacyAccountsLabel = useCallback(async () => {
    if (LEDGER_LEGACY_PATH === (await getHDPath())) {
      ...
      const newAddedAccounts = ledgerAccounts.filter(
        (address) => !existingAccounts.includes(toFormattedAddress(address)),
      );
      ...
      <SafeAreaView style={tw.style('flex-1 bg-default')}>
        <Box twClassName="flex-1 items-center justify-center px-6">
          {errorMsg ? (
            ...
            <>
              <ActivityIndicator size="large" color="#0376C9" />
              <Text
