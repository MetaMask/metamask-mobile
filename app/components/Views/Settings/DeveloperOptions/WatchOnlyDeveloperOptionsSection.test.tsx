...
describe('WatchOnlyDeveloperOptionsSection', () => {
  ...
  it('renders the default description when no account is watched', () => {
    ...
    expect(
      getByText(
        'Browse any EVM address read-only. The account has no keys, so every signature is rejected. Stop removes it and restores your previous account.',
      ),
    ).toBeOnTheScreen();
  });
  ...
  it('renders the watched address when an account is watched', () => {
    ...
    expect(getByText(`Watching ${WATCHED_ADDRESS}`)).toBeOnTheScreen();
  });
