...
describe('AccountsMenu', () => {
  ...
  describe('RESOURCES Section', () => {
    describe('About MetaMask Row', () => {
      it('render About MetaMask row', () => {
        ...
        // babel-jest leaves `///: ONLY_INCLUDE_IF` fences as plain comments, so
        // the remaining flask branch always assigns the title under Jest.
  ...
  describe('Analytics Tracking', () => {
    ...
    // Support routes through the consent flow, which defers tracking to the
    // callback passed as openSupportWithConsent's 3rd arg rather than firing it
    // on press. See 'Support Row' for the opener/tracking-callback coverage.
    it('track NAVIGATION_TAPS_GET_HELP event when support actually opens', () => {
      const { getByTestId } = render(<AccountsMenu />);
      const supportButton = getByTestId(AccountsMenuSelectorsIDs.SUPPORT);
