...
function renderOptinMetricsForImport(opts: { optedIn?: boolean } = {}) {
...
describeForPlatforms('import-wallet analytics (CV)', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('OptinMetrics dispatches "Analytics Preference Selected" with is_metrics_opted_in=true when user confirms with metrics enabled', async () => {
    ...
    try {
      const { getByTestId } = renderOptinMetricsForImport({ optedIn: false });
