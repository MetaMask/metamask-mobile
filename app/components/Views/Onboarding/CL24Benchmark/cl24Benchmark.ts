...
interface TurnBasedWaiter {
  partyId: string;
  resolve: (message: Uint8Array) => void;
}

export interface PartyOperation<T> {
  id: string;
  run: () => Promise<T>;
}
...
export interface BenchmarkMessageRelay {
  push(
    sessionId: string,
    sender: string,
    receiver: string,
...
async function runTurnBasedStage<T>(
  iteration: number,
  stage: string,
  partyIds: string[],
  runParty: (partyId: string, session: RootNetworkSession) => Promise<T>,
): Promise<{ duration: number; values: T[] }> {
