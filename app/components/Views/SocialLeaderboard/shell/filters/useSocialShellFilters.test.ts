...
describe('useSocialShellFilters', () => {
  it('starts with every tab at defaults and no sheet open', () => {
    ...
    expect(result.current.applied.forYou).toEqual(DEFAULT_FILTERS);
    ...
    expect(result.current.hasActiveFilters('forYou')).toBe(false);
  ...
  it('opening a sheet seeds the draft from that tab applied state', () => {
    ...
    act(() => {
      result.current.openSheet('forYou');
    });

    expect(result.current.openTab).toBe('forYou');
  ...
  it('getApiParams forwards the full chain set for all networks', () => {
    ...
    expect(result.current.getApiParams('forYou').chains).toEqual(ALL_CHAINS);
  });
  ...
  it('getApiParams narrows chains to the selected network when not all', () => {
    ...
    expect(result.current.getApiParams('forYou').chains).toEqual(['solana']);
  });
