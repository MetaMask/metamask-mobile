...
export function useSamplePetNames(chainId: SupportedCaipChainId | Hex) {
  ...
  useEffect(() => {
    ...
    const updatePetNames = () => {
      const petNamesByAddress = controller.state.namesByChainIdAndAddress[chainId as Hex] || {};
      const petNamesArray = Object.entries(petNamesByAddress).map(([address, name]) => ({
        address,
        name,
      }));
    ...
    return () => {
      Engine.controllerMessenger.unsubscribe('SamplePetnamesController:stateChange', updatePetNames);
    };
  }, [chainId]);
