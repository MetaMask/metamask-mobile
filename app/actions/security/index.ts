...
export type AllowLoginWithRememberMeUpdated = ReduxAction<ActionType.SET_ALLOW_LOGIN_WITH_REMEMBER_ME> & {
  enabled: boolean;
};

export type SetNftAutoDetectionModalOpen = ReduxAction<ActionType.SET_NFT_AUTO_DETECTION_MODAL_OPEN> & {
  open: boolean;
};

export type SetMultiRpcMigrationModalOpen = ReduxAction<ActionType.SET_MULTI_RPC_MIGRATION_MODAL_OPEN> & {
  open: boolean;
};

export type SetDataCollectionForMarketing = ReduxAction<ActionType.SET_DATA_COLLECTION_FOR_MARKETING> & {
  enabled: boolean;
};
