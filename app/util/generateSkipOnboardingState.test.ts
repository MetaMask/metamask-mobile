...
describe('generateSkipOnboardingState', () => {
  ...
  describe('applyVaultInitialization - host resolution and SRP profile selection', () => {
    ...
    describe('for loop - host iteration and SRP profile resolution', () => {
      ...
      it('should throw when command queue server is unreachable on all hosts', async () => {
        // Arrange
        Platform.OS = 'android';
        jest
          .spyOn(global, 'fetch')
        ...
        // Act & Assert
        await expect(applyVaultInitialization()).rejects.toThrow(
          'Command queue server did not provide an SRP profile',
        );
      });
