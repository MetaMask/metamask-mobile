...
describe('mapPerpsTransaction', () => {
  describe('trades', () => {
    it('maps Opened long → perpsOpenLong with out-direction token', () => {
      const result = mapPerpsTransaction({
        transaction: fillTx('Opened long', FillType.Standard, 43.99),
        chainId: ARBITRUM,
      });
    ...
    it('maps Opened short → perpsOpenShort with out-direction token', () => {
      const result = mapPerpsTransaction({
        transaction: fillTx('Opened short', FillType.Standard),
        chainId: ARBITRUM,
      });
      expect(result?.type).toBe('perpsOpenShort');
      expect(result?.data.token?.direction).toBe('out');
    });
    ...
    it.each([
      ...
      expect(result?.data.token?.direction).toBe('in');
    });
