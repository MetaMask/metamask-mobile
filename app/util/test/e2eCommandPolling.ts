...
import { hasOverrides, getCommandQueueServerPortInApp } from './utils';
...
function scheduleNext(delay: number): void {
  if (!hasOverrides || pollingDisabled) return;
 ...
 * Should only be called once, guarded by hasOverrides.
 */
export async function startE2ECommandPolling(): Promise<void> {
  if (!hasOverrides || hasStartedPolling) return;
