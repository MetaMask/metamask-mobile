...
export type NavigationDetails<T extends NavigationParams = NavigationParams> = readonly [string, T];
...
export const useParams = <T extends object | undefined, Strict extends boolean = false>(defaults?: Partial<T>) => {
	const route = useRoute<RouteParams<{ params: T }>>();
	const navParams = route.params;
	const params = useMemo(() => ({ ...defaults, ...navParams }), [defaults, navParams]);
