...
import { selectAccountLabelMap } from '../../selectors/keyringController';
...
export function getLabelTextByAddress(address: string) {
  ...
  const accountLabelMap = selectAccountLabelMap(store.getState());
  return accountLabelMap.get(address) ?? null;
}
