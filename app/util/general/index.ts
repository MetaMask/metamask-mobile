...
export const tlc = (input: unknown): string | undefined => {
  if (input == null) {
    return undefined;
  }
 ...
 */
export function timeoutFetch(url: string, options?: RequestInit, timeout = 500): Promise<Response | Error> {
  return Promise.race([
    fetch(url, options),
...
export const toLowerCaseEquals = (input1: unknown, input2: unknown): boolean => {
  if (input1 === input2) return true;
  if (input1 == null || input2 == null) return false;

  return tlc(input1) === tlc(input2);
};
 ...
 */
export const deepJSONParse = ({ jsonString, skipNumbers = true }: { jsonString: string; skipNumbers?: boolean }): JSONValue => {
  ...
  function parseProperties(obj: JSONValue): void {
        ...
        if (typeof value === 'string') {
          ...
          if (!isNumber || (isNumber && !skipNumbers)) {
            try {
              ...
              (obj as Record<string, JSONValue>)[key] = parsed;
