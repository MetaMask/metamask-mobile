...
export const resolveAgenticCliPreferenceForDelivery =
  async (): Promise<AgenticCliPreference> => {
    ...
    try {
      ...
      // Client-only `agenticCli` toggles are persisted locally; prefer that for gating.
      if (localPreference) {
      ...
      return resolveAgenticCliPreference(apiPreferences);
    } catch {
      return localPreference ?? DEFAULT_AGENTIC_CLI_PREFERENCE;
    }
  };

const getNotificationCreatedAtMs = (
  ...
  if (!Number.isNaN(numericCreatedAt)) {
...
export const shouldHideNewAgenticCliInAppNotification = (
  ...
  // No disable timestamp (e.g. toggled off before gating shipped): hide all agentic.
  if (inAppDisabledAt === null) {
    return true;
  }

  const createdAtMs = getNotificationCreatedAtMs(notification.createdAt);
  if (createdAtMs === null) {
    return true;
  }

  return createdAtMs >= inAppDisabledAt;
};
