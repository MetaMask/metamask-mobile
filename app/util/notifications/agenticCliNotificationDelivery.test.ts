...
describe('agenticCliNotificationDelivery', () => {
  ...
  it('hides new agentic notifications when createdAt is an ISO timestamp', () => {
    const disabledAt = Date.parse('2026-06-10T03:30:00.000Z');
    const newNotification = {
      id: '2',
      type: TRIGGER_TYPES.PLATFORM,
  ...
  it('hides all agentic notifications when in-app is off and no disable timestamp exists', () => {
    const notification = {
      id: '1',
      type: TRIGGER_TYPES.PLATFORM,
      createdAt: '2026-06-10T03:36:00.000Z',
  ...
  it('prefers locally persisted agenticCli preferences over API values for push gating', async () => {
    jest.mocked(Engine.controllerMessenger.call).mockResolvedValue({
      ...basePreferences,
      agenticCli: {
        pushNotificationsEnabled: true,
