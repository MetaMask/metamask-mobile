...
async function applyVaultInitialization() {
  ...
  if (!srpProfile) {
    throw new Error(
      '[E2E - generateSkipOnboardingState] Command queue server did not provide an SRP profile. ' +
        'Ensure CommandQueueServer is running and BrowserStack Local is enabled.',
    );
  }
