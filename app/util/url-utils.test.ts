...
describe('isLocalhostOrIPAddress', () => {
  ...
  it('returns false for malformed IPv4-looking strings', () => {
    expect(isLocalhostOrIPAddress('999.999.999.999')).toBe(false);
    expect(isLocalhostOrIPAddress('1.2.3')).toBe(false);
  });
});
