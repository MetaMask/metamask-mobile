...
export const formatCountdown = (seconds: number): string => {
  const normalizeTimeField = (value: number): string =>
    value >= 10 ? value.toString() : '0' + value.toString();
  if (seconds < 60) {
    return `00:${normalizeTimeField(seconds)}`;
  }
  const s = seconds % 60;
  const m = (seconds - s) / 60;
  return `${normalizeTimeField(m)}:${normalizeTimeField(s)}`;
};
...
export const convertToFixedPercentage = (value?: number | null) => {
