...
function buildMarketDataPayload(markets, activeProvider, isTestnet, now) {
    if (activeProvider === 'aggregated') {
        ...
        for (const market of markets) {
            const providerId = market.providerId ?? perpsConfig_js_1.PROVIDER_CONFIG.DefaultProvider;
            const key = (0, perpsConfig_js_1.buildProviderCacheKey)(providerId, isTestnet);
    ...
    return {
        providerNetworkKey: (0, perpsConfig_js_1.buildProviderCacheKey)(activeProvider ?? perpsConfig_js_1.PROVIDER_CONFIG.DefaultProvider, isTestnet),
        data: markets,
        timestamp: now,
    };
}
...
function buildUserDataPayload(positions, orders, accountState, address, activeProvider, isTestnet, now) {
    if (activeProvider === 'aggregated') {
        ...
        for (const position of positions) {
            ensureEntry(position.providerId ?? perpsConfig_js_1.PROVIDER_CONFIG.DefaultProvider).positions.push(position);
        }
        for (const order of orders) {
            ensureEntry(order.providerId ?? perpsConfig_js_1.PROVIDER_CONFIG.DefaultProvider).orders.push(order);
        }
        if (accountState) {
            ensureEntry(accountState.providerId ?? perpsConfig_js_1.PROVIDER_CONFIG.DefaultProvider).accountState = accountState;
        }
...
function persistMarketEntriesToDisk(diskCache, entries) {
    ...
    diskCache
        .setItem(perpsConfig_js_1.PERPS_DISK_CACHE_MARKETS, JSON.stringify(payload))
        .catch(() => {
 ...
 */
async function persistUserEntriesToDisk(diskCache, entries) {
    if (entries.length === 0) {
        return;
    }
    const payload = entries.length === 1 ? entries[0] : { entries };
    await diskCache.setItem(perpsConfig_js_1.PERPS_DISK_CACHE_USER_DATA, JSON.stringify(payload));
}
...
function hydrateFromDiskSync(diskCache, currentMarketCache, currentUserCache, staleGuardMs) {
    ...
    try {
        const marketsRaw = diskCache.getItemSync(perpsConfig_js_1.PERPS_DISK_CACHE_MARKETS);
        const userRaw = diskCache.getItemSync(perpsConfig_js_1.PERPS_DISK_CACHE_USER_DATA);
        if (marketsRaw) {
            try {
                    ...
                    if (entry.providerNetworkKey && Array.isArray(entry.data)) {
                        ...
                        if (!existing || existing.timestamp < entry.timestamp) {
                            const strippedData = entry.data.map((market) => {
                                ...
                                return {
                                    ...structuralMarket,
                                    price: perpsConfig_js_1.PERPS_CONSTANTS.FallbackPriceDisplay,
                                    change24h: perpsConfig_js_1.PERPS_CONSTANTS.FallbackDataDisplay,
                                    change24hPercent: perpsConfig_js_1.PERPS_CONSTANTS.FallbackPercentageDisplay,
                                };
                            });
