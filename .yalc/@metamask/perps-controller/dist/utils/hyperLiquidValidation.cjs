...
function createErrorResult(error, defaultResponse) {
    return {
        ...defaultResponse,
        success: false,
        error: error instanceof Error ? error.message : perpsErrorCodes_js_1.PERPS_ERROR_CODES.UNKNOWN_ERROR,
    };
}
...
function validateWithdrawalParams(params, debugLogger) {
    ...
    if (isNaN(amount) || amount <= 0) {
        debugLogger?.log('validateWithdrawalParams: Invalid amount', {
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.WITHDRAW_AMOUNT_POSITIVE,
    ...
    if (params.destination && !(0, utils_1.isValidHexAddress)(params.destination)) {
        debugLogger?.log('validateWithdrawalParams: Invalid destination address', {
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.WITHDRAW_INVALID_DESTINATION,
...
function validateDepositParams(params, debugLogger) {
    ...
    if (!params.assetId) {
        debugLogger?.log('validateDepositParams: Missing assetId', {
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.DEPOSIT_ASSET_ID_REQUIRED,
            params,
        });
        return {
            isValid: false,
    ...
    if (!params.amount) {
        debugLogger?.log('validateDepositParams: Missing amount', {
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.DEPOSIT_AMOUNT_REQUIRED,
            params,
        });
        return {
            isValid: false,
    ...
    if (isNaN(amount) || amount <= 0) {
        debugLogger?.log('validateDepositParams: Invalid amount', {
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.DEPOSIT_AMOUNT_POSITIVE,
            amount: params.amount,
            parsedAmount: amount,
            isNaN: isNaN(amount),
        });
    ...
    if (amount < minimumAmount) {
        debugLogger?.log('validateDepositParams: Below minimum deposit', {
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.DEPOSIT_MINIMUM_AMOUNT,
            amount,
            minimumAmount,
    ...
    if (!isSupported) {
        ...
        if (!isSupportedCaseInsensitive) {
            debugLogger?.log('validateAssetSupport: Asset not supported', {
                error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.WITHDRAW_ASSET_NOT_SUPPORTED,
                assetId,
                supportedAssetIds,
...
function validateBalance(withdrawAmount, withdrawableBalance, debugLogger) {
    ...
    if (withdrawAmount > withdrawableBalance) {
        ...
        debugLogger?.log('validateBalance: Insufficient balance', {
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.WITHDRAW_INSUFFICIENT_BALANCE,
            withdrawAmount,
            withdrawableBalance,
            shortfall,
...
function applyPathFilters(assets, params, debugLogger) {
    ...
    if (params.symbol && params.symbol in hyperLiquidConfig_js_1.HYPERLIQUID_ASSET_CONFIGS) {
        const config = hyperLiquidConfig_js_1.HYPERLIQUID_ASSET_CONFIGS[params.symbol];
...
function getSupportedPaths(params, debugLogger) {
    const isTestnet = params?.isTestnet ?? false;
    const assets = (0, hyperLiquidConfig_js_1.getSupportedAssets)(isTestnet);
...
function getMaxOrderValue(maxLeverage, orderType) {
    let marketLimit;
    if (maxLeverage >= 25) {
        marketLimit = perpsConfig_js_1.HYPERLIQUID_ORDER_LIMITS.MarketOrderLimits.HighLeverage;
    }
    else if (maxLeverage >= 20) {
        marketLimit = perpsConfig_js_1.HYPERLIQUID_ORDER_LIMITS.MarketOrderLimits.MediumHighLeverage;
    }
    else if (maxLeverage >= 10) {
        marketLimit = perpsConfig_js_1.HYPERLIQUID_ORDER_LIMITS.MarketOrderLimits.MediumLeverage;
    }
    else {
        marketLimit = perpsConfig_js_1.HYPERLIQUID_ORDER_LIMITS.MarketOrderLimits.LowLeverage;
    }
    ...
    return (0, orderTypes_js_1.getTriggerExecution)(orderType) === 'limit'
        ? marketLimit * perpsConfig_js_1.HYPERLIQUID_ORDER_LIMITS.LimitOrderMultiplier
        : marketLimit;
}
...
function validateTwapParams(params) {
    if (params.twapDuration === undefined) {
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_TWAP_DURATION_REQUIRED,
        };
    }
    ...
    if (!Number.isInteger(params.twapDuration) ||
        params.twapDuration < perpsConfig_js_1.HYPERLIQUID_TWAP_LIMITS.MinDurationMinutes ||
        params.twapDuration > perpsConfig_js_1.HYPERLIQUID_TWAP_LIMITS.MaxDurationMinutes) {
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_TWAP_DURATION_INVALID,
        };
    }
    return { isValid: true };
}
...
function validateScaleParams(params) {
    if (params.scaleMinPrice === undefined ||
        ...
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_SCALE_RANGE_REQUIRED,
        };
    }
    ...
    if (!Number.isFinite(minPrice) ||
        ...
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_SCALE_RANGE_INVALID,
        };
    }
    if (params.scaleNumOrders === undefined ||
        !Number.isInteger(params.scaleNumOrders) ||
        params.scaleNumOrders < orderTypes_js_1.SCALE_ORDER_COUNT.min ||
        params.scaleNumOrders > orderTypes_js_1.SCALE_ORDER_COUNT.max) {
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_SCALE_COUNT_INVALID,
        };
    }
    return { isValid: true };
}
...
function validateChaseParams(params) {
    const interval = params.chaseIntervalMs ?? perpsConfig_js_1.CHASE_ORDER_CONFIG.DefaultIntervalMs;
    if (!Number.isFinite(interval) ||
        interval < perpsConfig_js_1.CHASE_ORDER_CONFIG.MinIntervalMs) {
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_CHASE_INTERVAL_INVALID,
        };
    }
    const maxDuration = params.chaseMaxDurationMs ?? perpsConfig_js_1.CHASE_ORDER_CONFIG.DefaultMaxDurationMs;
    ...
    if (!Number.isFinite(maxDuration) || maxDuration < interval) {
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_CHASE_DURATION_INVALID,
        };
    }
    const maxRepricings = params.chaseMaxRepricings ?? perpsConfig_js_1.CHASE_ORDER_CONFIG.DefaultMaxRepricings;
...
function validateStrategyOrderParams(params) {
    ...
    const strategy = orderType !== undefined && (0, orderTypes_js_1.isStrategyOrderType)(orderType)
        ? orderType
        : undefined;
    ...
    if (foreignField) {
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_STRATEGY_PARAMS_NOT_SUPPORTED,
        };
    }
    if (!strategy) {
        return { isValid: true };
    ...
    if (params.price !== undefined ||
        ...
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_STRATEGY_FIELD_UNSUPPORTED,
        };
    }
    if (strategy === 'twap') {
...
function validateCoinExists(coin, coinToAssetId) {
    if (!coinToAssetId.has(coin)) {
        return {
            isValid: false,
            error: perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_UNKNOWN_COIN,
        };
    }
    return { isValid: true };
}
