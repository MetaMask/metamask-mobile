...
exports.buildOrdersArray = exports.formatPartialTpslSize = exports.validateOrderPrecision = exports.calculateOrderPriceAndSize = exports.calculateFinalPositionSize = exports.splitScaleSizes = exports.computeScalePriceLadder = exports.computeChaseQuotePrice = exports.getPriceTick = exports.floorToSizeDecimals = exports.getMaxAllowedAmount = exports.calculateMarginRequired = exports.calculatePositionSize = void 0;
...
function getMaxAllowedAmount(params) {
    ...
    const bufferedMax = maxAmount * (1 - perpsConfig_js_1.MAX_ORDER_MARGIN_BUFFER);
 ...
 */
function getPriceTick(params) {
    const { price, szDecimals } = params;
    const byDecimals = Math.pow(10, -(perpsConfig_js_1.DECIMAL_PRECISION_CONFIG.MaxPriceDecimals - szDecimals));
    const bySignificantFigures = Math.pow(10, Math.floor(Math.log10(Math.abs(price))) -
        (perpsConfig_js_1.DECIMAL_PRECISION_CONFIG.MaxSignificantFigures - 1));
    return Math.max(byDecimals, bySignificantFigures);
}
exports.getPriceTick = getPriceTick;
/**
 ...
 */
function computeChaseQuotePrice(params) {
    ...
    return (0, hyperLiquidAdapter_js_1.formatHyperLiquidPrice)({
        price: crosses ? reference : improved,
        szDecimals,
    });
}
exports.computeChaseQuotePrice = computeChaseQuotePrice;
/**
 ...
 */
function computeScalePriceLadder(params) {
    const { minPrice, maxPrice, count } = params;
    if (!Number.isInteger(count) ||
        count < orderTypes_js_1.SCALE_ORDER_COUNT.min ||
        count > orderTypes_js_1.SCALE_ORDER_COUNT.max) {
        throw new Error(perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_SCALE_COUNT_INVALID);
    }
...
exports.computeScalePriceLadder = computeScalePriceLadder;
/**
 ...
 */
function splitScaleSizes(params) {
    ...
    if (!Number.isSafeInteger(totalUnits) || totalUnits < count) {
        throw new Error(perpsErrorCodes_js_1.PERPS_ERROR_CODES.ORDER_SCALE_SIZE_TOO_SMALL);
    }
    const sliceUnits = Math.floor(totalUnits / count);
    const remainderUnits = totalUnits - sliceUnits * count;
    return Array.from({ length: count }, (_unused, index) => (0, hyperLiquidAdapter_js_1.formatHyperLiquidSize)({
        size: (index === 0 ? sliceUnits + remainderUnits : sliceUnits) / multiplier,
        szDecimals,
    }));
}
exports.splitScaleSizes = splitScaleSizes;
/**
...
function calculateOrderPriceAndSize(params) {
    ...
    if ((0, orderTypes_js_1.isTriggerOrderType)(orderType)) {
        ...
        const effectiveBps = maxSlippageBps ?? perpsConfig_js_1.ORDER_SLIPPAGE_CONFIG.DefaultMarketSlippageBps;
        const slippageValue = effectiveBps / hyperLiquidConfig_js_1.BASIS_POINTS_DIVISOR;
        orderPrice = isBuy
...
function buildOrdersArray(params) {
    ...
    if (stopLossPrice) {
        ...
        const slippageValue = perpsConfig_js_1.ORDER_SLIPPAGE_CONFIG.DefaultTpslSlippageBps / 10000;
