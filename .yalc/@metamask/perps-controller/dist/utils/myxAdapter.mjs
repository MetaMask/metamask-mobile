 ...
 */
import { fromMYXPrice, fromMYXSize, fromMYXCollateral, MYX_MAX_LEVERAGE, MYX_MINIMUM_ORDER_SIZE_USD } from "../constants/myxConfig.mjs";
import { MYX_HL_OVERLAPPING_MARKETS, MYXDirection, MYXDirectionEnum, MYXOperationEnum, MYXOrderStatusEnum, MYXOrderTypeEnum, MYXExecTypeEnum, MYXTradeFlowTypeEnum } from "../types/myx-types.mjs";
/**
 ...
 */
export function adaptMarketFromMYX(pool) {
 ...
 */
export function adaptPriceFromMYX(ticker) {
 ...
 */
export function adaptMarketDataFromMYX(pool, ticker, formatters) {
    const symbol = pool.baseSymbol || extractSymbolFromPoolId(pool.poolId);
 ...
 */
export function filterMYXExclusiveMarkets(pools) {
    return pools.filter((pool) => {
        const symbol = pool.baseSymbol || extractSymbolFromPoolId(pool.poolId);
        // Exclude markets that overlap with HyperLiquid
        return !MYX_HL_OVERLAPPING_MARKETS.includes(symbol);
    });
}
/**
 ...
 */
export function isOverlappingMarket(symbol) {
    return MYX_HL_OVERLAPPING_MARKETS.includes(symbol);
}
// ============================================================================
// Pool ID Utilities
 ...
 */
export function buildPoolSymbolMap(pools) {
    const map = new Map();
 ...
 */
export function buildSymbolPoolsMap(pools) {
    const map = new Map();
    for (const pool of pools) {
        const symbol = pool.baseSymbol || extractSymbolFromPoolId(pool.poolId);
 ...
 */
export function extractSymbolFromPoolId(poolId) {
 ...
 */
export function adaptPositionFromMYX(pos, poolSymbolMap) {
 ...
 */
export function adaptOrderFromMYX(historyOrder, poolSymbolMap) {
    ...
    // Map direction
    const side = historyOrder.direction === MYXDirectionEnum.Long ? 'buy' : 'sell';
    // Map order type
    ...
    const isTrigger = historyOrder.execType === MYXExecTypeEnum.TP ||
    ...
    return {
        ...
        isTrigger,
        detailedOrderType,
        reduceOnly: historyOrder.operation === MYXOperationEnum.Decrease ? true : undefined,
        providerId: 'myx',
    };
}
// ============================================================================
 ...
 */
export function adaptAccountStateFromMYX(accountInfo, walletBalance) {
 ...
 */
export function adaptOrderFillFromMYX(order, poolSymbolMap) {
 ...
 */
export function adaptFundingFromMYX(flows, poolSymbolMap) {
    return flows
 ...
 */
export function adaptUserHistoryFromMYX(flows) {
    return flows
        .filter((flow) => flow.type === MYXTradeFlowTypeEnum.MarginAccountDeposit ||
 ...
 */
export function adaptCandleFromMYX(item) {
    return {
        time: item.time,
        open: item.open,
        high: item.high,
 ...
 * @returns MetaMask CandleStick object
 */
export function adaptCandleFromMYXWebSocket(data) {
    return {
        time: data.t,
        open: data.o,
        high: data.h,
 ...
 */
export function toMYXKlineResolution(period) {
 ...
 */
export function assertMYXSuccess(response, context) {
...
//# sourceMappingURL=myxAdapter.mjs.map