 ...
 */
const hasExceededSignificantFigures = (priceString, maxSigFigs = perpsConfig_js_1.DECIMAL_PRECISION_CONFIG.MaxSignificantFigures) => {
 ...
 */
const roundToSignificantFigures = (priceString, maxSigFigs = perpsConfig_js_1.DECIMAL_PRECISION_CONFIG.MaxSignificantFigures) => {
    if (!priceString || priceString.trim() === '') {
        return priceString;
    }
    const cleaned = priceString.replace(/[$,]/gu, '').trim();
