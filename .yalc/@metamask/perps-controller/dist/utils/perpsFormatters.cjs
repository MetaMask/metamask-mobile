...
exports.PRICE_RANGES_UNIVERSAL = [
    ...
    },
    {
        ...
        significantDigits: 5,
        minimumDecimals: 2,
        maximumDecimals: perpsConfig_js_1.DECIMAL_PRECISION_CONFIG.MaxPriceDecimals,
        threshold: exports.PRICE_THRESHOLD.LOW,
    },
    {
...
const formatPerpsFiat = (balance, options) => {
    ...
    if (isNaN(value)) {
        ...
        return perpsConfig_js_1.PERPS_CONSTANTS.FallbackPriceDisplay;
    }
...
const formatPnl = (pnl) => {
    ...
    if (isNaN(value)) {
        return perpsConfig_js_1.PERPS_CONSTANTS.ZeroAmountDetailedDisplay;
    }
...
const formatFundingRate = (value, options) => {
    ...
    if (value === undefined || value === null) {
        return showZero ? perpsConfig_js_1.FUNDING_RATE_CONFIG.ZeroDisplay : '';
    }
    const percentage = value * perpsConfig_js_1.FUNDING_RATE_CONFIG.PercentageMultiplier;
    const formatted = percentage.toFixed(perpsConfig_js_1.FUNDING_RATE_CONFIG.Decimals);
    // Check if the result is effectively zero
    if (showZero && parseFloat(formatted) === 0) {
        return perpsConfig_js_1.FUNDING_RATE_CONFIG.ZeroDisplay;
    }
    return `${formatted}%`;
};
