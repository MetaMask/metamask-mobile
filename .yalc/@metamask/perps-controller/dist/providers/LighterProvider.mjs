...
export class LighterProvider {
    constructor(options) {
        ...
        _LighterProvider_ensureAccountIndex.set(this, async () => {
            ...
            if (__classPrivateFieldGet(this, _LighterProvider_configuredAccountIndex, "f") !== undefined) {
                ...
                if (!ownerAddress || ownerAddress !== __classPrivateFieldGet(this, _LighterProvider_boundAddress, "f")) {
                    ...
                    throw new Error(`${LIGHTER_UNSUPPORTED_CAPABILITY_PREFIX} configured account ${__classPrivateFieldGet(this, _LighterProvider_configuredAccountIndex, "f")} is not owned by the selected wallet address`);
                }
        ...
        _LighterProvider_assertStandardAccount.set(this, (accountType) => {
            ...
            if (accountType === undefined) {
                throw new Error(`${LIGHTER_UNSUPPORTED_CAPABILITY_PREFIX} account type could not be verified (account not found); refusing to assume a Standard account`);
            }
            if (accountType !== 0) {
                throw new Error(`${LIGHTER_UNSUPPORTED_CAPABILITY_PREFIX} Premium accounts are not supported yet: their fee semantics are unverified and history would be financially incorrect`);
            }
        });
        /**
         ...
         */
        _LighterProvider_isUnsupportedCapabilityError.set(this, (error) => String(error).includes(LIGHTER_UNSUPPORTED_CAPABILITY_PREFIX));
        _LighterProvider_isDataIntegrityError.set(this, (error) => String(error).includes(LIGHTER_DATA_INTEGRITY_PREFIX));
        /**
        ...
        _LighterProvider_readNonceLedger.set(this, async (accountIndex) => {
            ...
            try {
                ...
                throw new Error(`Lighter nonce ledger read failed; refusing writes: ${ensureError(error, 'LighterProvider.#readNonceLedger').message}`);
            }
            if (raw === null) {
        ...
        _LighterProvider_loadTpslJournal.set(this, async (settlementKey) => {
            ...
            try {
                ...
                throw new Error(`Lighter TP/SL journal read failed for ${settlementKey}; refusing protection changes: ${ensureError(error, 'LighterProvider.#loadTpslJournal').message}`);
            }
            if (baseRaw === null) {
                return null;
            }
        ...
        _LighterProvider_registerVenueKey.set(this, async (accountIndex, changePubKeyBody, generation, nextNonce, submit) => {
            ...
            __classPrivateFieldGet(this, _LighterProvider_assertSession, "f").call(this, generation);
            const result = await submit(LIGHTER_TX_TYPE_CHANGE_PUB_KEY, signed.txInfo, undefined, extractDispatchIdentity(signed));
        ...
        _LighterProvider_ensureMarketMargins.set(this, async () => {
            ...
            if (__classPrivateFieldGet(this, _LighterProvider_marginBySymbol, "f").size > 0 &&
                Date.now() - __classPrivateFieldGet(this, _LighterProvider_marginFetchedAt, "f") < LIGHTER_MARGIN_METADATA_TTL_MS) {
                return;
            }
        ...
        _LighterProvider_handleWsMessage.set(this, (raw) => {
            ...
            if (type.includes('account_all_positions') && message.positions) {
                ...
                for (const [marketId, position] of Object.entries(message.positions)) {
                    const adapted = adaptPositionFromLighter(position, __classPrivateFieldGet(this, _LighterProvider_maxLeverageForMarketId, "f").call(this, position.marketId));
        ...
        _LighterProvider_handleTradesMessage.set(this, (message) => {
            ...
            for (const marketTrades of Object.values(message.trades ?? {})) {
                for (const trade of marketTrades) {
                    ...
                    try {
                        fills.push(adaptFillFromLighterTrade(trade, symbol, __classPrivateFieldGet(this, _LighterProvider_accountIndex, "f") ?? -1));
                    }
                    catch (error) {
        ...
        try {
            ...
            return markets
                ...
                if (margins?.lastTradePrice && margins.lastTradePrice > 0) {
                    const minBaseSize = computeLighterMinOrderSize(market, margins.lastTradePrice);
    ...
    async getPositions(_params) {
        try {
            ...
            return account.positions
                .map((position) => adaptPositionFromLighter(position, __classPrivateFieldGet(this, _LighterProvider_maxLeverageForMarketId, "f").call(this, position.marketId)))
        ...
        try {
            ...
            const result = await __classPrivateFieldGet(this, _LighterProvider_withVenueWriteLock, "f").call(this, accountIndex, async (nextNonce, submit) => {
                if (leverageImfHundredths !== null) {
                    ...
                    await submit(LIGHTER_TX_TYPE_UPDATE_LEVERAGE, signedLeverage.txInfo, undefined, {
                ...
                return await submit(LIGHTER_TX_TYPE_CREATE_ORDER, signed.txInfo, undefined, {
    ...
    async cancelOrder(params, inheritedGeneration) {
        try {
            ...
            await __classPrivateFieldGet(this, _LighterProvider_withVenueNonce, "f").call(this, accountIndex, async (nonce, submit) => {
                ...
                return await submit(LIGHTER_TX_TYPE_CANCEL_ORDER, signed.txInfo, undefined, {
                    ...extractDispatchIdentity(signed),
    ...
    async updatePositionTPSL(params) {
        try {
            ...
            await __classPrivateFieldGet(this, _LighterProvider_withVenueWriteLock, "f").call(this, accountIndex, async (nextNonce, submit) => {
                ...
                const openOrders = rawOrders.map((order) => adaptOrderFromLighter(order, __classPrivateFieldGet(this, _LighterProvider_marketsById, "f").get(order.marketIndex)?.symbol ??
                    String(order.marketIndex)));
                ...
                if (wantsReplacement && groupedPayload !== null) {
                    ...
                    await submit(isSingleTrigger
                        ...
                        : LIGHTER_TX_TYPE_CREATE_GROUPED_ORDERS, signed.txInfo, () => {
    ...
    async updateMargin(params) {
        try {
            ...
            await __classPrivateFieldGet(this, _LighterProvider_withVenueNonce, "f").call(this, accountIndex, async (nonce, submit) => {
                ...
                return await submit(LIGHTER_TX_TYPE_UPDATE_MARGIN, signed.txInfo, undefined, {
                    ...extractDispatchIdentity(signed),
    ...
    async withdraw(params) {
        try {
            ...
            const minWithdraw = parseFloat(LIGHTER_BRIDGE_CONFIG[__classPrivateFieldGet(this, _LighterProvider_isTestnet, "f") ? 'testnet' : 'mainnet']
                .minWithdrawUsdc);
            ...
            const result = await __classPrivateFieldGet(this, _LighterProvider_withVenueNonce, "f").call(this, accountIndex, async (nonce, submit) => {
                ...
                return await submit(LIGHTER_TX_TYPE_WITHDRAW, signed.txInfo, undefined, {
                    ...extractDispatchIdentity(signed),
    ...
    getDepositRoutes(params) {
        ...
        return __classPrivateFieldGet(this, _LighterProvider_bridgeRoute, "f").call(this, LIGHTER_BRIDGE_CONFIG.mainnet.minDepositUsdc);
    }
    getWithdrawalRoutes(params) {
        ...
        return __classPrivateFieldGet(this, _LighterProvider_bridgeRoute, "f").call(this, LIGHTER_BRIDGE_CONFIG.mainnet.minWithdrawUsdc);
    }
    // ============================================================================
    // Block Explorer
    // ============================================================================
