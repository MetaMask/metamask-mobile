...
class LighterProvider {
    constructor(options) {
        ...
        _LighterProvider_registerVenueKey.set(this, async (accountIndex, changePubKeyBody) => {
            ...
            const result = await __classPrivateFieldGet(this, _LighterProvider_clientService, "f").sendTx(lighterConfig_js_1.LIGHTER_TX_TYPE_CHANGE_PUB_KEY, signed.txInfo);
        ...
        _LighterProvider_handleWsMessage.set(this, (raw) => {
            ...
            if (type.includes('account_all_positions') && message.positions) {
                ...
                for (const [marketId, position] of Object.entries(message.positions)) {
                    const adapted = (0, lighterAdapter_js_1.adaptPositionFromLighter)(position);
    ...
    async getPositions(_params) {
        try {
            ...
            return account.positions
                ...
                .map(lighterAdapter_js_1.adaptPositionFromLighter);
        }
        catch (caughtError) {
    ...
    async updatePositionTPSL(params) {
        try {
            ...
            const sizeInt = (0, lighterConfig_js_1.toLighterInteger)(coverSize, market.supportedSizeDecimals);
            ...
            const buildOrder = (orderType, triggerPriceRaw, clientOrderIndex) => {
                ...
                return [
                    ...
                    clientOrderIndex,
                    String(sizeInt),
                    String((0, lighterConfig_js_1.toLighterInteger)(execution, market.supportedPriceDecimals)),
                    isAsk,
                    orderType,
                    lighterConfig_js_1.LIGHTER_TIME_IN_FORCE_IMMEDIATE_OR_CANCEL,
                    1,
                    String((0, lighterConfig_js_1.toLighterInteger)(trigger, market.supportedPriceDecimals)),
            ...
            if (params.takeProfitPrice) {
                grouped.push(...buildOrder(lighterConfig_js_1.LIGHTER_ORDER_TYPE_TAKE_PROFIT, params.takeProfitPrice, clientBase + 1));
                orderCount += 1;
            }
            if (params.stopLossPrice) {
                grouped.push(...buildOrder(lighterConfig_js_1.LIGHTER_ORDER_TYPE_STOP_LOSS, params.stopLossPrice, clientBase + 2));
                orderCount += 1;
            }
            const groupingType = orderCount === 2 ? lighterConfig_js_1.LIGHTER_GROUPING_ONE_CANCELS_THE_OTHER : 0;
            await __classPrivateFieldGet(this, _LighterProvider_withVenueNonce, "f").call(this, accountIndex, async (nonce) => {
                const signed = await __classPrivateFieldGet(this, _LighterProvider_getSignerBridge, "f").call(this).execute({
