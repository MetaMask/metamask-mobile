...
const VALID_MARKET_TYPES = new Set(Object.values(index_js_1.MarketCategory));
...
const GlobalSnapshotMarketStruct = (0, superstruct_1.object)({
    symbol: (0, superstruct_1.string)(),
    provider: (0, superstruct_1.string)(),
    dex: (0, superstruct_1.string)(),
    name: (0, superstruct_1.nullable)((0, superstruct_1.string)()),
    description: (0, superstruct_1.nullable)((0, superstruct_1.string)()),
    iconUrl: (0, superstruct_1.nullable)((0, superstruct_1.string)()),
    szDecimals: (0, superstruct_1.number)(),
    maxLeverage: (0, superstruct_1.number)(),
    markPrice: (0, superstruct_1.string)(),
    price: (0, superstruct_1.string)(),
    midPrice: (0, superstruct_1.nullable)((0, superstruct_1.string)()),
    oraclePrice: (0, superstruct_1.string)(),
    change24h: (0, superstruct_1.string)(),
    changePercent24h: (0, superstruct_1.number)(),
    funding: (0, superstruct_1.string)(),
    volume24h: (0, superstruct_1.string)(),
    openInterest: (0, superstruct_1.string)(),
    category: (0, superstruct_1.nullable)((0, superstruct_1.string)()),
    keywords: (0, superstruct_1.nullable)((0, superstruct_1.array)((0, superstruct_1.string)())),
    tags: (0, superstruct_1.nullable)((0, superstruct_1.array)((0, superstruct_1.string)())),
    listedAt: (0, superstruct_1.nullable)((0, superstruct_1.number)()),
    trend: (0, superstruct_1.array)((0, superstruct_1.tuple)([(0, superstruct_1.number)(), (0, superstruct_1.string)()])),
});
const GlobalSnapshotStruct = (0, superstruct_1.object)({
    schemaVersion: (0, superstruct_1.number)(),
    provider: (0, superstruct_1.string)(),
    network: (0, superstruct_1.string)(),
    enabledDexes: (0, superstruct_1.array)((0, superstruct_1.string)()),
    fingerprint: (0, superstruct_1.string)(),
    generatedAt: (0, superstruct_1.number)(),
    receivedAt: (0, superstruct_1.number)(),
    maxAgeMs: (0, superstruct_1.number)(),
    complete: (0, superstruct_1.boolean)(),
    perDexErrors: (0, superstruct_1.array)((0, superstruct_1.object)({
        dex: (0, superstruct_1.string)(),
        error: (0, superstruct_1.string)(),
    })),
    markets: (0, superstruct_1.array)(GlobalSnapshotMarketStruct),
});
/**
 ...
 */
const TerminalPerpetualItemStruct = (0, superstruct_1.type)({
    symbol: (0, superstruct_1.string)(),
    name: (0, superstruct_1.optional)((0, superstruct_1.nullable)((0, superstruct_1.string)())),
    description: (0, superstruct_1.optional)((0, superstruct_1.nullable)((0, superstruct_1.string)())),
    szDecimals: (0, superstruct_1.optional)((0, superstruct_1.number)()),
    maxLeverage: (0, superstruct_1.optional)((0, superstruct_1.number)()),
    marginTableId: (0, superstruct_1.optional)((0, superstruct_1.number)()),
    onlyIsolated: (0, superstruct_1.optional)((0, superstruct_1.boolean)()),
    isDelisted: (0, superstruct_1.optional)((0, superstruct_1.boolean)()),
...
class TerminalMarketService {
    ...
    async fetchMarkets() {
        if (__classPrivateFieldGet(this, _TerminalMarketService_cache, "f") &&
            Date.now() - __classPrivateFieldGet(this, _TerminalMarketService_cache, "f").timestamp < perpsConfig_js_1.TERMINAL_API_CONFIG.CacheTtlMs) {
            return {
        ...
        const timeoutId = setTimeout(() => controller.abort(new Error('Terminal API fetch timed out')), perpsConfig_js_1.TERMINAL_API_CONFIG.FetchTimeoutMs);
        let response;
        try {
    ...
    logError(error, method) {
        __classPrivateFieldGet(this, _TerminalMarketService_deps, "f").logger.error((0, errorUtils_js_1.ensureError)(error, `TerminalMarketService.${method}`), {
            tags: { feature: perpsConfig_js_1.PERPS_CONSTANTS.FeatureName, source: 'terminal-api' },
            context: {
...
exports.TerminalMarketService = TerminalMarketService;
_TerminalMarketService_deps = new WeakMap(), _TerminalMarketService_cache = new WeakMap(), _TerminalMarketService_globalSnapshotCache = new WeakMap(), _TerminalMarketService_globalSnapshotInFlight = new WeakMap(), _TerminalMarketService_globalSnapshotGeneration = new WeakMap(), _TerminalMarketService_instances = new WeakSet(), _TerminalMarketService_fetchAndValidateGlobalSnapshot = async function _TerminalMarketService_fetchAndValidateGlobalSnapshot(identity, url) {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(new Error('Terminal global snapshot timed out')), perpsConfig_js_1.TERMINAL_API_CONFIG.FetchTimeoutMs);
    try {
        ...
        if ((0, utils_1.stringToBytes)(text).byteLength > GLOBAL_SNAPSHOT_MAX_PAYLOAD_BYTES) {
            throw new Error('Terminal global snapshot payload exceeds 1 MiB');
        }
        let body;
        try {
        ...
        if (!(0, superstruct_1.is)(body, GlobalSnapshotStruct)) {
    ...
    return (0, hyperLiquidConfig_js_1.canonicalizeHyperLiquidDexes)(normalized);
    ...
    const digest = await (0, utils_1.sha256)((0, utils_1.stringToBytes)(canonicalIdentity));
    return `sha256:${(0, utils_1.bytesToHex)(digest).slice(2)}`;
    ...
    return {
        ...
        change24h: (0, marketDataTransform_js_1.formatChange)(change24h, formatters),
    ...
    if (category === 'pre_ipo') {
        return index_js_1.MarketCategory.PreIpo;
    }
    ...
    return {
        ...
        markets: (0, marketUtils_js_1.clonePerpsMarketData)(result.markets),
    };
    ...
    for (const item of raw) {
        if ((0, superstruct_1.is)(item, TerminalPerpetualItemStruct)) {
            valid.push(item);
        }
        else {
            __classPrivateFieldGet(this, _TerminalMarketService_deps, "f").logger.error((0, errorUtils_js_1.ensureError)(new Error('Terminal API item failed schema validation'), 'TerminalMarketService.validateItems'), {
                tags: {
...
//# sourceMappingURL=TerminalMarketService.cjs.map