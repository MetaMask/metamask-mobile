...
import { PERPS_CONSTANTS, SUBSCRIPTION_BENEFITS_CACHE } from "../constants/perpsConfig.mjs";
import { getSelectedEvmAccountFromMessenger } from "../utils/accountUtils.mjs";
import { ensureError } from "../utils/errorUtils.mjs";
import { formatAccountToCaipAccountId } from "../utils/rewardsUtils.mjs";
/**
 * Default MetaMask builder fee, in basis points.
 * This is the fee every user pays when no cheaper source applies.
 */
const DEFAULT_FEE_BIPS = BUILDER_FEE_CONFIG.MaxFeeDecimal * BASIS_POINTS_DIVISOR;
/**
...
export class RewardsIntegrationService {
    ...
    async resolveFee() {
        ...
        if (rewardsDiscountBips !== undefined) {
            const rewardsFeeBips = DEFAULT_FEE_BIPS * (1 - rewardsDiscountBips / BASIS_POINTS_DIVISOR);
    ...
    async refreshSubscriptionBenefits() {
        ...
        if (snapshotAge < SUBSCRIPTION_BENEFITS_CACHE.FreshMs ||
...
async function _RewardsIntegrationService_readSubscriptionBenefits(source) {
    ...
    try {
        ...
        __classPrivateFieldGet(this, _RewardsIntegrationService_deps, "f").logger.error(ensureError(error, 'RewardsIntegrationService.refreshSubscriptionBenefits'), {
...
async function _RewardsIntegrationService_calculateRewardsDiscount() {
    try {
        const evmAccount = getSelectedEvmAccountFromMessenger(__classPrivateFieldGet(this, _RewardsIntegrationService_messenger, "f"));
        if (!evmAccount) {
        ...
        const caipAccountId = formatAccountToCaipAccountId(evmAccount.address, chainId, __classPrivateFieldGet(this, _RewardsIntegrationService_deps, "f").logger);
        if (!caipAccountId) {
        ...
        __classPrivateFieldGet(this, _RewardsIntegrationService_deps, "f").logger.error(ensureError(error, 'RewardsIntegrationService.calculateUserFeeDiscount'), {
            tags: { feature: PERPS_CONSTANTS.FeatureName },
            context: {
                name: 'RewardsIntegrationService.calculateUserFeeDiscount',
                data: {},
...
//# sourceMappingURL=RewardsIntegrationService.mjs.map