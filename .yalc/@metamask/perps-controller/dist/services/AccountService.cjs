...
class AccountService {
    ...
    async withdraw(options) {
        ...
        try {
            __classPrivateFieldGet(this, _AccountService_deps, "f").tracer.trace({
                name: index_js_1.PerpsTraceNames.Withdraw,
                id: traceId,
            ...
            if (context.stateManager) {
                context.stateManager.update((state) => {
                    ...
                    const feeAmount = perpsConfig_js_1.WITHDRAWAL_CONSTANTS.DefaultFeeAmount;
                    const netAmount = Math.max(0, grossAmount - feeAmount);
                    // Get current account address via messenger
                    const evmAccount = (0, accountUtils_js_1.getSelectedEvmAccountFromMessenger)(__classPrivateFieldGet(this, _AccountService_messenger, "f"));
                    ...
                    const withdrawalRequest = {
                        ...
                        asset: hyperLiquidConfig_js_1.USDC_SYMBOL,
            ...
            if (result.success) {
                ...
                __classPrivateFieldGet(this, _AccountService_deps, "f").metrics.trackPerpsEvent(index_js_1.PerpsAnalyticsEvent.WithdrawalTransaction, {
                    [eventNames_js_1.PERPS_EVENT_PROPERTY.STATUS]: eventNames_js_1.PERPS_EVENT_VALUE.STATUS.EXECUTED,
                    [eventNames_js_1.PERPS_EVENT_PROPERTY.WITHDRAWAL_AMOUNT]: parseFloat(params.amount),
                ...
                refreshAccountState().catch((refreshError) => {
                    __classPrivateFieldGet(this, _AccountService_deps, "f").logger.error((0, errorUtils_js_1.ensureError)(refreshError, 'AccountService.withdraw'), {
            ...
            if (context.stateManager) {
                context.stateManager.update((state) => {
                    state.lastError = result.error ?? perpsErrorCodes_js_1.PERPS_ERROR_CODES.WITHDRAW_FAILED;
                    state.lastUpdateTimestamp = Date.now();
                    state.lastWithdrawResult = {
                        success: false,
                        error: result.error ?? perpsErrorCodes_js_1.PERPS_ERROR_CODES.WITHDRAW_FAILED,
                        amount: params.amount,
                        asset: hyperLiquidConfig_js_1.USDC_SYMBOL,
                        timestamp: Date.now(),
                        txHash: '',
            ...
            const errorMessage = error instanceof Error
                ? error.message
                : perpsErrorCodes_js_1.PERPS_ERROR_CODES.WITHDRAW_FAILED;
            __classPrivateFieldGet(this, _AccountService_deps, "f").logger.error((0, errorUtils_js_1.ensureError)(error, 'AccountService.withdraw'), {
                tags: { feature: perpsConfig_js_1.PERPS_CONSTANTS.FeatureName },
                context: {
                    name: 'AccountService.withdraw',
            ...
            __classPrivateFieldGet(this, _AccountService_deps, "f").metrics.trackPerpsEvent(index_js_1.PerpsAnalyticsEvent.WithdrawalTransaction, {
                [eventNames_js_1.PERPS_EVENT_PROPERTY.STATUS]: eventNames_js_1.PERPS_EVENT_VALUE.STATUS.FAILED,
                [eventNames_js_1.PERPS_EVENT_PROPERTY.WITHDRAWAL_AMOUNT]: params.amount,
                [eventNames_js_1.PERPS_EVENT_PROPERTY.COMPLETION_DURATION]: completionDuration,
    ...
    async validateWithdrawal(options) {
        ...
        try {
            ...
            __classPrivateFieldGet(this, _AccountService_deps, "f").logger.error((0, errorUtils_js_1.ensureError)(error, 'AccountService.validateWithdrawal'), {
                tags: { feature: perpsConfig_js_1.PERPS_CONSTANTS.FeatureName },
                context: {
...
//# sourceMappingURL=AccountService.cjs.map