...
4. DEV recipe evidence uses structured `[PerpsConnectionProof]`, `[PerpsLoadProof]`, and `[PerpsDetailLoadProof]` records. Reconnection proof records identify context change, reconnect start, health validation, preload commit or rejection, and completion without wallet addresses.
