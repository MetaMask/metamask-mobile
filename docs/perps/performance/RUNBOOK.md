...
`release pending` rather than synthesizing them.
...
Metro compilation and fixture/account setup remain visible in `trace.json` but are excluded from product durations. The relative ordering of Homepage Ready and Perps bootstrap is measured rather than assumed.
