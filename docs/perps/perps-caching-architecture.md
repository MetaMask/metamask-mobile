...
When configured, the Hyperliquid schema-v2 Terminal snapshot is the preferred global-market preload. Core accepts it only for the exact provider/network/static-DEX identity and marks every returned row with snapshot provenance; an invalid, stale, partial, or unavailable snapshot falls back to the provider. Positions, orders, and account state are fetched as one account-scoped atomic snapshot and seeded into all three Mobile channels before any subscriber is notified.
...
| Account switch                        | Provider recreated (all instance caches reset)                                                                         | `cachedUserDataByProvider` cleared if address changed. Market data preserved. | User-scoped positions/orders/account/fills cleared; global markets/prices/OI/top-of-book/candles preserved | User data key deleted. Market data preserved. | Preserved             |
...
**Double-clear on account/network switch**: User-scoped channels are cleared once immediately in the Redux store subscriber and again inside `performReconnection()` to prevent a prior-account flash. Account-only reconnection preserves global channels in both passes; provider/network/DEX changes clear every channel.
