...
- `interaction_type` (required): Type of user interaction
  ...
  - **Chase v15 coverage:** Controller v15 exports only `chase_backgrounded_converted` and `chase_terminated` as Chase interaction types. Mobile includes the existing `order_type: chase` and `reduce_only` properties on the Chase place-order tap.
  - **Chase v15 limitations:** The controller exports no `chase_concurrency_limit_hit` or `chase_reprice` interaction types, and no `fill_pct_at_terminate` or `chase_max_distance_bps` property keys. Mobile does not fabricate these values. The max-distance lifecycle event also has no matching `interaction_type` or `notification_type`, so its local notification has no analytics attribution. User termination remains the only source of `chase_terminated`.
