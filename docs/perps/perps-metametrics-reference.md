...
| `trading_mode` | `'lite'` \| `'pro'` | Current Perps interface mode via `enrichWithTradingMode` in `analytics.trackEvent` |

`trading_mode` is the Lite/Pro interface mode. It is intentionally separate from `mode` (`PERPS_EVENT_PROPERTY.MODE`), which search already uses for query intent (`'discovery'` \| `'intent'` \| `'browse'`).
...
- `interaction_type` (required): Type of user interaction
  ...
  - **Mode selection:** `'mode_selection_dismissed'` — Lite/Pro chooser closed without selecting _(local constant `PERPS_MODE_SELECTION_DISMISSED`, pending addition to `PERPS_EVENT_VALUE`)_. Includes `source`, `entry`, and `time_on_screen_ms`. Selection itself uses `button_clicked` with `trading_mode`. Header toggles also use `button_clicked` with `trading_mode`, but only after the mode is actually applied (not when the chooser opens instead).
...
- **Lite/Pro mode enrichment**: `app/util/analytics/enrichWithTradingMode.ts`, `app/components/UI/Perps/utils/perpsAnalyticsAttribution.ts` (`getTradingModeAnalyticsProperties`)
