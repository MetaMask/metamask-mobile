...
- **Reusable job:** `.github/workflows/run-appium-e2e-workflow.yml` — plans shards then runs Playwright with `--grep` and explicit `SPEC_FILES`. Default: fixed `total_splits` from the smoke YAML + LPT / equal-count packing. Opt-in label `appium-dynamic-shards`: choose N from timings (20m − 5m overhead, max 6) then the same packing.
