...
| Unhealthy session                     | Test-scoped `driver` fixture recreates once (`sessionRecreated=true` annotation). Soft-reload device-health failures also mark the shared session for recreate so later tests do not cascade. |
