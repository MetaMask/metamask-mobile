...
- **Seedless:** `SmokeSeedlessOnboarding` uses **3** shards per platform (thin keep list in `tests/smoke-appium/seedless/`; may drop to 2 after runtime is confirmed). Smart E2E selects this tag from `tests/tags.js` — prefer CV/unit for Account Already Exists, attribution props, and Import SRP UI.
