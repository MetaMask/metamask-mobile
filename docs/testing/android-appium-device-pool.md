...
`ci.yml` sets `android-device-pool-size: 2`. Most Android smoke jobs forward
that input. MultiChain API and Network Expansion are hardcoded to `1` because
their specs still start a local dapp on one host port (`EADDRINUSE` at N=2);
#35647 restores the input. Fixture validation stays one emulator / cold boot.
Set the input to `1` to revert remaining suites to the historical path.
