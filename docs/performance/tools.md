...
| **Release Profiler**        | Library                                                     | Production-like CPU profile                      | RC build → shake → profile → `chrome://tracing`. [release-build-profiler.md](../readme/release-build-profiler.md)                                                           |
...
| **Bundle analysis**         | External tooling (not wired)                                | Startup / bundle size                            | Expo Atlas: `EXPO_UNSTABLE_ATLAS=true npx expo export` → `npx expo-atlas`; or `source-map-explorer`.                                                                        |
