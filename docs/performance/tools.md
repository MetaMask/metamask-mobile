...
| **Ad-hoc Hermes profile analysis** | GitHub Action (`Analyze App Profiling`)              | Review one 6-hour performance run per scenario, or every run in a lookback window   | Runs `yarn skills`, then uses the `mms-swaps-cpu-profile-audit` bundled analyzer for timing evidence and `mms-performance` guardrails. Downloads only Hermes `.cpuprofile` files, groups segment 1..N and retries by scenario, then Claude reviews each scenario. `lookback_hours` (or `--lookback-hours`) analyzes every scheduled run in the window and reports per-run medians plus how often each frame stayed hot. BrowserStack app-profiling metrics are excluded. Script: `tests/scripts/analyze-app-profiling.mjs`. |
