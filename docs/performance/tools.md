...
| **Hermes profile analysis** | GitHub Action (`Analyze App Profiling`)                     | Manual run/scenario/lookback analysis            | Ad-hoc only via Actions → `workflow_dispatch` (run ID, scenario, or custom lookback). Optional digest to the configured personal Slack DM. Uses the `mms-swaps-cpu-profile-audit` bundled analyzer, groups segment 1..N and retries by scenario, and reports per-run medians plus repeated hot frames. BrowserStack app-profiling metrics are excluded. Script: `.github/scripts/qa-automation/performance-tests/analyze-app-profiling.mjs`. |
