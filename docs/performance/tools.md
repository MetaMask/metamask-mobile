...
| **Ad-hoc app-profiling analysis** | GitHub Action (`Analyze App Profiling`)              | Review one 6-hour performance run per scenario   | `workflow_dispatch` downloads `app-profiling-*.json` (+ Hermes `.cpuprofile` when present), flags heuristic issues, then Claude reviews each scenario. Script: `tests/scripts/analyze-app-profiling.mjs`. |
