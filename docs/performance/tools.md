...
| **Ad-hoc Hermes profile analysis** | GitHub Action (`Analyze App Profiling`)              | Review one 6-hour performance run per scenario   | `workflow_dispatch` downloads only Hermes `.cpuprofile` files, groups segment 1..N and retries by scenario, summarizes sampled JS stacks, then Claude reviews each scenario. BrowserStack app-profiling metrics are excluded. Script: `tests/scripts/analyze-app-profiling.mjs`. |
