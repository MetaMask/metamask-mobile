...
| **Hermes profile analysis** | GitHub Action (`Analyze App Profiling`)              | Daily 24-hour report, or manual run/scenario/lookback analysis   | Runs daily at 08:00 UTC over the preceding 24 hours and sends the digest to the configured personal Slack DM. `workflow_dispatch` remains available for a run ID, scenario, or custom lookback. Uses the `mms-swaps-cpu-profile-audit` bundled analyzer, groups segment 1..N and retries by scenario, and reports per-run medians plus repeated hot frames. BrowserStack app-profiling metrics are excluded. Script: `tests/scripts/analyze-app-profiling.mjs`. |
