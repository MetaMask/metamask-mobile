# Explore Search — Handover Document

> **Audience**: engineers (and AI agents) picking up ownership of, or contributing to, the Explore search functionality in MetaMask Mobile.
>
...
>
> **Last verified against**: `main` @ July 2026. File paths and symbols were verified at the time of writing; treat exact line numbers as approximate and re-verify paths if this doc is more than a few months old.

---

## Table of contents

1. [Mental model (TL;DR)](#1-mental-model-tldr)
2. [How it works](#2-how-it-works)
3. [Where it is used](#3-where-it-is-used)
4. [Key areas to look out for](#4-key-areas-to-look-out-for)
5. [Long-term vision: centralising search](#5-long-term-vision-centralising-search)
6. [Agent knowledge base](#6-agent-knowledge-base)

---

## 1. Mental model (TL;DR)

- "Explore", "Trending" and "Discover" are the **same feature**. The product name is **Explore**; the code lives under `TrendingView`/`Trending` for historical reasons.
...
- `TokenSearchDiscoveryDataController` (registered in the Engine) is **not** part of listing search results. It hydrates full token metadata + price **after** a user taps a search result (see [§2.6](#26-what-happens-when-a-result-is-tapped)).
- Swaps/Bridge token selection uses a **different** search API (`POST {bridge-api}/getTokens/search`), which is the biggest fracture we want to close ([§5](#5-long-term-vision-centralising-search)).
- Manage/Import Tokens (`AddAsset`) reuses the **same** token-search stack as Explore (`useTrendingSearch`), scoped to a single chain.
...
| Browser omni-search | `app/components/UI/UrlAutocomplete/index.tsx` | Reuses `useExploreSearch` inside the browser URL bar (see [§3.4](#34-browser-omni-search-urlautocomplete)) |
...
- Optional internal debounce (`enableDebounce`, 200 ms). Explore disables it because `useExploreSearch` already debounces — see [§4.2](#42-debounce-layering).
...
| Bridge API | `POST {BRIDGE_API_BASE_URL}/getTokens/search` (JWT-authenticated) | **Swaps/Bridge only** — not the token API ([§3.2](#32-swapsbridge-token-selection)) |
...
> **History**: an older core package, `@metamask/token-search-discovery-controller` (`TokenSearchDiscoveryController`, `TokenSearchApiService`, Portfolio API `/tokens-search/*` endpoints, Moralis-shaped responses), is **deprecated and removed from core** (core PR #7789). Do not reintroduce it or copy patterns from it.
...
Important composition constraint: anything using `useExploreSearch` must be wrapped in `PerpsSectionProvider` (`feeds/perps/PerpsSectionProvider.tsx`) because `usePerpsFeed` → `usePerpsMarkets` reads perps connection contexts.
...
### 3.1 Explore search (primary surface)
...
### 3.2 Swaps/Bridge token selection
...
**This is a different API, different result type (`PopularToken` vs `TrendingAsset`), different auth model, and different debounce/min-length policy than Explore.** It exists because swaps need swappability guarantees the token API historically did not provide. This is the primary fracture to resolve ([§5](#5-long-term-vision-centralising-search)).

### 3.3 Manage / Import tokens
...
### 3.4 Browser omni-search (`UrlAutocomplete`)
...
### 3.5 Adjacent search implementations (not on the shared stack)

These are the other token/asset searches in the app. They are relevant to the centralisation story ([§5](#5-long-term-vision-centralising-search)) even though they are not "Explore search":
...
- **`TokenSearchDiscoveryController` (no "Data") no longer exists** — removed from core along with `@metamask/token-search-discovery-controller`. Old docs/PRs referencing `searchTokensFormatted`, `MoralisTokenResponseItem` or Portfolio `/tokens-search/*` endpoints are about the removed stack.
...
### 4.3 Stale responses and pagination guards
...
If you touch these hooks, keep both guards. Longer term this is exactly the duplication a shared search service should absorb ([§5](#5-long-term-vision-centralising-search)).

### 4.4 Search-result ordering subtleties
...
### 4.5 Networks must be supported server-side
...
### 4.6 Provider requirements

Any component calling `useExploreSearch` (directly or transitively) must be inside `PerpsSectionProvider`. Forgetting this crashes at runtime with a perps context error — it's the most common integration mistake (both `ExploreSearchScreen` and `UrlAutocomplete` wrap for this reason).

### 4.7 Security/risk filtering is surface-dependent
...
### 4.10 Things that look related but aren't
...
## 5. Long-term vision: centralising search
...
> Purpose: make AI agents (Cursor/Claude/Copilot etc.) productive on Explore-search tasks without re-deriving the architecture. Point agents at this file, or paste the relevant subsection into the task prompt.

### 6.1 Canonical facts (safe to assert)

- Product name **Explore**; code names `TrendingView` (screens) and `Trending` (shared hooks/components).
- Omni-search hook: `useExploreSearch(query, { exposePagination? })` → `{ sections: SearchFeedSection[] }`; feeds: `tokens | perps | stocks | predictions | sites`; 200 ms debounce owned by this hook.
- Token search is stateless: `searchTokens`/`getTrendingTokens`/`fetchRwas` from `@metamask/assets-controllers` hit `https://token.api.cx.metamask.io`. No Redux state, no search controller.
- `TokenSearchDiscoveryDataController` = post-tap asset hydration only (`AssetLoader` flow), not search listing.
- Swaps/Bridge search is a separate stack (`useSearchTokens`, bridge API, JWT, min length 3) — do not conflate.
- Import tokens (`AddAsset`) shares the Explore token pipeline via `useTrendingSearch` with `enableDebounce: true`, single-chain scope, `includeStocks: true`.
- Consumers of `useExploreSearch` must render inside `PerpsSectionProvider`.

### 6.2 File map (start here for any Explore-search task)
...
**Change debounce behaviour** — change it in exactly one layer (see §4.2); update `isLoading` logic that treats "debouncing" as loading; update timer-based tests (`jest.advanceTimersByTime`).

**Add a network to token search/trending** — verify both APIs support it; add to `TRENDING_NETWORKS_LIST` (+ code-fence guards like the Solana/Tron examples); add thresholds to `TRENDING_NETWORK_THRESHOLDS`; keep multi-chain baseline ≥ per-network values.

**Touch result ordering** — read §4.4 first; first-page-only sort and server-order pagination are intentional.
...
- Run: `yarn jest <file>` (never npm). Timers: debounce tests use fake timers.
...
*Questions or corrections: update this doc in place — it is the single source of truth for Explore search handover. If you change the architecture (new feeds, API migration, centralisation steps from §5), update §2/§5/§6 in the same PR.*
