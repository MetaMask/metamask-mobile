...
| `trending`             | Explore / Trending          | `handleTrendingUrl`      | Params: `tab=now\|macro\|rwas\|crypto\|sports\|sites`, `screen=stocks\|trending-tokens\|sites\|favorite-sites\|search`, `q`/`query` (prefills search when `screen=search`), `chainId` (CAIP/hex/decimal; chain-filters the trending tokens list and implies `screen=trending-tokens` when `screen`/`tab` are omitted), `timeframe=5m\|1h\|6h\|24h` (preselects the trending tokens time filter) (unknown values fall back to Explore) |
