...
- [ ] Is analytics enabled? (check `HAS_OVERRIDES` environment variable is NOT set to `true`)
...
The most common cause of missing events is the `HAS_OVERRIDES=true` environment variable.
If set to true, events will not be sent to Segment.
This is intentional for testing environments.

**Impact**: When `HAS_OVERRIDES=true`, the Segment client is replaced with a mock that does nothing.
...
1. Check `HAS_OVERRIDES` environment variable
