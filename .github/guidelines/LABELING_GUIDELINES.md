...
- **run-appium-ios-tests**: Also runs Appium iOS smoke tests on a PR targeting `main` (normally PRs run Appium Android only; main-target PRs with shared smoke/Appium infrastructure changes run iOS automatically). Uses the same Smart E2E Selection tags as Detox/Appium Android — it does not bypass path filters, build gates, or AI tag selection. Remove `pr-not-ready-for-e2e` when the PR is ready for E2E validation. Adding or removing this label re-triggers CI. Not honored on fork PRs or PRs targeting `release/*`/`stable`.
