...
#
# On re-runs (e.g. update-release-changelog.yml push trigger), the CHANGELOG.md
# already contains the Runway version from the previous sed. A pre-processing step
# converts it back to SemVer so auto-changelog recognises the existing entry and
...
if [[ ${#PATCH} -eq 2 ]]; then
  ...
  SED_SAFE_SEMVER=$(printf '%s' "$SEMVER_VERSION" | sed 's/[.]/\\./g')
  SED_SAFE_RUNWAY=$(printf '%s' "$RUNWAY_VERSION" | sed 's/[.]/\\./g')

  echo "OTA hotfix detected: Runway ${RUNWAY_VERSION} → SemVer ${SEMVER_VERSION}"
  echo "Changelog branch: ${CHANGELOG_BRANCH}"

  # Pre-processing: if a previous run already converted SEMVER→RUNWAY in CHANGELOG.md,
  # convert it back so auto-changelog recognises the existing entry and updates it
  # instead of creating a duplicate.
  if [[ -f CHANGELOG.md ]] && grep -q "${RUNWAY_VERSION}" CHANGELOG.md; then
    echo "Pre-processing: temporarily restoring ${RUNWAY_VERSION} → ${SEMVER_VERSION} in CHANGELOG.md"
    sed -i "s/${SED_SAFE_RUNWAY}/${SEMVER_VERSION}/g" CHANGELOG.md
    git add CHANGELOG.md
    if ! git diff --cached --quiet; then
      git commit -m "chore(temp): restore SemVer ${SEMVER_VERSION} in changelog for auto-changelog"
    fi
  fi
  ...
  # Post-processing: replace the SemVer prerelease back to the Runway version in CHANGELOG.md.
  # After the github-tools script we are on the changelog branch; it has already committed and pushed.
  if [[ -f CHANGELOG.md ]] && grep -q "${SEMVER_VERSION}" CHANGELOG.md; then
    echo "Post-processing: replacing ${SEMVER_VERSION} → ${RUNWAY_VERSION} in CHANGELOG.md"
    sed -i "s/${SED_SAFE_SEMVER}/${RUNWAY_VERSION}/g" CHANGELOG.md
    git add CHANGELOG.md
    if ! git diff --cached --quiet; then
