...
```
builds.yml               # WHAT to build (configuration data, project root)
.github/
...
### 1. `builds.yml` — Configuration Data (project root)
...
│  └── Load config from builds.yml (project root)                 │
...
For local builds, `builds.yml` is used to generate default env vars so you only need secrets and overrides in `.js.env`:

1. Run `yarn setup:expo` or `yarn setup` — generates `.js.env.defaults` from `main-dev` in builds.yml
2. `build.sh` sources `.js.env.defaults` first, then `.js.env` (overrides)
3. `.js.env` — add only secrets (Infura, Firebase, etc.) and optional overrides; see `.js.env.example`

This keeps `.js.env` minimal and in sync with CI dev builds.
...
2. .js.env.defaults (from builds.yml main-dev, generated by setup)
3. LaunchDarkly (runtime)       ← Overrides remote feature flags
4. builds.yml (build time)      ← Default values
```
...
1. Add to `builds.yml` anchor (production default):
...
→ Check build name matches exactly in `builds.yml`
