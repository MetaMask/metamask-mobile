...
├─────────────────────────────────────────────────────────────────────────────┤
│  Phase 4: Add E2E test workflows                             ⏳ PENDING    │
├─────────────────────────────────────────────────────────────────────────────┤
│  Phase 5: Deprecate Bitrise                                  ⏳ PENDING    │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```javascript
// scripts/verify-build-config.js
const { loadConfig } = require('./apply-build-config');
...
function verify(buildName, expectedVars) {
  const config = loadConfig(buildName);
  const mismatches = [];

  CRITICAL_VARS.forEach((key) => {
    const expected = expectedVars[key];
    const actual = config.env?.[key] || config.secrets?.[key];
  ...
  console.log('✅ Config matches expected values');
}

// CLI: node verify-build-config.js main-prod
if (require.main === module) {
  const buildName = process.argv[2];
  // Expected vars come from environment (set by old remapping)
  const expectedVars = {};
  CRITICAL_VARS.forEach((key) => {
    if (process.env[key]) {
      expectedVars[key] = process.env[key];
    }
  });
  verify(buildName, expectedVars);
}
...
- script@1:
    ...
    inputs:
      - content: |
          ...
          # Determine build name from MODE and ENVIRONMENT
          BUILD_NAME="${METAMASK_BUILD_TYPE}-${METAMASK_ENVIRONMENT}"
          # Normalize: "main-production" -> "main-prod"
          BUILD_NAME=$(echo "$BUILD_NAME" | sed 's/production/prod/' | sed 's/development/dev/')
          
          echo "Verifying builds.yml config for: $BUILD_NAME"
          
          # Run verification (compares old env vars with new config)
          node scripts/verify-build-config.js "$BUILD_NAME"
```
...
- [ ] `verify-build-config.js` created and tested locally
- [ ] Verification step added to all Bitrise build workflows
- [ ] 1+ week of builds passing with "✅ Config matches"
...
| ------------------------------ | --------------------------------------------- |
| `SEGMENT_WRITE_KEY`            | Analytics - wrong key = data in wrong project |
| `MM_SENTRY_DSN`                | Error tracking - wrong DSN = errors lost      |
| `IOS_GOOGLE_CLIENT_ID`         | Auth - wrong ID = login fails                 |
| `ANDROID_GOOGLE_CLIENT_ID`     | Auth - wrong ID = login fails                 |
| `MM_CARD_BAANX_API_CLIENT_KEY` | Card feature - wrong key = API errors         |
...
## Phase 4: E2E Test Workflows ⏳

**Goal:** Replace Bitrise E2E pipelines with GitHub Actions.

### Workflows to Create

```
.github/workflows/
├── e2e-android.yml      # Android E2E tests
├── e2e-ios.yml          # iOS E2E tests
├── e2e-smoke.yml        # Smoke tests (PR gate)
└── e2e-regression.yml   # Full regression (nightly)
```

### Key Components

| Bitrise Feature  | GitHub Actions Equivalent                   |
| ---------------- | ------------------------------------------- |
| iOS Simulator    | `macos-latest` runner                       |
| Android Emulator | `reactivecircus/android-emulator-runner@v2` |
| Detox            | Same (runs in GitHub Actions)               |
| Parallel tests   | Matrix strategy                             |
| Test sharding    | Custom sharding logic                       |
...
on:
  workflow_call:
    inputs:
      ...
      test_suite:
        required: true
        type: string  # smoke, regression, specific file
  ...
  e2e:
    ...
    steps:
      ...
      - name: Run Android Emulator
        ...
        with:
          api-level: 31
          script: yarn test:e2e:android --configuration ${{ inputs.test_suite }}
```

---

## Phase 5: Deprecate Bitrise ⏳
...
Week 5-6: Run both systems in parallel (E2E)
Week 7:   Disable Bitrise triggers, monitor GitHub Actions
Week 8:   Remove bitrise.yml, update documentation
```
...
2. **Phase 3-4 rollback:** Keep Bitrise pipelines active, disable GitHub Actions workflows
3. **Phase 5 rollback:** Re-enable Bitrise from git history
