...
- [ ] **PR against `release/x.y.z`**: Submit your PR against the `release/x.y.z` branch. This is where we merge new features to be included in release x.y.z so they get some time to receive extra testing before being pushed to `main` for production. If your PR is a hot-fix that needs to be published urgently, you may submit a PR against the `main` branch, but this PR will receive tighter scrutiny before merging.
