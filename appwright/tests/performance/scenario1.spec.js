...
test('Account creation with 50+ accounts, SRP 1 + SRP 2 + SRP 3', async ({ device }, testInfo) => {
  ...
  const screen1Timer = new TimerHelper('Time since the user clicks on "Account list" button until the account list is visible');
  const screen2Timer = new TimerHelper('Time since the user clicks on "Add account" button until the next modal is visible');
  const screen3Timer = new TimerHelper('Time since the user clicks on "Create Ethereum account" button until the account is visible');
