...
const { removeFencedCode, lintTransformedFile } = require("./remove-fenced-code.js");
...
module.exports.transform = async ({ src, filename, options }) => {
    ...
    if (['.js', '.cjs', '.mjs', '.ts'].includes(path.extname(filename))) {
        const [processedSource, didModify] = removeFencedCode(filename, { all: new Set(['flask']), active: new Set([]) }, src);
        if (didModify) {
