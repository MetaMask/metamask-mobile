...
Feature: Address
  ...
  Scenario: A user enters an invalid address
    ...
    And I enter invalid address "0xB8B4EE5B1b693971eB60bDa15211570df2dB221L" into senders input field
  ...
  Scenario: A user adds an address to their contacts from the send flow and confirms it is visible on the contacts view
    ...
    And I enter a contract address "0x1FDb169Ef12954F20A15852980e1F0C122BfC1D6" in the sender's input box
    ...
    When I enter in a contact name "TestAlias"
    Then the 'Save' button becomes enabled
    And I tap the 'Save' button
    And the contact name "TestAlias" appears in the senders input box above the contact address
    And I navigate to the main wallet screen
    And I tap on the burger menu
    And I tap on "Settings" in the menu
    And In settings I tap on "Contacts"
    Then the saved contact "TestAlias" should appear
  ...
  Scenario: A user can add a contact on the contacts view
    ...
    When I input "Birdis" into the contact name field
    And I input "curtis.eth" in the Address field
    And I tap button "Add contact" which is now enabled
    Then the saved contact "Birdis" should appear

  Scenario: A user has a saved address and edits that saved address
    Given I have a saved contact "Birdis" on the contacts view
    When I tap on contact name "Birdis"
    And I tap on 'Edit' button to edit Saved contact details
    Then I can edit the contact name to "James"
    When I tap the 'Edit Contact' button which is enabled to confirm the change
    Then the saved contact "James" should appear
    When I go back to the main wallet screen
    And On the Main Wallet view I tap "Send" navigating to the send screen
    Then I should see the edited name "James" contact under 'Recents' on the send screen
  ...
  Scenario: A user has a saved address then deletes it
    ...
    When I input "jande" into the contact name field
    And I input "mike.eth" in the Address field
    And I tap button "Add contact" which is now enabled
    Then the saved contact "jande" should appear
    When I tap on contact name "jande"
    And I tap on 'Edit' button to edit Saved contact details
    And I tap on button with text "Delete"
    And I tap on button with text "Delete"
    Then the deleted contact "jande" should not appear
    When I go back to the main wallet screen
    And On the Main Wallet view I tap "Send" navigating to the send screen
    Then the deleted contact "jande" should not appear
  ...
  Scenario: A user saves an address on mainnet and should not see that saved address on Goreli
    ...
    And I tap on button with text "Add contact"
    When I input "Gusto" into the contact name field
    And I input "guto.eth" in the Address field
    And I tap button "Add contact" which is now enabled
    Then the saved contact "Gusto" should appear
    When I go back to the main wallet screen
    And On the Main Wallet view I tap "Send" navigating to the send screen
    Then I should see the edited name "Gusto" contact under 'Recents' on the send screen
    And I navigate back to main wallet screen
    ...
    And I tap on "Settings" in the menu
    And In settings I tap on "Contacts"
    Then I should not see "gusto" appear in the contact list