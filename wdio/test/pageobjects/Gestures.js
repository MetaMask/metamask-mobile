...
class Gestures {
  ...
  static async typeText(elementId, text) {
    driver.$(`~${elementId}`).waitForExist();
    await driver.$(`~${elementId}`).touchAction('tap');
    return driver.$(`~${elementId}`).addValue(text + '\n');
  }
  ...
  static async swipeOnPercentage(from, to) {
    ...
    SCREEN_SIZE = SCREEN_SIZE || (await driver.getWindowRect()); // Get the start position on the screen for the swipe
