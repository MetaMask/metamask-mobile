...
- Position hooks (async state initialization in usePerpsLivePositions)
- Perps market action buttons (Long/Short vs Close/Modify rendering)
- PerpsController cached state
...
- [x] **3. Resolve branch and PR number:**
  - **If PR_NUMBER is set** — confirm you are on branch `fix/perps/tat-2236-0319-1419` in `/Users/deeeed/dev/metamask/metamask-mobile-3`. If not, set status to `blocked: wrong branch` and stop.
  - **If PR_NUMBER is empty** — create the branch and draft PR:
    ```bash
    cd /Users/deeeed/dev/metamask/metamask-mobile-3
    git checkout main && git pull origin main
    git checkout -b fix/perps/tat-2236-0319-1419
    git commit --allow-empty -m "feat: TAT-2236 — Button color flash when open a market with an open position"
    git push -u origin fix/perps/tat-2236-0319-1419
    unset GH_TOKEN && gh pr create --draft --title "TAT-2236: Button color flash when open a market with an open position" --body "WIP — automated fix for TAT-2236"
    ```
    Update `PR_NUMBER:` in the Task block above with the PR number from output.
- [x] **4. Confirm CDP is live:**
  ```bash
...
- [x] **5. Confirm Metro is running** — check `.agent/metro.log` exists and is recent.
- [x] **6. List available recipes and CDP expressions:**
  ```bash
...
- [x] **7. Navigate to the affected screen via CDP:**
  ```bash
  bash scripts/perps/agentic/app-navigate.sh <target>
  ```
- [x] **8. Observe the bug via CDP eval.** Use ES5 only — no arrow functions, no const/let, no template literals.
  ```javascript
  // GOOD — ES5
...
- **Never pause for user input** — this is a fully autonomous flow. After outputting text, immediately continue to the next step. Complete ALL 26 steps in a single uninterrupted run.
...
- **before.mp4 BEFORE code changes** — if the bug is visual, step 11 must complete before step 12. Skip only for non-visual bugs.
- **Recipe before code** — write recipe.json (step 10) before any source changes (step 14).
- **validate-recipe.sh must exit 0** — do not skip, do not proceed if it fails.
- **Stage only for the fix** — allowed commits: empty bootstrap, reproduction marker, marker cleanup. The marker cleanup commit must contain ONLY marker removal — never the fix.
- **Report must include reproduction commit** — the SHA from step 9 is evidence. Include it and the Metro log excerpt in the report.
