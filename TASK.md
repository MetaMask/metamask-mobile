...
BRANCH: fix/perps/tat-2236-0319-1615
PR_NUMBER: 27671
STATUS: working
SESSION: mm-3
...
Perps market detail screen — action button area (Long/Short vs Close Position/Modify). The position hook initializes with empty state before async resolution, causing a brief flash of wrong button state when navigating to a market with an existing position.

## Root Cause Hypothesis

> _Fill this in at step 11. Do not assume a root cause before investigating._
...
- [x] **3. Resolve branch and PR number:**
  - **If PR_NUMBER is set** — confirm you are on branch `fix/perps/tat-2236-0319-1615` in `/Users/deeeed/dev/metamask/metamask-mobile-3`. If not, set status to `blocked: wrong branch` and stop.
  - **If PR_NUMBER is empty** — create the branch and draft PR:
    ```bash
    cd /Users/deeeed/dev/metamask/metamask-mobile-3
    git checkout main && git pull origin main
    git checkout -b fix/perps/tat-2236-0319-1615
    git commit --allow-empty -m "feat: TAT-2236 — Button color flash when open a market with an open position"
    git push -u origin fix/perps/tat-2236-0319-1615
...
- [ ] **8. Add a DevLogger reproduction marker** in the relevant source code that fires exactly when the bug condition occurs:
  ```javascript
  ...
  ```
- [ ] **9. Write `automation/<PR_NUMBER>/recipe.json`** — an executable recipe that validates the fix via `validate-recipe.sh`. Must cover all acceptance criteria. See the Reference section below for schema and example.
  If you cannot write a valid recipe, set status to `blocked: cannot reproduce` and stop.
- [ ] **10. Record before.mp4 (if visual)** — if the bug has a visible UI symptom, record the recipe reproduction flow before any fix code is written. Use the video recording commands from the agentic toolkit. Skip if the bug is purely logical (e.g. wrong data, crash, race condition with no UI artifact).

> REMINDER: Do NOT stop or wait for user input. Continue autonomously through all steps.

### Investigate

- [ ] **11. Check Metro logs** for errors related to the affected area:
  ```bash
  grep -i "error\|warn\|fail" .agent/metro.log | tail -30
  ```
- [ ] **12. Use DevLogger and CDP to trace the issue.** Never guess — confirm root cause with a file:line reference. Read the relevant source files. Understand the data flow before changing anything. Update the Root Cause Hypothesis section above with your findings.

### Fix (minimal change only)

- [ ] **13. Make the minimal change** to fix the bug. No refactoring, no cleanup. Only modify files directly related to the fix.
- [ ] **14. Update or add tests** to cover the fix. If the affected code has existing tests, update them. If no tests exist for the specific bug condition, add a focused test.

### Validate (every step mandatory — do NOT skip any)

- [ ] **15. Run `yarn lint:fix`** — auto-fix eslint issues on changed files. STOP if unfixable errors remain.
- [ ] **16. Run `yarn format`** — auto-format with prettier. No need to check — just run it.
- [ ] **17. Run `yarn lint:tsc`** — STOP if you introduced new type errors. Fix before proceeding.
  ```bash
  ...
  ```
- [ ] **18. Run affected tests** — `yarn jest <specific-test-file> --no-coverage`. STOP if any test fails. Fix before proceeding. NEVER use `--findRelatedTests`.
- [ ] **19. Run `yarn coverage:analyze`** — target 80% newCodeCoverage on changed lines.
- [ ] **20. Run validate-recipe.sh** — must exit 0. Do NOT skip. Do NOT proceed if it fails.
  ```bash
  bash scripts/perps/agentic/validate-recipe.sh automation/<PR_NUMBER>/ --skip-manual
  ```
- [ ] **21. Record after.mp4** — same navigation path as before.mp4. Use the video recording commands from the agentic toolkit. Verify the file exists and is non-empty.
- [ ] **22. Remove the reproduction marker** added in step 8. **Only stage and commit the marker removal — do NOT include fix files in this commit.**
  ```bash
...
- [ ] **23. Write `automation/<PR_NUMBER>/report.md`** — must include ALL of the following:
  ...
  - **JIRA**: ticket number
- [ ] **24. Write `automation/<PR_NUMBER>/pr-description.md`** — PR description following the repo's PR template format:
  ```markdown
  ## **Description**
...
- [ ] **25. Commit the fix:**
  ```bash
  git add -A -- ':!automation' && git commit -m "fix: TAT-2236 — <one-line summary of the fix>"
  ```
  Verify no automation files are staged: `git diff --cached --name-only | grep -v '^automation/'`
- [ ] **26. Push the branch:**
  ```bash
  git push origin fix/perps/tat-2236-0319-1615
  ```
- [ ] **27. Update the PR description:**
  ```bash
  unset GH_TOKEN && gh pr edit <PR_NUMBER> --body-file automation/<PR_NUMBER>/pr-description.md
  ```
- [ ] **28. Update Status** — set `STATUS: done` in the Task block above.
- [ ] **29. Print final report to terminal.**

---

## Rules (non-negotiable)

- **Never pause for user input** — this is a fully autonomous flow. After outputting text, immediately continue to the next step. Complete ALL 29 steps in a single uninterrupted run.
...
- **Commit and push the fix before updating PR description** — allowed commits: empty bootstrap, reproduction marker, marker cleanup, and the fix commit (step 25). The marker cleanup commit must contain ONLY marker removal — never the fix.
