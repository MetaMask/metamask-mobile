...
// `eslint-plugin-import-x` ships separate CommonJS and ES module builds, so it
// must be imported (not required) here to stay object-identical with the copy
// registered by `@metamask/eslint-config-typescript`.
...
import routeIsolationZonesModule from './scripts/eslint-route-isolation-zones.js';
import frameworkConfig from './tests/framework/eslint-config.js';

const { routeIsolationZones } = routeIsolationZonesModule;

/**
 * `@metamask/eslint-config-typescript@14` newly pulls in jsdoc's
 * `recommended-typescript-error` preset, which reports around 18,000
 * pre-existing violations. Keep the preset off for now; the jsdoc rules the
 * shared config sets directly (`check-syntax`, `check-indentation`,
 * `no-types`) stay enforced.
 *
...
// format, so it has to be translated before ESLint can consume it.
...
export default defineConfig([
  {
    ignores: [
      // Flat config no longer skips dot-directories the way `.eslintrc` did.
      // Keep them out of scope: `.github/scripts` and `.yarn/plugins` are not
      // covered by `tsconfig.json`, so type-aware rules cannot run on them.
      '**/.*/**',
      '**/.*',
      '**/node_modules/**',
      '**/android/**',
  ...
  {
    linterOptions: {
      // ESLint v9 reports these by default. The repo has ~800 stale directives
      // and already opted out of the equivalent `eslint-comments` rule below,
      // so keep the reporting off until they can be cleaned up.
    ...
    extends: [
      ...
      // TODO: Add `@metamask/eslint-config` when ready
    ],
    ...
    rules: {
      ...
      // Redundant with TypeScript, and a source of false positives on
      // namespace-style imports such as `React.forwardRef`.
      // See https://typescript-eslint.io/troubleshooting/typed-linting/performance
      'import-x/no-named-as-default-member': 'off',
      // Added to `eslint-plugin-import-x`'s `warnings` preset in v4. It fires
      // on the widespread (and intentional) habit of renaming default imports,
      // such as `import styles from './Foo.styles'`.
      ...
      // `@react-native/eslint-config` enables these, but its `eslint-plugin-ft-flow`
      // is not ESLint v9 compatible and no source file here is Flow-annotated.
...

  {
    ...
    languageOptions: {
      parserOptions: {
        // The repo lints every TypeScript file against the root `tsconfig.json`
        // rather than letting typescript-eslint infer a project per file.
  ...
  },
  {
    ...
    ignores: ['tests/smoke-appium/**/*.test.ts'],
    rules: {
      'no-restricted-syntax': [
        'error',
        {
  ...
  },
  {
    ...
    // `ignores` applies to the whole config entry — listing burn-down paths
    // here would incorrectly skip expo/perps for those files, so burn-down is
    // excluded from *this* block only and picked up by the next entry.
    ...
    ignores: [
      ...
      'app/util/number/**',
      // BN.js → BigInt burn-down: still allowed util/number imports; see next entry.
    ...
    rules: {
      ...
      // burn-down allowlist from this entry's `ignores`; the burn-down
      // entry below intentionally does not re-declare this rule, so
      // allow-listed files remain exempt.
  ...
  },
  {
    ...
    // config entry is required because ESLint replaces `no-restricted-imports`
    // when the same rule is set again — we cannot use one entry with only
    // `ignores` for util/number without silently dropping other fences.
  ...
  },
  {
    ...
    // `app/components/Views/**`. Declared in its own entry because the
    // BN.js migration entry above uses `ignores`, which would otherwise
    // silently exempt every Views file on the burn-down list from route
    // isolation. Re-declares the BN.js zone here so that non-burn-down
    // Views files keep their BN.js fence (ESLint replaces
    // `import-x/no-restricted-paths` per entry rather than merging).
    // See scripts/eslint-route-isolation-zones.js for zone generation.
    files: ['app/components/Views/**/*.{ts,tsx,js,jsx}'],
    ignores: utilNumberImportBurndownFiles,
    rules: {
      'import-x/no-restricted-paths': [
        'error',
        {
  ...
  },
  {
    ...
    // paths, so this entry is a no-op for non-Views burn-down files.
    files: utilNumberImportBurndownFiles,
    rules: {
      'import-x/no-restricted-paths': ['error', { zones: routeIsolationZones }],
    },
  },
]);
